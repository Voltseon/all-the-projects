...
class DayCare
  ...
  module EggGenerator
    ...
    def generate(mother, father)
      ...
      egg.calc_stats
      egg.hp = 1 if $player.onehp && egg.hp > 0
      return egg
    end
    ...
    def get_moves_to_inherit(egg, mother, father)
      ...
      if Settings.pbGetGeneration <= 5 && !move_father.female?
      ...
      if Settings.pbGetGeneration >= 6 && move_mother.female?
        egg.species_data.egg_moves.each do |move|
    ...
    def set_shininess(egg, mother, father)
      ...
      if father.owner.language != mother.owner.language
        shiny_retries += (Settings.pbGetGeneration >= 8) ? 6 : 5
      end
    ...
    def set_pokerus(egg)
      egg.givePokerus if rand(65_536) < Settings::POKERUS_CHANCE && !$player.murphyslaw
    end
  end
  ...
  class DayCareSlot
    ...
    def add_exp(amount = 1)
      ...
      @pokemon.calc_stats
      @pokemon.hp = 1 if $player.onehp && @pokemon.hp > 0
  ...
  def initialize
    ...
    reset_egg_counters
    @gain_exp = Settings.pbGetGeneration <= 6 && !$player.levelone
    @share_egg_moves = Settings.pbGetGeneration >= 8
  end
  ...
  def update_on_step_taken
    ...
    if @step_counter >= 256
      ...
      if !@egg_generated && count == 2 && !$player.murphyslaw

...
def pbFishing(hasEncounter, rodType = 1)
  ...
  biteChance = 20 + (25 * rodType)   # 45, 70, 95, 100
  biteChance *= 1.5 if speedup   # 67.5, 100, 100
  hookChance = 100
  biteChance = 0 if $player.murphyslaw && biteChance < 100
  pbFishingBegin
  ...
  loop do
    ...
    if hasEncounter && rand(100) < biteChance
      ...
      if Settings::FISHING_AUTO_HOOK || rand(100) < hookChance || rodType == 4
        pbFishingEnd {
          pbMessageDisplay(msgWindow, _INTL("Landed a Pokémon!")) if !Settings::FISHING_AUTO_HOOK && rodType != 4
        }
        ret = true
        break
      end

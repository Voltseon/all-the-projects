...
# Show puddle animations
EventHandlers.add(:on_step_taken, :puddle_rustling,
  proc { |event|
    next if !$scene.is_a?(Scene_Map)
    event.each_occupied_tile do |x, y|
      next if !$map_factory.getTerrainTagFromCoords(event.map.map_id, x, y, true).shows_puddle
      spriteset = $scene.spriteset(event.map_id)
      spriteset&.addUserAnimation(8, x, y, true, 1)
    end
  }
)

# Show puddle animations
EventHandlers.add(:on_leave_tile, :puddle_rustling_leave,
  proc { |event, map, oldX, oldY|
    next if !$scene.is_a?(Scene_Map)
    next if !$map_factory.getTerrainTagFromCoords(event.map.map_id, oldX, oldY, true).shows_puddle
    spriteset = $scene.spriteset(event.map_id)
    spriteset&.addUserAnimation(8, oldX, oldY, true, 1)
  }
)
...
def pbWait(numFrames, continue_game = false)
  numFrames.times do
    Graphics.update
    Input.update
    continue_game ? $scene.update : pbUpdateSceneMap
  end
end

...
class Game_Temp
  ...
  attr_accessor :encounter_grace

  def encounter_grace
    @encounter_grace = 0 if !@encounter_grace
    return @encounter_grace
  end
end
...
EventHandlers.add(:on_player_step_taken_can_transfer, :poison_party,
  proc { |handled|
    ...
    next if handled[0]
    next if Settings.pbGetGeneration > 4 || $PokemonGlobal.stepcount % 4 != 0
    flashed = false
    $player.able_party.each do |pkmn|
      ...
      pkmn.hp -= 1 if pkmn.hp > 1 || Settings.pbGetGeneration <= 3
      if pkmn.hp == 1 && Settings.pbGetGeneration > 3
        pkmn.status = :NONE
...
# Lose HP on Step Challenge
EventHandlers.add(:on_player_step_taken_can_transfer, :damage_on_step_challenge,
  proc { |handled|
    next unless $player.losehponstep
    next if handled[0]
    $player.able_party.each do |pkmn|
      pkmn.hp -= 1
      if pkmn.hp == 0
...
def pbBattleOnStepTaken(repel_active)
  ...
  return if $game_temp.encounter_grace < 4 && !$player.murphyslaw
...
EventHandlers.add(:on_enter_map, :setup_new_map,
  proc { |old_map_id|   # previous map ID, is 0 if no map ID
    ...
    if $game_map.metadata&.weather
      new_weather = new_map_metadata.weather
    end
    if $game_map.metadata&.weather && rand(100) < new_weather[1]
      $game_screen.weather(new_weather[0], 9, 0)
    else
      new_weather = :None
      next if rand(2) >= 1
      if GameData::Weather.get($game_screen.weather_type).category == :None &&
        $game_map.metadata&.outdoor_map
        rain_chance = pbGetSeason == 2 ? 2 : 10
        sun_chance = pbGetSeason < 2 ? 5 : (pbGetSeason == 3) ? 20 : 10
        snow_chance = pbGetSeason == 3 ? 3 : (pbGetSeason == 1) ? 100 : 20
        new_weather = :None
        new_weather = :Rain if rand(rain_chance) < 1 && new_weather == :None && !$game_map.metadata&.flags.include?("CannotRain") && $player.weatherrain
        new_weather = :Sun if rand(sun_chance) < 1 && PBDayNight.isDay? && new_weather == :None && !$game_map.metadata&.flags.include?("CannotSun") && $player.weathersun
        new_weather = :Snow if rand(snow_chance) < 1 && new_weather == :None && !$game_map.metadata&.flags.include?("CannotHail") && $player.weatherhail
        if !$player.weatherclear
          possible_weathers = []
          possible_weathers.push(:Rain) if $player.weatherrain && new_weather != :Rain
          possible_weathers.push(:Sun) if $player.weathersun && new_weather != :Sun
          possible_weathers.push(:Snow) if $player.weatherhail && new_weather != :Snow
          possible_weathers.push(:Sandstorm) if $player.weathersandstorm && new_weather != :Sandstorm
          new_weather = possible_weathers.sample
        end
...
EventHandlers.add(:on_map_or_spriteset_change, :show_darkness,
  proc { |scene, _map_changed|
    ...
    if map_metadata&.dark_map
      if $game_temp.darkness_sprite.nil?
...
def pbEventCanReachPlayer?(event, player, distance)
  return false if $game_temp.shadow_clear?
...
def pbLedge(_xOffset, _yOffset)
  if $game_player.pbFacingTerrainTag.ledge
    if pbJumpToward(2, true)
      $scene.spriteset.addUserAnimation(($PokemonGlobal&.diving ? Settings::DUST_ANIMATION_DIVE_ID : Settings::DUST_ANIMATION_ID), $game_player.x, $game_player.y, true, 1)
...
def pbMoveTowardPlayer(event,should_wait=true)
...
def pbItemBall(item, quantity = 1)
  ...
  if $bag.add(item, quantity)   # If item can be picked up
    meName = (item.is_key_item?) ? "Key item get" : (item.is_mega_stone?) ? "Mega stone get" : "RSE 220 Obtained an Item!"
    if item == :LEFTOVERS

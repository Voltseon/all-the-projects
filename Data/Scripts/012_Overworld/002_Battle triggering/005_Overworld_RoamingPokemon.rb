...
def pbResetAllRoamers
  return false if !$PokemonGlobal.roamPokemon
  ret = false
...
def pbGetRoamer(index)
  data = Settings::ROAMING_SPECIES[index]
  return data
end
...
def pbRoamingAreas(idxRoamer)
  ...
  roamData = pbGetRoamer(idxRoamer)
...
def pbRoamPokemon
  ...
  if !$PokemonGlobal.roamPosition
    ...
    Settings::ROAMING_SPECIES.length.times do |i|
      next if !GameData::Species.exists?(pbGetRoamer(i)[0])
...
def pbRoamPokemonOne(idxRoamer)
  ...
  if !currentMap || !mapIDs.include?(currentMap)
  ...
  if newMapChoices.length > 0
    ...
    Pokemon.play_cry(roamData[0]) if $PokemonGlobal.roamPosition[idxRoamer] == $game_map.map_id
  end
end

...
class PokemonEncounters
  ...
  def setup(map_ID)
    ...
    @step_chances     = {}
    @encounter_tables = {}
    encounter_data = $player.wild_encounters[[map_ID, $PokemonGlobal.encounter_version]]
    if encounter_data
      encounter_data[1].each { |type, value| @step_chances[type] = value }
      @encounter_tables = Marshal.load(Marshal.dump(encounter_data[2]))
    end
  end
  ...
  def map_has_encounter_type?(map_ID, enc_type)
    ...
    return encounter_data[2][enc_type] && encounter_data[2][enc_type].length > 0
  end
  ...
  def encounter_triggered?(enc_type, repel_active = false, triggered_by_step = true)
    ...
    # Murphy's Law
    return true if $player.murphyslaw
    ...
    encounter_chance /= 1.5 if $game_player.move_speed < 4 && !$PokemonGlobal&.bicycle && !$PokemonGlobal&.surfing && !$PokemonGlobal&.diving && $PokemonGlobal&.mounted_pkmn != -1
  ...
  def allow_encounter?(enc_data, repel_active = false)
    ...
    # Repel
    if repel_active
      first_pkmn = (Settings.pbGetGeneration >= 6) ? $player.first_pokemon : $player.first_able_pokemon
  ...
  def find_valid_encounter_type_for_time(base_type, time)
    ret = nil
    weather = GameData::Weather.get($game_screen.weather_type).category
    if weather != :None
      try_type = nil
      if weather == :Sun
        try_type = (base_type.to_s + "Sunny").to_sym
      elsif weather == :Rain
        try_type = (base_type.to_s + "Rain").to_sym
      elsif weather == :Sandstorm
        try_type = (base_type.to_s + "Sandstorm").to_sym
      elsif weather == :Hail
        try_type = (base_type.to_s + "Hail").to_sym
      end
  ...
  def choose_wild_pokemon(enc_type, chance_rolls = 1)
    ...
    if first_pkmn
      ...
      case first_pkmn.ability_id
      when :FLASHFIRE
        favored_type = :FIRE if Settings.pbGetGeneration >= 8 &&
                                GameData::Type.exists?(:FIRE) && rand(100) < 50
      when :HARVEST
        favored_type = :GRASS if Settings.pbGetGeneration >= 8 &&
                                 GameData::Type.exists?(:GRASS) && rand(100) < 50
      when :LIGHTNINGROD
        favored_type = :ELECTRIC if Settings.pbGetGeneration >= 8 &&
      ...
      when :STORMDRAIN
        favored_type = :WATER if Settings.pbGetGeneration >= 8 &&
  ...
  def choose_wild_pokemon_for_map(map_ID, enc_type)
    ...
    enc_list = encounter_data[2][enc_type]
    return nil if !enc_list || enc_list.length == 0
    # Calculate the total probability value
    chance_total = 0
    enc_list.each { |a| chance_total += a[0] }
...
def pbGenerateWildPokemon(species, level, isRoamer = false)
  ...
  if first_pkmn
    ...
    when :COMPOUNDEYES
      chances = [60, 20, 5]
    when :SUPERLUCK
      chances = [60, 20, 5] if Settings.pbGetGeneration >= 8
    end
    chances = [60, 20, 5] if first_pkmn.super_shiny?
  end
  itemrnd = rand(100)
  ...
  # Give Pokérus
  genwildpoke.givePokerus if rand(65_536) < Settings::POKERUS_CHANCE && !$player.murphyslaw
  ...
  if first_pkmn
    if first_pkmn.hasAbility?(:CUTECHARM) && !genwildpoke.singleGendered?
      ...
      if !isRoamer && (Settings.pbGetGeneration >= 8 || (rand(100) < 50))

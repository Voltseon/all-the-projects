...
module BattleCreationHelperMethods
  ...
  def prepare_battle(battle)
    ...
    battle.sendToBoxes = $PokemonSystem.sendtoboxes if Settings.pbGetGeneration >= 7
    ...
    # Terrain
    if battleRules["defaultTerrain"].nil? && Settings.pbGetGeneration >= 8
...
class WildBattle
  ...
  def self.start_core(*args)
    ...
    # Randomizer
    randomize_before_battle(foe_party) if $player.random_before_battles
...
class TrainerBattle
  ...
  def self.start_core(*args)
    setBattleRule($player.battle_pokemon_usage) unless $player.battle_pokemon_usage.nil?
    outcome_variable = $game_temp.battle_rules["outcomeVar"] || 1
    can_lose         = $game_temp.battle_rules["canLose"] || false
    # Skip battle if the player has no able Pokémon, or if holding Ctrl in Debug mode
    if BattleCreationHelperMethods.skip_battle?
...
EventHandlers.add(:on_end_battle, :evolve_and_black_out,
  proc { |decision, canLose|
    ...
    pbEvolutionCheck if Settings.pbGetGeneration >= 6 ||
...
PICKUP_COMMON_ITEMS = [
  ...
  :BIGMUSHROOM,   # Levels                                   51-60, 61-70, 71-80, 81-90, 91-100
...
def pbPickup(pkmn)
  ...
  common_items = pbDynamicItemList($player.special_items(:PICKUP_COMMON_ITEMS))
  rare_items = pbDynamicItemList($player.special_items(:PICKUP_RARE_ITEMS))
...
def pbHoneyGather(pkmn)
  ...
  return if pkmn.egg? || !pkmn.hasAbility?(:HONEYGATHER) || pkmn.hasItem? || $player.murphyslaw
  chance = 5 + (((pkmn.level - 1) / 10) * 5)
  return unless rand(100) < chance
  pkmn.item = $player.special_items(:HONEYGATHER)
end

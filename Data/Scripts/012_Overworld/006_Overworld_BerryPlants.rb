...
def pbBerryPlant
  ...
  when 5  # X berries
    ...
    if pbConfirmMessage(message)
      ...
      # Random Gold Berry
      if (rand(100)==1)
        pbMessage(_INTL("You got lucky and received a Gold Berry as well!"))
        $PokemonBag.pbStoreItem(:GOLDBERRY,1)
      end
...
def pbPickBerry(berry, qty = 1)
  ...
  if pbConfirmMessage(message)
    ...
    if qty>1
      pbMessage(_INTL("\\me[Item get]You picked the {1} \\c[1]{2}\\c[0].\\wtnp[30]",qty,itemname))
    else
      pbMessage(_INTL("\\me[Item get]You picked the \\c[1]{1}\\c[0].\\wtnp[30]",itemname))
    end
    pocket = berry.pocket
    pbMessage(_INTL("{1} put the \\c[1]{2}\\c[0] in the <icon=bagPocket{3}>\\c[1]{4}\\c[0] Pocket.\1",
       $Trainer.name,itemname,pocket,PokemonBag.pocketNames()[pocket]))

...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT,false) || (!$DEBUG && !vHI(:THINBLADE))
...
def pbDive
  ...
  move = :DIVE
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE,false) || (!$DEBUG && !vHI(:SCUBAGEAR))
    pbMessage(_INTL("The sea is deep here. A Pokémon may be able to go underwater."))
    return false
  end
  if pbConfirmMessage(_INTL("The sea is deep here. Would you like to dive?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    #pbMessage(_INTL("{1} used {2}!",speciesname,GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn {
...
def pbSurfacing
  ...
  if pbConfirmMessage(_INTL("Light is filtering down from above. Would you like to surface?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    #pbMessage(_INTL("{1} used {2}!",speciesname,GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn {
...
def pbRockSmash
  move = :ROCKSMASH
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH,false) || (!$DEBUG && !vHI(:STURDYPICKAXE))
    pbMessage(_INTL("It's a rugged rock, but a Pokémon may be able to smash it."))
    return false
  end
  if pbConfirmMessage(_INTL("This rock appears to be breakable. Would you like to smash it?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    #pbMessage(_INTL("{1} used {2}!",speciesname,GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    return true
...
def pbStrength
  if $PokemonMap.strengthUsed
    pbMessage(_INTL("It's possible to move boulders around."))
    return false
  end
  move = :STRENGTH
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH,false) || (!$DEBUG && !vHI(:PRESSUREPISTON))
  ...
  if pbConfirmMessage(_INTL("Would you like to move boulders?"))
    speciesname = (movefinder) ? movefinder.name : $Trainer.name
    #pbMessage(_INTL("{1} used {2}!",speciesname,GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbMessage(_INTL("It's now possible to move boulders around!"))
...
def pbSurf
  ...
  move = :SURF
  movefinder = $Trainer.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_SURF,false) || (!$DEBUG && !vHI(:FLOATIE))
    return false
  end
...
# Scale Replenish
#===============================================================================
HiddenMoveHandlers::CanUseMove.add(:SCALEYREPLENISH,proc { |move,pkmn,showmsg|
  if ![:BURN, :POISON, :PARALYSIS, :SLEEP, :FROZEN, :PETRIFIED].include?(pkmn.status)
    pbMessage(_INTL("Cant use that now."))
    next false
  end
  next true
})

HiddenMoveHandlers::UseMove.add(:SCALEYREPLENISH,proc { |move,pokemon|
  old_status = pokemon.status
  pokemon.status = :NONE
  case old_status
  when :BURN
    pbMessage(_INTL("{1} healed its burn!",pokemon.name))
  when :POISON
    pbMessage(_INTL("{1} cured its poisoning!",pokemon.name))
  when :PARALYSIS
    pbMessage(_INTL("{1} cured its paralysis!",pokemon.name))
  when :SLEEP
    pbMessage(_INTL("{1} woke up!",pokemon.name))
  when :FROZEN
    pbMessage(_INTL("{1} thawed out!",pokemon.name))
  when :PETRIFIED
    pbMessage(_INTL("{1} broke free from its petrification!",pokemon.name))
  end
  next true
})
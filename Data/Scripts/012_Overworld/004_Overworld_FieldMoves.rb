...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!$DEBUG && (!movefinder && !(vHI(:HM01) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
...
HiddenMoveHandlers::CanUseMove.add(:CUT, proc { |move, pkmn, showmsg|
  ...
  unless (facingEvent && facingEvent.name[/cuttree/i]) || [:Grass, :TallGrass].include?($game_player.terrain_tag.id)
...
HiddenMoveHandlers::UseMove.add(:CUT, proc { |move, pokemon|
  ...
  elsif [:Grass, :TallGrass].include?($game_player.terrain_tag.id)
    range = [-1, 0, 1]
    range += [-2, 2] if pokemon.hasAbility?(:HYPERCUTTER)
    pbSEPlay("Cut", 80)
    range.each do |x|
      range.each do |y|
        if pbPhenomenonActive? && $game_temp.phenomenon.type == :PhenomenonGrass
          pbPhenomenonCancel if $game_temp.phenomenon.x == $game_player.x + x && $game_temp.phenomenon.y == $game_player.y + y
        end
        [0, 1, 2].each do |layer|
          tile = $game_map.get_tile($game_player.x + x, $game_player.y + y, layer)
          next unless [394, 396, 392, 2487, 2617, 2619, 2989].include?(tile)
          set_tile = (tile == 2487) ? 2477 : (tile == 2619) ? 2610 : (tile == 2989) ? 2969 : (layer == 0) ? 387 : 0
          $game_map.set_tile($game_player.x + x, $game_player.y + y, layer, set_tile)
          if tile == 396
            lay = ($game_map.get_tile($game_player.x + x, $game_player.y + y - 1, 1) == 388) ? 1 : 2
            $game_map.set_tile($game_player.x + x, $game_player.y + y - 1, lay, 0)
          elsif tile == 2619
            lay = ($game_map.get_tile($game_player.x + x, $game_player.y + y - 1, 1) == 2611) ? 1 : 2
            $game_map.set_tile($game_player.x + x, $game_player.y + y - 1, lay, 0)
          end
          $scene.spriteset.addUserAnimation(Settings::GRASS_ANIMATION_ID, $game_player.x + x, $game_player.y + y, true, 1)
        end
        $game_map.events.each_value do |event|
          next if event.x != $game_player.x + x && event.y != $game_player.y + y
          event.calculate_bush_depth
        end
        FollowingPkmn.get_event.calculate_bush_depth if FollowingPkmn && FollowingPkmn.active?
        $game_player.calculate_bush_depth
      end
    end
  end
  next true
...
def pbSmashEvent(event)
  ...
  elsif event.name[/lashrock/i]
    $scene.spriteset.addUserAnimation(18, event.x, event.y, true, 1)
    pbSEPlay("Fire Lash", 80)
  end
...
def pbDive
  ...
  move = :DIVE
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && (!movefinder && !(vHI(:HM06) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
...
def pbCanSurface?(showmsg = false)
  surface_map_id = nil
  GameData::MapMetadata.each do |map_data|
    next if !map_data.dive_map_id || map_data.dive_map_id != $game_map.map_id
    surface_map_id = map_data.id
...
HiddenMoveHandlers::UseMove.add(:FLASH, proc { |move, pokemon|
  next pbFlash(move, pokemon)
})

def pbFlash(move = :FLASH, pokemon = nil)
  darkness = $game_temp.darkness_sprite
  return false if !darkness || darkness.disposed?
  if !pokemon.nil? && !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} used {2}!", pokemon.name, GameData::Move.get(move).name))
  end
...
def pbCanFly?(pkmn = nil, show_messages = false)
  ...
  return false unless $DEBUG || $player.get_pokemon_with_move(:FLY) || (vHI(:HM02) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))
  if !$game_player.can_map_transfer_with_follower?
    pbMessage(_INTL("It can't be used when you have someone with you.")) if show_messages
    return false
  end
  if !$game_map.metadata&.outdoor_map || ($PokemonGlobal.escapePoint rescue false) && $PokemonGlobal.escapePoint.length > 0
...
def pbRockSmash
  move = :ROCKSMASH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && (!movefinder && !(vHI(:HM05) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
...
# Fire Lash
#===============================================================================
def pbFireLashRandomEncounter
  if $PokemonEncounters.encounter_triggered?(:FireLash, false, false)
    $stats.fire_lash_battles += 1
    pbEncounter(:FireLash)
  end
end
...
def pbFireLash
  move = :FIRELASH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_FIRELASH, false) || (!$DEBUG && (!movefinder && !(vHI(:HM07) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
    pbMessage(_INTL("It's an icy rock, but a Pokémon may be able to thaw it."))
    return false
  end
  if pbConfirmMessage(_INTL("This icy rock seems thawable with a hidden move.\nWould you like to use Fire Lash?"))
...
HiddenMoveHandlers::CanUseMove.add(:FIRELASH, proc { |move, pkmn, showmsg|
  next false if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_FIRELASH, showmsg)
  facingEvent = $game_player.pbFacingEvent
  if !facingEvent || !facingEvent.name[/lashrock/i]
    pbMessage(_INTL("You can't use that here.")) if showmsg
    next false
  end
  next true
...
HiddenMoveHandlers::UseMove.add(:FIRELASH, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} used {2}!", pokemon.name, GameData::Move.get(move).name))
  end
  $stats.fire_lash_count += 1
...
def pbStrength
  ...
  move = :STRENGTH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!$DEBUG && (!movefinder && !(vHI(:HM04) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
...
HiddenMoveHandlers::CanUseMove.add(:STRENGTH, proc { |move, pkmn, showmsg|
  next false if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, showmsg) || !pbBoulderInMap?
  if $PokemonMap.strengthUsed
...
def pbBoulderInMap?
  $game_map.events.each do |event|
    return true if event[1].name[/strengthboulder/i]
  end
  return false
end
...
def pbSurf
  ...
  move = :SURF
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_SURF, false) || (!$DEBUG && (!movefinder && !(vHI(:HM03) && (vHI(:HMEMULATOR) || $player.rocketmode || $player.metronome))))
    return false
  end
...
HiddenMoveHandlers::CanUseMove.add(:SURF, proc { |move, pkmn, showmsg|
  ...
  if $game_temp&.heart_swapped
    pbMessage(_INTL("You can't use that now.")) if showmsg
    next false
  end
...
def pbWaterfall
  move = :WATERFALL
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(9, false) || (!$DEBUG && !movefinder)

...
def pbSetGameTime(hour = 12, minute = 0, second = 0)
  $game_variables[PBDayNight::TIME_VARIABLE] = Time.new(2000, 1, 1, hour, minute, second)
end

def pbGetGameTime
  if $game_variables[PBDayNight::TIME_VARIABLE].is_a?(Time)
    return $game_variables[PBDayNight::TIME_VARIABLE]
  else
    return Time.new(2000, 1, 1, 12, 0, 0)
  end
end
...
module PBDayNight
  ...
  def self.pbGetDayNightMinutes
    now = pbGetGameTime   # Get the current in-game time

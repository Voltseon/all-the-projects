...
MenuHandlers.add(:battle_pokemon_debug_menu, :set_shadowness, {
  "name"   => _INTL("Set shadownes"),
  "parent" => :main,
  "usage"  => :both,
  "effect" => proc { |pkmn, battler, battle|
    cmd = 0
    loop do
      msg_idx = pkmn.shadowPokemon? ? 1 : 0
      msg = [_INTL("Is normal."), _INTL("Is shadow.")][msg_idx]
      cmd = pbMessage("\\ts[]" + msg,
                      [_INTL("Make normal"),
                       _INTL("Make shadow")], -1, nil, cmd)
      break if cmd < 0
      case cmd
      when 0   # Make normal
        pbPurify(pkmn, self)

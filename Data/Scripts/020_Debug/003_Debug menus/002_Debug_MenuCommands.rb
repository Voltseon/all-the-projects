...
DebugMenuCommands.register("demoparty", {
  ...
  "effect"      => proc {
    party = []
    species = [:BAHORN, :OCOLURE, :TRAPPET, :RAPTEAR, :MOUSNOT, :RABBUN]
    for id in species
...
DebugMenuCommands.register("fillfully", {
  "parent"      => "pokemonmenu",
  "name"        => _INTL("Fill Storage Boxes Fully"),
  "description" => _INTL("Add Pokémon to storage until full."),
  "effect"      => proc {
    added = 0
    box_qty = $PokemonStorage.maxPokemon(0)
    completed = true
    pokelist = []
    while pokelist.length < $PokemonStorage.maxBoxes * PokemonBox::BOX_SIZE
      pokelist.push(GameData::Species.get(rand(1..76)))
    end
    pokelist.each do |species_data|
      sp = species_data.species
      f = species_data.form
      # Record each form of each species as seen and owned
      if f == 0

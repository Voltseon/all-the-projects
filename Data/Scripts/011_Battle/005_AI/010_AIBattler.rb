...
class Battle::AI::AIBattler
  ...
  def rough_end_of_round_damage
    ...
    if self.status == :POISON
      ...
      if battler.takesIndirectDamage?
        amt = (Settings.pbGetGeneration >= 7) ? self.totalhp / 16 : self.totalhp / 8
    ...
    if self.effects[PBEffects::Trapping] > 1 && battler.takesIndirectDamage?
      amt = (Settings.pbGetGeneration >= 6) ? self.totalhp / 8 : self.totalhp / 16
      if @ai.battlers[self.effects[PBEffects::TrappingUser]].has_active_item?(:BINDINGBAND)
        amt = (Settings.pbGetGeneration >= 6) ? self.totalhp / 6 : self.totalhp / 8
      end
      ret += [amt, 1].max
    end
    # Perish Song
  ...
  def can_become_trapped?
    ...
    return false if Settings.pbGetGeneration >= 6 && has_type?(:GHOST)

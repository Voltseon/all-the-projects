...
Battle::AI::Handlers::ItemRanking.add(:AGUAVBERRY,
  proc { |item, score, battler, ai|
    if Settings.pbGetGeneration == 7   # Heals 50%
      score += 2
    elsif Settings.pbGetGeneration <= 6   # Heals 12.5%
      score -= 3
    end
...
Battle::AI::Handlers::ItemRanking.addIf(:gems,
  ...
  proc { |item, score, battler, ai|
    score += 2 if Settings.pbGetGeneration <= 5   # 1.5x boost rather than 1.3x
    boosted_type = {
      :BUGGEM      => :BUG,
      :DARKGEM     => :DARK,
      :DRAGONGEM   => :DRAGON,

...
Battle::AI::Handlers::GeneralMoveScore.add(:shadow_moves,
  proc { |score, move, user, ai, battle|
    if move.rough_type == :SHADOW || move.rough_type == :LIGHT
      old_score = score
      score += 10
      PBDebug.log_score_change(score - old_score, "prefer using a Shadow/Light move")
    end
    next score
  }
)
  ...
  proc { |score, move, user, target, ai, battle|
    if ai.trainer.high_skill? && target.faster_than?(user) && move.rough_priority(user) > 0
      ...
      ai.each_foe_battler(user.side) do |b, i|
        ...
        next if Settings.pbGetGeneration <= 5 && b.effects[PBEffects::ProtectRate] > 1
        score -= 5
      end
      if score != old_score
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:thawing_move_against_frozen_target,
  proc { |score, move, user, target, ai, battle|
    if ai.trainer.medium_skill? && target.status == :FROZEN
      if move.rough_type == :FIRE || (Settings.pbGetGeneration >= 6 && move.move.thawsUser?)
        old_score = score
        score -= 20
    ...
    if ai.trainer.high_skill? && move.damagingMove? && target.effects[PBEffects::Substitute] == 0
      if target.ability_active?
        if Battle::AbilityEffects::OnBeingHit[target.ability] ||
           ...
           (!user.has_active_ability?(:SHEERFORCE) || move.move.addlEffect == 0 || $player.nomoveeffects))
          old_score = score
          score += 8

...
class Battle::AI::AIMove
  ...
  def rough_damage
    ...
    if user.ability_active?
      ...
      when :NORMALIZE
        multipliers[:power_multiplier] *= 1.2 if Settings.pbGetGeneration >= 7
      when :SNIPER
    ...
    if user.has_active_ability?(:PARENTALBOND)
      multipliers[:power_multiplier] *= (Settings.pbGetGeneration >= 7) ? 1.25 : 1.5
    end
    # "of Ruin" abilities
    if @ai.trainer.medium_skill?
      [:TABLETSOFRUIN, :SWORDOFRUIN, :VESSELOFRUIN, :BEADSOFRUIN].each_with_index do |abil, i|
        ...
        mult = (i.even?) ? multipliers[:power_multiplier] : multipliers[:defense_multiplier]
        mult *= 0.75 if @ai.battle.pbCheckGlobalAbility(abil) && user.has_active_ability?(abil) && category
      end
    end
    ...
    # Terrain moves
    if @ai.trainer.medium_skill?
      terrain_multiplier = (Settings.pbGetGeneration >= 8) ? 1.3 : 1.5
    ...
    if @ai.trainer.medium_skill?
      ...
      when :Sun, :HarshSun
        ...
        when :WATER
          multipliers[:final_damage_multiplier] /= 2 if @move.function_code != "IncreasePowerInSunWeather"
        end
      when :Rain, :HeavyRain
        case calc_type
        when :FIRE
      ...
      when :Hail
        if Settings::HAIL_WEATHER_TYPE > 0 && target.has_type?(:ICE) && physicalMove?(calc_type) &&
    ...
    # Glaive Rush
    if @ai.trainer.medium_skill? && target.effects[PBEffects::GlaiveRush] > 0
      multipliers[:final_damage_multiplier] *= 2
    end
  ...
  def apply_rough_accuracy_modifiers(user, target, calc_type, modifiers)
    ...
    if @ai.trainer.medium_skill?
      ...
      when "BadPoisonTarget"
        modifiers[:base_accuracy] = 0 if Settings.pbGetGeneration >= 6 &&
  ...
  def get_score_change_for_additional_effect(user, target = nil)
    ...
    return -999 if user.has_active_ability?(:SHEERFORCE) || $player.nomoveeffects
    ...
    return 5 if @move.addlEffect < 100 &&
                (Settings.pbGetGeneration >= 6 || function_code != "EffectDependsOnEnvironment") &&

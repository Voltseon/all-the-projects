...
class Battle::AI
  ...
  def pbChooseToSwitchOut(terrible_moves = false)
    ...
    if @user.has_active_ability?(:ZEROTOHERO) && @user.battler.form == 0
      switchFunctions = [
      ...
      @user.eachMoveWithIndex do |m, i|
        next if !switchFunctions.include?(m.function_code)
        next if !@battle.pbCanChooseMove?(@user.index, i, false)
        if @battle.pbRegisterMove(@user.index, i, false)
          PBDebug.log("   => will use #{m.name} to switch out")
          return true
        end
      end
    end
  ...
  def calculate_entry_hazard_damage(pkmn, side)
    ...
    if @battle.sides[side].effects[PBEffects::StealthRock] && GameData::Type.exists?(:ROCK)
      ...
      ret += pkmn.totalhp * eff / 8 if !Effectiveness.ineffective?(eff) || @field.weather != :LightOasis
    end
    # Spikes
  ...
  proc { |battler, reserves, ai, battle|
    ...
    if ai.trainer.high_skill?
      ...
      ai.each_foe_battler(battler.side) do |b, i|
        ...
        next if Settings.pbGetGeneration >= 6 && b.has_type?(:GHOST)
...
Battle::AI::Handlers::ShouldSwitch.add(:high_damage_from_foe,
  proc { |battler, reserves, ai, battle|
    ...
    ai.each_foe_battler(battler.side) do |b, i|
      ...
      switch_chance = (move_data.baseDamage > 90) ? 50 : 25

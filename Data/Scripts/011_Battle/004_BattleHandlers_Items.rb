...
BattleHandlers::StatusCureItem.add(:EMONBERRY,
  proc { |item,battler,battle,forced|
    next false if !forced && !battler.canConsumeBerry?
    next false if battler.status != :PETRIFIED
    itemName = GameData::Item.get(item).name
    PBDebug.log("[Item triggered] #{battler.pbThis}'s #{itemName}") if forced
    battle.pbCommonAnimation("EatBerry",battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("{1}'s {2} broke it out of petrification!",battler.pbThis,itemName)) if !forced
    next true
  }
)
...
BattleHandlers::StatusCureItem.add(:LUMBERRY,
  proc { |item,battler,battle,forced|
    ...
    if forced
      ...
      when :PETRIFIED
        battle.pbDisplay(_INTL("{1}'s {2} depetrified it!",battler.pbThis,itemName))
      end
      if oldConfusion
...
BattleHandlers::EORHealingItem.add(:PURITYORB,
  proc { |item,battler,battle|

class Battle
  ...
  def pbEOREndWeather(priority)
    ...
    if @field.weatherDuration == 0
      ...
      when :LightOasis then pbDisplay(_INTL("The light faded."))
      end
      @field.weather = :None
    ...
    when :LightOasis  then pbDisplay(_INTL("The light continues to shine."))
    end
  ...
  def pbEORStatusProblemDamage(priority)
    ...
    priority.each do |battler|
      ...
      battler.droppedBelowHalfHP = false
      dmg = (Settings.pbGetGeneration >= 7) ? battler.totalhp / 16 : battler.totalhp / 8
  ...
  def pbEORTrappingDamage(battler)
    ...
    hpLoss = (Settings.pbGetGeneration >= 6) ? battler.totalhp / 8 : battler.totalhp / 16
    if @battlers[battler.effects[PBEffects::TrappingUser]].hasActiveItem?(:BINDINGBAND)
      hpLoss = (Settings.pbGetGeneration >= 6) ? battler.totalhp / 6 : battler.totalhp / 8
    end
    @scene.pbDamageAnimation(battler)
  ...
  def pbEOREndBattlerSelfEffects(battler)
    ...
    if battler.inHyperMode?
      if pbRandom(100) < ($player.murphyslaw ? battler.idxOwnSide == 0 ? 0 : 100 : 10)
  ...
  def pbEndOfRoundPhase
    ...
    if Settings::AFFECTION_EFFECTS && @internalBattle
      priority.each do |battler|
        ...
        next if pbRandom(100) < ($player.murphyslaw ? 0 : 80)
    ...
    allBattlers.each do |battler|
      ...
      battler.effects[PBEffects::Firewall]    = false

class Battle
  ...
  def pbCanSwitchOut?(idxBattler, partyScene = nil)
    ...
    return true if battler.fainted? || battler.hasActiveAbility?(:RUNAWAY)
    ...
    return true if Settings.pbGetGeneration >= 6 && battler.pbHasType?(:GHOST)
  ...
  def pbReplace(idxBattler, idxParty, batonPass = false)
    ...
    pbCalculatePriority(false, [idxBattler]) if Settings.pbGetGeneration >= 8
  end
  ...
  def pbEffectsOnBattlerEnteringPosition(battler)
    ...
    if position.effects[PBEffects::HealingWish]
      ...
      elsif Settings.pbGetGeneration < 8
        position.effects[PBEffects::HealingWish] = false
      end
    end
    # Lunar Dance
  ...
  def pbEntryHazards(battler)
    ...
    if battler_side.effects[PBEffects::StealthRock] && battler.takesIndirectDamage? &&
      ...
      if !Effectiveness.ineffective?(eff) || @field.weather == :LightOasis
    ...
    # Shadow Traps
    if battler_side.effects[PBEffects::ShadowTraps] > 0 && !battler.fainted? && !battler.airborne?
      !battler.shadowPokemon? && !battler.hasActiveItem?(:HEAVYDUTYBOOTS)
      pbDisplay(_INTL("{1} was caught in Shadow Traps!", battler.pbThis))
      stacks = battler_side.effects[PBEffects::ShadowTraps]
      if battler.pbCanLowerStatStage?(:DEFENSE)
        battler.pbLowerStatStage(:DEFENSE, stacks, nil)
        battler.pbItemStatRestoreCheck
      end
      if battler.pbCanLowerStatStage?(:SPECIAL_DEFENSE)
        battler.pbLowerStatStage(:SPECIAL_DEFENSE, stacks, nil)
        battler.pbItemStatRestoreCheck
      end
   end
    # Toxic Spikes

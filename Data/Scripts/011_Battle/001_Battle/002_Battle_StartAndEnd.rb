class Battle
  ...
  def pbStartBattleCore
    ...
    when :LightOasis  then pbDisplay(_INTL("The oasis is shining."))
    end
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      tMoney *= $player.moneymultiplier
      oldMoney = pbPlayer.money.decrypt
      pbPlayer.money = oldMoney + tMoney.round
      moneyGained = pbPlayer.money.decrypt - oldMoney
      if moneyGained > 0
    ...
    if @field.effects[PBEffects::PayDay] > 0
      ...
      oldMoney = pbPlayer.money.decrypt
      pbPlayer.money = ((oldMoney + @field.effects[PBEffects::PayDay]) * $player.moneymultiplier).round
      moneyGained = pbPlayer.money.decrypt - oldMoney
      if moneyGained > 0
        $stats.battle_money_gained += moneyGained
  ...
  def pbLoseMoney
    ...
    tMoney = pbPlayer.money.decrypt if tMoney > pbPlayer.money.decrypt
    oldMoney = pbPlayer.money.decrypt
    pbPlayer.money = pbPlayer.money.decrypt - tMoney
    moneyLost = oldMoney - pbPlayer.money.decrypt
    if moneyLost > 0
  ...
  def pbEndOfBattle
    ...
    when 1
      ...
      if trainerBattle?
        ...
        if $player.rocketmode
          pbDisplayPaused(_INTL("{1} discarded their Pokémon and stole {2}'s Pokémon!", $player.name, @opponent[0].full_name))
          items = []
          $player.party.each do |pkmn|
            next unless pkmn.item
            items.push(pkmn.item)
          end
          $player.party = []
          @opponent[0].party.each do |pkmn|
    ...
    when 4
      ...
      PBDebug.log("")
      @scene.pbWildBattleSuccess if Settings.pbGetGeneration < 6
    end
    $player.add_encounter_location($game_map.map_id) unless pbParty(1).any? { |p| next unless p; p.shiny? || $player.seen?(p.species) }
    ...
    if @internalBattle
      ...
      infected.each do |idxParty|
        ...
        if idxParty > 0 && $player.party[idxParty - 1].pokerusStage == 0 && rand(3) == 0   # 33%
          $player.party[idxParty - 1].givePokerus(strain) && !$player.murphyslaw
        end

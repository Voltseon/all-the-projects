class Battle
  ...
  def pbCanRun?(idxBattler)
    ...
    return true if battler.pbHasType?(:GHOST) && Settings.pbGetGeneration >= 6
  ...
  def pbRun(idxBattler, duringBattle = false)
    ...
    if !duringBattle
      if battler.pbHasType?(:GHOST) && Settings.pbGetGeneration >= 6
        pbSEPlay("Battle flee")
        pbDisplayPaused(_INTL("You got away safely!"))
        @decision = 3
        return 1
    ...
    rate = 0 if rate < 256 && $player.murphyslaw

class Battle
  ...
  def pbGainEVsOne(idxParty, defeatedBattler)
    return if $game_temp.special_training_level || $player.noevgain
  ...
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    return if $game_temp.special_training && !$game_temp.special_training_level
    return if $player.levelone
    ...
    if expShare.length > 0 && (isPartic || hasExpShare)
      if numPartic == 0   # No participants, all Exp goes to Exp Share holders
        exp = a / (Settings.pbGetGeneration <= 5 ? expShare.length : 1)
      elsif Settings.pbGetGeneration <= 5   # Gain from participating and/or Exp Share
      ...
      exp = a / (Settings.pbGetGeneration <= 5 ? numPartic : 1)
    ...
    exp = (exp * 1.5).floor if Settings.pbGetGeneration <= 6 && trainerBattle?
    exp *= $player.expmultiplier
    # Scale the gained Exp based on the gainer's level (or not)
    if Settings.pbGetGeneration == 5 || Settings.pbGetGeneration >= 7
      exp /= 5
    ...
    # EXP Cap Special Training
    badges = $player.badge_count || 0
    badges = 8 if badges > 8
    exp = (exp * 0.1).floor if $game_temp.special_training_level && pkmn.level >= SPECIAL_TRAINING_LEVEL_CAP[badges]
    ...
    loop do   # For each level gained in turn...
      ...
      moveList.each { |m| pbLearnMove(idxParty, m[1]) if m[0] == curLevel && $PokemonSystem.asknewmoves!=1 }
      if false # mid battle evolution
        canevolve = false
        @battlers.each do |b|
          next unless idxParty == b.pokemonIndex
          canevolve = true
          break
        end
        if canevolve
          ...
          old_item=pkmn.item
          opponents_still_alive = false
          @battlers.each do |opp|
            next unless opp.opposes?
            next if opp.fainted? || opp.captured
            opponents_still_alive = true
          end
          if newspecies && opponents_still_alive
            pbCommonAnimation("Evolve1", battler)
            pbDisplayPaused(_INTL("Oh! {1} is evolving...", pkmn.name))
            shouldEvolve = true
            5.times do |i|
              pbCommonAnimation("Evolve2", battler)
              if Input.trigger?(Input::BACK)
                pbDisplayPaused(_INTL("Oh! {1} stopped evolving...", pkmn.name))
                shouldEvolve = false
                break
              end
            end
            if shouldEvolve
              ...
              if $PokemonSystem.asknewmoves!=1
                moves_to_learn.each do |move|
                  pbLearnMove(pkmn, move, true) { pbUpdate }
                end
              end
              battler.pbUpdate(true)
              pbCommonAnimation("Evolve3", battler)
              if battler

...
class Battle
  ...
  def pbSetSeen(battler)
    ...
    s = (battler.is_a?(Battler)) ? battler.displaySpecies : battler.species
    if battler.shadowPokemon?
      $player.shadow_pkmn[s] = [false,false] if $player.shadow_pkmn[s].nil?
      $player.shadow_pkmn[s][0] = true
    end
    if battler.is_a?(Battler)
      pbPlayer.pokedex.register(s, battler.displayGender,
  ...
  def pbSetCaught(battler)
    ...
    if battler.shadowPokemon?
      $player.shadow_pkmn[s] = [false,false] if $player.shadow_pkmn[s].nil?
      $player.shadow_pkmn[s][1] = true
    end
    pbPlayer.pokedex.register_caught(s)
  end
  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    when :LightOasis  then pbDisplay(_INTL("A light oasis appeared!"))
    end
  ...
  def pbStartWeatherAbility(new_weather, battler, ignore_primal = false)
    ...
    fixed_duration = true if Settings.pbGetGeneration >= 6 &&

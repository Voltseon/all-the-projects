  ...
  def initialize(viewport,z)
    ...
    if USE_GRAPHICS
      ...
      @buttons = Array.new(Pokemon::MAX_MOVES) do |i|
        ...
        button.bitmap = @buttonBitmap.bitmap
        button.x      = self.x+4
        button.x      += (((i%2)==0) ? -4 : @buttonBitmap.width/2-4)
        button.y      = self.y+8
        button.y      += (((i/2)==0) ? -4 : BUTTON_HEIGHT-4)
        button.src_rect.width  = @buttonBitmap.width/2
        button.src_rect.height = BUTTON_HEIGHT
        addSprite("button_#{i}",button)
        next button
  ...
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button,i|
      ...
      if moves[i].type
        ...
        moveNameBase = button.bitmap.get_pixel(1,button.src_rect.y+8)
        moveNameShadow = button.bitmap.get_pixel(3,button.src_rect.y+39)
      end
      textPos.push([moves[i].name,x,y,2,moveNameBase,moveNameShadow])
    end
...
class TargetMenuDisplay < BattleMenuBase
  ...
  TEXT_SHADOW_COLOR = Color.new(56,23,4)

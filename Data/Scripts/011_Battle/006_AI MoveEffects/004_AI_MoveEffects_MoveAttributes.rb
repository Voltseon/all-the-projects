...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("DoublePowerIfTargetAsleepCureTarget",
  proc { |score, move, user, target, ai, battle|
    if [:SLEEP, :DROWSY].include?(target.status) && target.statusCount > 1   # Will cure status
      if target.wants_status_problem?(:SLEEP) || target.wants_status_problem?(:DROWSY)
        score += 15
      else
        score -= 10
      end
...
Battle::AI::Handlers::MoveFailureCheck.add("StartWeakenElectricMoves",
  proc { |move, user, ai, battle|
    next battle.field.effects[PBEffects::MudSportField] > 0 if Settings.pbGetGeneration >= 6
...
Battle::AI::Handlers::MoveFailureCheck.add("StartWeakenFireMoves",
  proc { |move, user, ai, battle|
    next battle.field.effects[PBEffects::WaterSportField] > 0 if Settings.pbGetGeneration >= 6
...
Battle::AI::Handlers::MoveEffectScore.add("ProtectUser",
  proc { |score, move, user, ai, battle|
    ...
    # Don't prefer if the user used a protection move last turn, making this one
    # less likely to work
    score -= (user.effects[PBEffects::ProtectRate] - 1) * ((Settings.pbGetGeneration >= 6) ? 15 : 10)
    next score
  }
)
    ...
    ai.each_foe_battler(user.side) do |b, i|
      ...
      if b.battler.affectedByContactEffect? && b.check_for_move { |m| m.contactMove? }
        drop_score = ai.get_score_for_target_stat_drop(
          0, b, [:ATTACK, (Settings.pbGetGeneration >= 8) ? 1 : 2], false)
...
Battle::AI::Handlers::MoveEffectScore.add("ProtectUserSideFromMultiTargetDamagingMoves",
  proc { |score, move, user, ai, battle|
    ...
    ai.each_battler do |b, i|
      ...
      next if !b.check_for_move { |m| (Settings.pbGetGeneration >= 7 || move.damagingMove?) &&
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("EnsureNextMoveAlwaysHits",
  proc { |score, move, user, target, ai, battle|
    ...
    user.battler.eachMove do |m|
      next if target.effects[PBEffects::Minimize] && m.tramplesMinimize? && Settings.pbGetGeneration >= 6
      acc = m.accuracy

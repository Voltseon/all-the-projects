...
Battle::AI::Handlers::MoveEffectScore.add("HealUserFullyAndFallAsleep",
  proc { |score, move, user, ai, battle|
    ...
    if ai.trainer.has_skill_flag?("HPAware")
      if user.hp >= user.totalhp * 0.5 || user.has_active_ability?(:PURIFYINGSALT)
        score -= 10
      else
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("StartPerishCountsForAllBattlers",
  proc { |move, user, target, ai, battle|
    ...
    if $player.wonderguard && target.ability != :WONDERGUARD
      next Battle::AbilityEffects.triggerMoveImmunity(:WONDERGUARD, user.battler, target.battler,
...
Battle::AI::Handlers::MoveFailureCheck.add("AttackerFaintsIfUserFaints",
  proc { |move, user, ai, battle|
    next Settings.pbGetGeneration >= 7 && user.effects[PBEffects::DestinyBondPrevious]
  }
)

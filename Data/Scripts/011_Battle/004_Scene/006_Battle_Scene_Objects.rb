...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  NAME_SHADOW_COLOR_S     = Color.new(207, 192, 181)
  ...
  def initializeDataBoxGraphic(sideSize)
    ...
    if @sideSize == 1   # One Pokémon on side, use the regular dara box BG
      suffix = @battler.shadowPokemon? ? "_shadow" : "_normal"
      bgFilename = ["Graphics/Pictures/Battle/databox#{suffix}",
                    "Graphics/Pictures/Battle/databox#{suffix}_foe"][@battler.index % 2]
      if onPlayerSide
        @showHP  = true
        @showExp = true
      end
    else   # Multiple Pokémon on side, use the thin dara box BG
      suffix = @battler.shadowPokemon? ? "_shadow" : ""
      bgFilename = ["Graphics/Pictures/Battle/databox#{suffix}_thin",
                    "Graphics/Pictures/Battle/databox#{suffix}_thin_foe"][@battler.index % 2]
    end
  ...
  def initializeOtherGraphics(viewport)
    ...
    @numbersBitmap = AnimatedBitmap.new("Graphics/Pictures/Battle/icon_numbers#{(@battler.shadowPokemon? ? "_shadow" : "")}")
    @hpBarBitmap   = AnimatedBitmap.new("Graphics/Pictures/Battle/overlay_hp")
    @expBarBitmap  = @battler.shadowPokemon? ? AnimatedBitmap.new("Graphics/Pictures/Battle/overlay_shadow") : AnimatedBitmap.new("Graphics/Pictures/Battle/overlay_exp")
...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  def x=(value)
    ...
    @expBar.x    += 28 unless @battler.shadowPokemon?
  ...
  def exp_fraction
    if @battler.shadowPokemon?
      return @battler.pokemon.heart_gauge.to_f/@battler.pokemon.max_gauge_size
    else
  ...
  def draw_name
    ...
    shadow = @battler.shadowPokemon? ? NAME_SHADOW_COLOR_S : NAME_SHADOW_COLOR
    pbDrawTextPositions(self.bitmap,
      [[@battler.name, @spriteBaseX + 8 - nameOffset, 12, false, NAME_BASE_COLOR, shadow]]
    )
  end
  ...
  def draw_level
    ...
    pbDrawImagePositions(self.bitmap,
      [["Graphics/Pictures/Battle/overlay_lv#{suffix}", @spriteBaseX + 140, 16]]
    )
    # Level number
  ...
  def draw_gender
    ...
    shadow_color = NAME_SHADOW_COLOR_S if @battler.shadowPokemon?
...
#===============================================================================
# Terastalized sprite for Pokémon (used in battle)
#===============================================================================
class Battle::Scene::TeraSprite < RPG::Sprite
  def initialize(viewport)
    super(viewport)
    self.blend_type = 1
  end
  
  def linkParent(parent)
    self.ox = parent.ox
    self.oy = parent.oy
    self.visible = false if !parent.visible
    self.x = parent.x + parent.spriteXExtra
    self.y = parent.y + parent.spriteYExtra
    self.z = parent.z
    self.bitmap = parent.bitmap
    self.zoom_x = parent.zoom_x * 1.1
    self.zoom_y = parent.zoom_y * 1.1
    self.opacity = parent.opacity * 0.2
    self.color = parent.color
    self.angle = parent.angle
    self.mirror = parent.mirror
    self.bush_depth = parent.bush_depth
    self.tone = getTeraColor(parent.pkmn&.tera_type)
    self.src_rect = parent.src_rect
  end

  def getTeraColor(type)
    case type
    when :NORMAL then return Tone.new(255,255,255,0)
    when :FIRE then return Tone.new(50,0,0,0)
    when :WATER then return Tone.new(0,0,50,0)
    when :ELECTRIC then return Tone.new(20,20,0,0)
    when :GRASS then return Tone.new(0,50,0,0)
    when :ICE then return Tone.new(0,20,20,0)
    when :FIGHTING then return Tone.new(20,0,0,0)
    when :POISON then return Tone.new(20,0,20,0)
    when :GROUND then return Tone.new(10,10,0,0)
    when :FLYING then return Tone.new(0,50,20,0)
    when :PSYCHIC then return Tone.new(90,0,70,0)
    when :BUG then return Tone.new(10,30,0,0)
    when :ROCK then return Tone.new(10,10,0,0)
    when :GHOST then return Tone.new(10,0,10,0)
    when :DRAGON then return Tone.new(10,0,30,0)
    when :DARK then return Tone.new(30,30,30,0)
    when :STEEL then return Tone.new(80,80,80,0)
    when :FAIRY then return Tone.new(200,0,200,0)
    end
...
class Battle::Scene::BattlerSprite < RPG::Sprite
  ...
  def initialize(viewport, sideSize, index, battleAnimations, battle)
    ...
    @battle = battle
    @pkmn             = nil
    @teraSprite   = Battle::Scene::TeraSprite.new(viewport)
    @sideSize         = sideSize
    @index            = index
  ...
  def spriteXExtra; return @spriteXExtra; end
  def spriteYExtra; return @spriteYExtra; end
  ...
  def update(frameCounter = 0)
    ...
    @updating = true
    # Update bitmap
    @teraSprite.visible = @battle.battlers[@index].terastallized
    @_iconBitmap.update

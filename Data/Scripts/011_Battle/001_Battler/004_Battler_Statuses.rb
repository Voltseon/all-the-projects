class PokeBattle_Battler
  ...
  def pbCanInflictStatus?(newStatus,user,showMessages,move=nil,ignoreStatus=false)
    ...
    if self.status==newStatus && !ignoreStatus
      if showMessages
        ...
        when :PURIFY    then msg = _INTL("{1} is already under perification!", pbThis)
        when :PETRIFIED then msg = _INTL("{1} is already petrified!", pbThis)
        end
    ...
    if immuneByAbility
      if showMessages
        ...
        if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
          ...
          when :PETRIFIED then msg = _INTL("{1} cannot be petrified!", pbThis)
          when :PURIFY    then msg = _INTL("{1} is not affected by purify!", pbThis)
          end
        elsif immAlly
          case newStatus
          when :SLEEP
          ...
          when :FROZEN
            msg = _INTL("{1} cannot be frozen solid because of {2}'s {3}!",
               pbThis,immAlly.pbThis(true),immAlly.abilityName)
          when :PETRIFIED
            msg = _INTL("{1} cannot be petrified because of {2}'s {3}!",
                pbThis,immAlly.pbThis(true),immAlly.abilityName)
          when :PURIFY
            msg = _INTL("{1} cannot be affected by purify because of {2}'s {3}!",
                pbThis,immAlly.pbThis(true),immAlly.abilityName)
          end
        else
          case newStatus
          ...
          when :PETRIFIED    then msg = _INTL("{1}'s {2} prevents petrification!", pbThis, abilityName)
          when :PURIFY    then msg = _INTL("{1}'s {2} prevents purification!", pbThis, abilityName)
          end
        end
        @battle.pbDisplay(msg)
  ...
  def pbInflictStatus(newStatus,newStatusCount=0,msg=nil,user=nil)
    ...
    if msg && !msg.empty?
      ...
      when :PURIFY
        @battle.pbDisplay(_INTL("{1} is being purified!", pbThis))
      when :PETRIFIED
        @battle.pbDisplay(_INTL("{1} was petrified!", pbThis))
      end
    end
  ...
  #=============================================================================
  def petrified?
    return pbHasStatus?(:PETRIFIED)
  end

  def pbCanPetrify?(user, showMessages, move = nil)
    return pbCanInflictStatus?(:PETRIFIED, user, showMessages, move)
  end

  def pbPetrify(msg = nil)
    pbInflictStatus(:PETRIFIED, 0, msg)
  end
  ...
  #=============================================================================
  def purify?
    return pbHasStatus?(:PURIFY)
  end

  def pbCanPurify?(user, showMessages, move = nil)
    return pbCanInflictStatus?(:PURIFY, user, showMessages, move)
  end

  def pbPurify(msg = nil)
    pbInflictStatus(:PURIFY, 0, msg)
  end
  ...
  def pbContinueStatus
    ...
    when :PURIFY
      @battle.pbDisplay(_INTL("{1} is being purified!", pbThis))
    when :PETRIFIED
      @battle.pbDisplay(_INTL("{1} is petrified!", pbThis))
    end
  ...
  def pbCureStatus(showMessages=true)
    ...
    if showMessages
      ...
      when :PURIFY    then @battle.pbDisplay(_INTL("{1} has been purified!", pbThis))
      when :PETRIFIED then @battle.pbDisplay(_INTL("{1} broke out of petrification!", pbThis))
      end
    end

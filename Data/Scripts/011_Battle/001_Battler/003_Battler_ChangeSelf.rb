class PokeBattle_Battler
  ...
  def pbChangeForm(newForm,msg)
    ...
    @battle.scene.pbCommonAnimation("ChangeForm",self)
  ...
  def pbCheckFormOnStatusChange
    ...
    # Lemelt - change if burned
    if hasActiveAbility?(:IGNITION) && burned?
  ...
  def pbCheckFormOnWeatherChange
    ...
    # Clourricane - Rainy Forecast
    if isSpecies?(:CLOURRICANE)
      if hasActiveAbility?(:RAINYFORECAST)
        newForm = 0
        case @battle.pbWeather

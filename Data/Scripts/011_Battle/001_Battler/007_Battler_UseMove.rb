class PokeBattle_Battler
  ...
  def pbUseMove(choice,specialUsage=false)
    ...
    if targets.length == 0 && move.pbTarget(user).num_targets > 0 && !move.worksWithNoTargets?
      ...
      for i in 0...numHits
        ...
        break if [:SLEEP, :FROZEN, :PETRIFIED].include?(user.status)
  ...
  def pbProcessMoveHit(move,user,targets,hitNum,skipAccuracyCheck)
    ...
    # Count a hit for Bug Cry (if it applies)
    user.effects[PBEffects::BugCry] -= 1 if user.effects[PBEffects::BugCry]>0
    ...
    if move.pbDamagingMove?
      ...
      move.pbAnimateHitAndHPLost(user,targets,move.function=="070")
    end
    ...
    targets.each { |b| b.pbFaint if b && b.fainted? }
    user.pbFaint if user.fainted?
    if !user.fainted? && user.hasActiveAbility?(:COMBOSTAR)
      @battle.pbShowAbilitySplash(b,true)
      @battle.pbHideAbilitySplash(b)
      pbRaiseStatStage(:ATTACK,1,false)
      pbRaiseStatStage(:SPATK,1,false)
    end
    # Changes form if hit by a fire type move and has Scorching Fur
    designatedType = :NORMAL
    if move.type
      designatedType = GameData::Type.get(move.calcType).id
    end
    if designatedType == :FIRE
      targets.each do |b|
        next if b.fainted?
        next if b.damageState.calcDamage==0 || b.damageState.substitute
        if b.hasActiveAbility?(:SCORCHINGFUR) || b.hasActiveAbility?(:IGNITION)
          @battle.pbShowAbilitySplash(b,true)
          @battle.pbHideAbilitySplash(b)
          pbChangeForm(1,_INTL("{1} transformed!",b.name))
        end
      end
    end
    return true

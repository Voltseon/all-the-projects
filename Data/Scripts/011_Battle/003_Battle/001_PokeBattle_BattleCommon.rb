module PokeBattle_BattleCommon
    ...
    @caughtPokemon.each do |pkmn|
      ...
      if !pbPlayer.owned?(pkmn.species)
        ...
        if $Trainer.has_pokedex
          pbDisplayPaused(_INTL("{1}'s data was added to the The Book of Ethos.",pkmn.name))
  ...
  def pbThrowPokeBall(idxBattler,ball,catch_rate=nil,showPlayer=false)
    ...
    if $PokemonSystem.difficulty == 3 && $Trainer.encounter_mons.include?(battler.species) && $Trainer.encounter_maps.include?($game_map.name) && !battler.shiny? && !$game_switches[76]
      if itemName.starts_with_vowel?
        pbDisplay(_INTL("{1} threw an {2}!",pbPlayer.name,itemName))
      else
        pbDisplay(_INTL("{1} threw a {2}!",pbPlayer.name,itemName))
      end
      pbDisplay(_INTL("But has already encountered a Pokémon in this area."))
      $PokemonBag.pbStoreItem(GameData::Item.get(ball),1)
      return
    end
    if battler.fainted?
      if itemName.starts_with_vowel?
    ...
    # Deflecting ball if switch is enabled
    if $game_switches[99]
      @scene.pbThrowAndDeflect(ball,1)
      pbDisplay(_INTL("The Poké Ball deflected!"))
      return
    end
  ...
  def pbCaptureCalc(pkmn,battler,catch_rate,ball)
    ...
    if battler.status == :SLEEP || battler.status == :FROZEN || battler.status == :PETRIFIED
      x *= 2.5
    elsif battler.status != :NONE
      x *= 1.5
    end
    # Catch rate modifier for difficulty

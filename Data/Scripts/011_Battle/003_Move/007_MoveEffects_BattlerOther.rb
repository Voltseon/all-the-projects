...
class Battle::Move::BadPoisonTarget < Battle::Move::PoisonTarget
  ...
  def pbOverrideSuccessCheckPerHit(user, target)
    return (Settings.pbGetGeneration >= 6 && statusMove? && user.pbHasType?(:POISON))
  end
end
...
class Battle::Move::ParalyzeTargetIfNotTypeImmune < Battle::Move::ParalyzeTarget
  def pbFailsAgainstTarget?(user, target, show_message)
    if Effectiveness.ineffective?(target.damageState.typeMod) && @field.weather != :LightOasis
...
class Battle::Move::SetUserTypesToUserMoveType < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    user.eachMoveWithIndex do |m, i|
      break if Settings.pbGetGeneration >= 6 && i > 0
...
class Battle::Move::SetTargetAbilityToSimple < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.unstoppableAbility? || [:TRUANT, :SLOWSTART, :SIMPLE].include?(target.ability_id)
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return false
...
class Battle::Move::UserTargetSwapAbilities < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if !target.ability ||
       (user.ability == target.ability && Settings.pbGetGeneration <= 5)
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end

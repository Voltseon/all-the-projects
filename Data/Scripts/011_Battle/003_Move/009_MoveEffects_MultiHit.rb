...
class Battle::Move::HitTwoToFiveTimes < Battle::Move
  ...
  def pbNumHits(user, targets)
    ...
    r = ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : hitChances.length - 1 : r)
    return hitChances[r]
  end
end
...
class Battle::Move::TwoTurnAttackInvulnerableInSkyTargetCannotAct < Battle::Move::TwoTurnMove
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if Settings.pbGetGeneration >= 6 && target.pbWeight >= 2000   # 200.0kg
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
...
class Battle::Move::MultiTurnAttackConfuseUserAtEnd < Battle::Move
  def pbEffectAfterAllHits(user, target)
    if !target.damageState.unaffected && user.effects[PBEffects::Outrage] == 0
      user.effects[PBEffects::Outrage] = 2 + ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 1 : @battle.pbRandom(2))
      user.currentMove = @id
    end

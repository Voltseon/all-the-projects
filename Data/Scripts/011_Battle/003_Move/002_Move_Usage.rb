class Battle::Move
  ...
  def pbImmunityByAbility(user, target, show_message)
    return true if $player.godmode && target.pbOwnedByPlayer?
    ...
    if $player.wonderguard && target.ability != :WONDERGUARD
      ret = Battle::AbilityEffects.triggerMoveImmunity(:WONDERGUARD, user, target,
                                                       self, @calcType, @battle, show_message)
    end
    return ret
  end
    ...
    if damage >= target.hp
      ...
      if nonLethal?(user, target)
        ...
        if target.hasActiveAbility?(:STURDY) && !@battle.moldBreaker
          ...
          damage -= 1
        elsif target.hasActiveItem?(:FOCUSBAND) && @battle.pbRandom(100) < ($player.murphyslaw ? target.idxOwnSide == 0 ? 0 : 100 : 10)
          target.damageState.focusBand = true
          damage -= 1
        elsif Settings::AFFECTION_EFFECTS && @battle.internalBattle &&
              target.pbOwnedByPlayer? && !target.mega?
          chance = ($player.murphyslaw ? target.idxOwnSide == 0 ? 0 : 100 : [0, 0, 0, 10, 15, 25][target.affection_level])
  ...
  def pbEndureKOMessage(target)
    if target.damageState.disguise
      ...
      target.pbReduceHP(target.totalhp / 8, false) if Settings.pbGetGeneration >= 8

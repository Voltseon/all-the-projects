...
class Battle::Move::RandomlyDamageOrHealTarget < Battle::Move
  def pbOnStartUse(user, targets)
    ...
    r = ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 75 : @battle.pbRandom(100))
    if r < 40
      @presentDmg = 40
    elsif r < 70
      @presentDmg = 80
...
class Battle::Move::CurseTargetOrLowerUserSpd1RaiseUserAtkDef1 < Battle::Move
  ...
  def pbTarget(user)
    if user.pbHasType?(:GHOST)
      ghost_target = (Settings.pbGetGeneration >= 8) ? :RandomNearFoe : :NearFoe
...
class Battle::Move::EffectDependsOnEnvironment < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    return if $player.nomoveeffects
    chance = pbAdditionalEffectChance(user, target)
    if $player.murphyslaw
      chance = 0 if chance < 100 && user.idxOwnSide == 0
      chance = 100 if chance > 0 && user.idxOwnSide == 1
    end
...
class Battle::Move::UseMoveDependingOnEnvironment < Battle::Move
  ...
  def pbOnStartUse(user, targets)
    ...
    when :Psychic
      ...
      case @battle.environment
      when :Grass, :TallGrass, :Forest, :ForestGrass
        try_move = (Settings.pbGetGeneration >= 6) ? :ENERGYBALL : :SEEDBOMB
      when :MovingWater, :StillWater, :Underwater
        try_move = :HYDROPUMP
      when :Puddle
        try_move = :MUDBOMB
      when :Cave
        try_move = (Settings.pbGetGeneration >= 6) ? :POWERGEM : :ROCKSLIDE
      when :Rock, :Sand
        try_move = (Settings.pbGetGeneration >= 6) ? :EARTHPOWER : :EARTHQUAKE
      when :Snow
        try_move = :BLIZZARD
        try_move = :FROSTBREATH if Settings.pbGetGeneration == 6
        try_move = :ICEBEAM if Settings.pbGetGeneration >= 7
      when :Ice
        try_move = :ICEBEAM
      when :Volcano
        try_move = :LAVAPLUME
...
class Battle::Move::UseRandomMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    1000.times do
      ...
      move_id = :SPLASH if $player.murphyslaw && user.idxOwnSide == 0
      move_data = GameData::Move.get(move_id)
      next if @moveBlacklist.include?(move_data.function_code)
      next if move_data.has_flag?("CannotMetronome")
      next if move_data.type == :SHADOW || move_data.type == :LIGHT
      next if move_data.base_damage <= 55 if $player.murphyslaw && user.idxOwnSide != 0
...
#      "SwitchOutTargetStatusMove",                        # Roar, Whirlwind    # See below
  ...
  def pbMoveFailed?(user, targets)
    ...
    @battle.pbParty(user.index).each_with_index do |pkmn, i|
      ...
      next if Settings.pbGetGeneration >= 6 && pkmn.egg?
      pkmn.moves.each do |move|
        next if @moveBlacklist.include?(move.function_code)
        next if move.type == :SHADOW || move.type == :LIGHT
...
class Battle::Move::UseRandomUserMoveIfAsleep < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    if $player.murphyslaw
      10.times do
        if user.idxOwnSide == 0
          break if user.moves[choice].baseDamage < 1
        else
          break if user.moves[choice].baseDamage > 0
        end
        choice = @sleepTalkMoves[@battle.pbRandom(@sleepTalkMoves.length)]
      end
    end
...
class Battle::Move::ReplaceMoveThisBattleWithTargetLastMoveUsed < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if !lastMoveData ||
       ...
       lastMoveData.type == :SHADOW || lastMoveData.type == :LIGHT
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return false

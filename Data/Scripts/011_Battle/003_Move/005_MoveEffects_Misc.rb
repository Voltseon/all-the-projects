...
class Battle::Move::DoesNothingUnusableInGravity < Battle::Move
  ...
  def pbOnStartUse(user, targets)
    @splashDmg = ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 200 : (@battle.pbRandom(1000)<1) ? 200 : 0)
  end
  ...
  def pbEffectGeneral(user)
    if @splashDmg == 0
      ...
      user.pbUseMoveSimple(:SUPERSPLASH)
    end
  end
end
...
#===============================================================================
# Entry hazard. Lays Shadow Traps on the opposing side (max. 3 layers). (Shadow Trap)
#===============================================================================
class Battle::Move::AddShadowTrapsToFoeSide < Battle::Move
  def canMagicCoat?; return true; end

  def pbMoveFailed?(user, targets)
    if user.pbOpposingSide.effects[PBEffects::ShadowTraps] >= 3
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
    return false
  ...
  def pbEffectGeneral(user)
    user.pbOpposingSide.effects[PBEffects::ShadowTraps] += 1
    @battle.pbDisplay(_INTL("Shadow traps were scattered all around {1}'s feet!",
                            user.pbOpposingTeam(true)))
  end
end
...
class Battle::Move::RemoveUserBindingAndEntryHazards < Battle::Move::StatUpMove
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    if user.pbOwnSide.effects[PBEffects::ShadowTraps] > 0
      user.pbOwnSide.effects[PBEffects::ShadowTraps] = 0
  ...
  def pbAdditionalEffect(user, target)
    super if Settings.pbGetGeneration >= 8
  end
end

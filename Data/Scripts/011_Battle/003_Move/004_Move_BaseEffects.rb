...
class Battle::Move::ProtectMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if (!@sidedEffect || Settings.pbGetGeneration <= 5) &&
       user.effects[PBEffects::ProtectRate] > 1 &&
       ($player.murphyslaw ? user.idxOwnSide == 0 ? 1 : 0 : @battle.pbRandom(user.effects[PBEffects::ProtectRate])) != 0
      user.effects[PBEffects::ProtectRate] = 1
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
  ...
  def pbEffectGeneral(user)
    ...
    user.effects[PBEffects::ProtectRate] *= (Settings.pbGetGeneration >= 6) ? 3 : 2

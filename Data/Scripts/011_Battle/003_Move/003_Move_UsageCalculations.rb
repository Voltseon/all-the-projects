class Battle::Move
  ...
  def pbAccuracyCheck(user, target)
    ...
    return true if target.effects[PBEffects::Minimize] && tramplesMinimize? && Settings.pbGetGeneration >= 6
    ...
    if $player.murphyslaw
      threshold = 0 if threshold < 100 && user.idxOwnSide == 0
      threshold = 100 if threshold > 0 && user.idxOwnSide != 0
    end
    # Calculation
  ...
  def pbIsCritical?(user, target)
    ...
    # Other effects
    return user.idxOwnSide != 0 if $player.murphyslaw
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    if user.effects[PBEffects::ParentalBond] == 1
      multipliers[:power_multiplier] /= (Settings.pbGetGeneration >= 7) ? 4 : 2
    end
    # Other
    ...
    when :LightOasis
      multipliers[:final_damage_multiplier] *= 1.5 if type == :LIGHT
    end
    # Critical hits
    ...
    if !self.is_a?(Battle::Move::Confusion)
      random = ($player.murphyslaw ? user.idxOwnSide == 0 ? 85 : 100 : 85 + @battle.pbRandom(16))
  ...
  def pbAdditionalEffectChance(user, target, effectChance = 0)
    ...
    if (Settings.pbGetGeneration >= 6 || @function_code != "EffectDependsOnEnvironment") &&
       (user.hasActiveAbility?(:SERENEGRACE) || user.pbOwnSide.effects[PBEffects::Rainbow] > 0)
      ret *= 2
    end
    ret *= 2 if user.super_shiny?
    ret = 100 if $DEBUG && Input.press?(Input::CTRL)
    if $player.murphyslaw
      ret = 0 if ret < 100 && user.idxOwnSide == 0
      ret = 100 if ret > 0 && user.idxOwnSide != 0
    end
    return ret
  end

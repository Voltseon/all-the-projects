...
#===============================================================================
# Decreases the target's accuracy by 1 stage and has a chance to flinch
#===============================================================================
class Battle::Move::LowerTargetAccuracy1ChanceToFlinch < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    return false if damagingMove?
    can_flinch = !(target.hasActiveAbility?(:INNERFOCUS) && !@battle.moldBreaker)
    return !target.pbCanLowerStatStage?(@statDown[0], user, self, show_message) if !can_flinch
    return !can_flinch
  end
...
class Battle::Move::LowerTargetEvasion1RemoveSideEffects < Battle::Move::TargetStatDownMove
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    return false if targetSide.effects[PBEffects::StealthRock] ||
                    ...
                    targetSide.effects[PBEffects::ShadowTraps] > 0 ||
                    targetSide.effects[PBEffects::StickyWeb]
    return false if Settings.pbGetGeneration >= 6 &&
    ...
    return false if Settings.pbGetGeneration >= 8 && @battle.field.terrain != :None
    return super
  end
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.pbOwnSide.effects[PBEffects::StealthRock] ||
       (Settings.pbGetGeneration >= 6 &&
       target.pbOpposingSide.effects[PBEffects::StealthRock])
      target.pbOwnSide.effects[PBEffects::StealthRock]      = false
      target.pbOpposingSide.effects[PBEffects::StealthRock] = false if Settings.pbGetGeneration >= 6
    ...
    if target.pbOwnSide.effects[PBEffects::Spikes] > 0 ||
      ...
      target.pbOpposingSide.effects[PBEffects::Spikes] = 0 if Settings.pbGetGeneration >= 6
    ...
    if target.pbOwnSide.effects[PBEffects::ToxicSpikes] > 0 ||
      ...
      target.pbOpposingSide.effects[PBEffects::ToxicSpikes] = 0 if Settings.pbGetGeneration >= 6
      @battle.pbDisplay(_INTL("{1} blew away poison spikes!", user.pbThis))
    end
    if target.pbOwnSide.effects[PBEffects::ShadowTraps] > 0 ||
      (Settings.pbGetGeneration >= 6 &&
      target.pbOpposingSide.effects[PBEffects::ShadowTraps] > 0)
     target.pbOwnSide.effects[PBEffects::ShadowTraps]      = 0
     target.pbOpposingSide.effects[PBEffects::ShadowTraps] = 0 if Settings.pbGetGeneration >= 6
     @battle.pbDisplay(_INTL("{1} blew away shadow traps!", user.pbThis))
   end
    if target.pbOwnSide.effects[PBEffects::StickyWeb] ||
      ...
      target.pbOpposingSide.effects[PBEffects::StickyWeb] = false if Settings.pbGetGeneration >= 6
      @battle.pbDisplay(_INTL("{1} blew away sticky webs!", user.pbThis))
    end
    if Settings.pbGetGeneration >= 8 && @battle.field.terrain != :None

...
class Battle::Move::FixedDamageUserLevelRandom < Battle::Move::FixedDamageMove
  def pbFixedDamage(user, target)
    ...
    return ($player.murphyslaw ? user.idxOwnSide == 0 ? min : max : min + @battle.pbRandom(max - min + 1))
  end
end
...
class Battle::Move::OHKO < Battle::Move::FixedDamageMove
  ...
  def pbAccuracyCheck(user, target)
    ...
    if $player.murphyslaw
      acc = 0 if acc < 100 && user.idxOwnSide == 0
      acc = 100 if acc > 0 && user.idxOwnSide != 0
    end
...
class Battle::Move::RandomPowerDoublePowerIfTargetUnderground < Battle::Move
  ...
  def pbOnStartUse(user, targets)
    ...
    magni = ($player.murphyslaw ? idxOwnSide == 0 ? 4 : 10 : magnitudes[@battle.pbRandom(magnitudes.length)])
...
class Battle::Move::DoublePowerIfUserPoisonedBurnedParalyzed < Battle::Move
  def damageReducedByBurn?; return Settings.pbGetGeneration <= 5; end
...
# targets. (Surf)
#===============================================================================
class Battle::Move::DoublePowerIfTargetUnderwater < Battle::Move::FlinchTarget
...
# burned. (Firewall)
#===============================================================================
class Battle::Move::ProtectUserFirewall < Battle::Move::ProtectMove
  def initialize(battle, move)
    super
    @effect = PBEffects::Firewall
  end
end
...
class Battle::Move::RemoveProtections < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    target.effects[PBEffects::Firewall]               = false
...
# Ignores all abilities that alter this move's success or damage. This move is
# physical if user's Attack is higher than its Special Attack (after applying
# stat stages), and special otherwise. Also turns target into a shadow type.
# (Shadow Geyser)
#===============================================================================
class Battle::Move::ShadowGeyser < Battle::Move::CategoryDependsOnHigherDamageIgnoreTargetAbility
  def pbFailsAgainstTarget?(user, target, show_message)
    if !target.canChangeType? || !GameData::Type.exists?(:SHADOW) ||
       !target.pbHasOtherType?(:SHADOW) || target.shadowPokemon?
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return false
  ...
  def pbEffectAgainstTarget(user, target)
    target.pbChangeTypes(:SHADOW)
    typeName = GameData::Type.get(:SHADOW).name
    @battle.pbDisplay(_INTL("{1}'s type changed to {2}!", target.pbThis, typeName)) if !target.fainted?
  end
end
...
class Battle::Move::TypeDependsOnUserIVs < Battle::Move
  ...
  def pbBaseDamage(baseDmg, user, target)
    return super if Settings.pbGetGeneration >= 6
    hp = pbHiddenPower(user.pokemon)
    return hp[1]
  end
end
...
#===============================================================================
# Type depends on the user's type, chance to boost Special Attack. (Evo Burst)
#===============================================================================
class Battle::Move::TypeDependsOnUserIncreasesSpAtk < Battle::Move::RaiseUserSpAtk1
  def pbBaseType(user)
    return user.types[0]
  end
end
...
#===============================================================================
# Type depends on the user's type, chance to boost Attack. (Evo Thrust)
#===============================================================================
class Battle::Move::TypeDependsOnUserIncreasesAttack < Battle::Move::RaiseUserAttack1
  def pbBaseType(user)
    return user.types[0]
  end
end

...
#===============================================================================
# User turns 1/2 of max HP into a substitute, then switches out. (Shed Tail)
#===============================================================================
class Battle::Move::UserShedTail < Battle::Move::SwitchOutUserPassOnEffects
  def pbMoveFailed?(user, targets)
    super(user, targets)
    ...
    if user.hp <= @subLife
      @battle.pbDisplay(_INTL("But it does not have enough HP left!"))
      return true
    end
    return false
  end
...
class Battle::Move::BindTarget < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    if user.hasActiveItem?(:GRIPCLAW)
      target.effects[PBEffects::Trapping] = (Settings.pbGetGeneration >= 5) ? 8 : 6
    else
      target.effects[PBEffects::Trapping] = 5 + ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 1 : @battle.pbRandom(2))
    end
...
class Battle::Move::TrapTargetInBattle < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if Settings.pbGetGeneration >= 6 && target.pbHasType?(:GHOST)
  ...
  def pbAdditionalEffect(user, target)
    ...
    return if Settings.pbGetGeneration >= 6 && target.pbHasType?(:GHOST)
    target.effects[PBEffects::MeanLook] = user.index
    @battle.pbDisplay(_INTL("{1} can no longer escape!", target.pbThis))
  end
end
...
class Battle::Move::DisableTargetStatusMoves < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    return true if pbMoveFailedAromaVeil?(user, target, show_message)
    if Settings.pbGetGeneration >= 6 && target.hasActiveAbility?(:OBLIVIOUS) &&
       !@battle.moldBreaker
      if show_message
        @battle.pbShowAbilitySplash(target)
        if Battle::Scene::USE_ABILITY_SPLASH

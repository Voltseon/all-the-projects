...
BattleHandlers::MoveImmunityTargetAbility.add(:HAYFEVER,
  proc { |ability,user,target,move,type,battle|
    next false if !move.powderMove?
    battle.pbShowAbilitySplash(target)
    if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("It doesn't affect {1}...",target.pbThis(true)))
    else
    ...
    battle.pbHideAbilitySplash(target)
    next if !target.pbCanLowerStatStage?(:ACCURACY,target)
...
BattleHandlers::AccuracyCalcUserAbility.add(:COMBOSTAR,
  proc { |ability,mods,user,target,move,type|
...
BattleHandlers::DamageCalcUserAbility.add(:COMBOSTAR,
  proc { |ability,user,target,move,mults,baseDmg,type|
    mults[:base_damage_multiplier] *= 1.2 if move.addlEffect > 0
  }
)
...
BattleHandlers::DamageCalcUserAbility.add(:DECIUOUS,
  proc { |ability,user,target,move,mults,baseDmg,type|
...
BattleHandlers::TargetAbilityOnHit.add(:DEATHBRINGER,
  proc { |ability,user,target,move,battle|
    next if !move.pbContactMove?(user)
    next if rand(0..10)>1
    battle.pbShowAbilitySplash(target)
...
BattleHandlers::UserAbilityOnHit.add(:HEALTHDRAIN,
  proc { |ability,user,target,move,battle|
    next if !move.contactMove?
    ...
    battle.pbShowAbilitySplash(user)
    if target.hasActiveAbility?(:LIQUIDOOZE) && !battle.moldBreaker
      battle.pbShowAbilitySplash(target)
      if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1} sucked up the liquid ooze!",user.pbThis))
      end
      user.pbReduceHP(totalDamage/8, true)
      battle.pbHideAbilitySplash(target)
    end
    msg = nil
    if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      msg = _INTL("{1}'s {2} drained {3}'s health!",user.pbThis,user.abilityName,target.pbThis(true))
    end
    user.pbRecoverHP(totalDamage/8, true)
    battle.pbHideAbilitySplash(user)
  }
)
...
BattleHandlers::UserAbilityEndOfMove.add(:ENRAGED,
  proc { |ability,user,targets,move,battle|
    next if battle.pbAllFainted?(user.idxOpposingSide)
...
BattleHandlers::UserAbilityEndOfMove.add(:EXECUTIONER,
  proc { |ability,user,targets,move,battle|
    next if battle.pbAllFainted?(user.idxOpposingSide)
    numFainted = 0
    targets.each { |b| numFainted += 1 if b.damageState.fainted }
    next if numFainted==0
    user.pbChangeForm(1,_INTL("{1} transformed!",user.name))
  ...
  proc { |ability,battler,battle|
    ...
    battler.eachAlly do |b|
      ...
      if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
        ...
        when :PETRIFIED
          battle.pbDisplay(_INTL("{1}'s {2} depetrified its partner!",battler.pbThis,battler.abilityName))
        end
      end
      battle.pbHideAbilitySplash(battler)
    end
...
BattleHandlers::EORHealingAbility.add(:HYDRATION,
  proc { |ability,battler,battle|
    ...
    if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      ...
      when :PETRIFIED
        battle.pbDisplay(_INTL("{1}'s {2} depetrified it!",battler.pbThis,battler.abilityName))
      end
    end
    battle.pbHideAbilitySplash(battler)
  }
...
BattleHandlers::EORHealingAbility.add(:RESTORATION,
  proc { |ability,battler,battle|
    next unless battle.pbRandom(100)<30
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("{1}'s {2} cured some of its pp!",battler.pbThis,battler.abilityName)) if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
    battle.pbHideAbilitySplash(battler)
    for i in battler.moves
      move = battler.moves(i)
      move.pp += 1 if move.pp < move.total_pp
    end
  }
)
...
BattleHandlers::AbilityOnSwitchIn.add(:SPOTTER,
  proc { |ability,battler,battle|
    battler.pbRaiseStatStageByAbility(:ACCURACY,2,battler)
  }
)
...
BattleHandlers::AbilityOnSwitchIn.add(:RAINYFORECAST,
  proc { |ability,battler,battle|
    pbBattleWeatherAbility(:Rain, battler, battle)
  }
)

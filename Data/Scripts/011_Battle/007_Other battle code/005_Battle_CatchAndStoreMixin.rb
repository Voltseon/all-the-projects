module Battle::CatchAndStoreMixin
  ...
  def pbRecordAndStoreCaughtPokemon
    @caughtPokemon.each do |pkmn|
      $player.find_shiny if pkmn.shiny? || pkmn.super_shiny?
      pbSetCaught(pkmn)
  ...
  def pbThrowPokeBall(idxBattler, ball, catch_rate = nil, showPlayer = false)
    ...
    # at a Shadow Pokémon)
    if trainerBattle? && !(GameData::Item.get(ball).is_snag_ball? && battler.shadowPokemon?) && $player.firstencounter != :SHADOW
      @scene.pbThrowAndDeflect(ball, 1)
      pbDisplay(_INTL("The Trainer blocked your Poké Ball! Don't be a thief!"))
      return
    elsif $player.firstencounter && $player.encounter_locations.include?($game_map.map_id) && !([:SHINY, :SHADOW].include?($player.firstencounter) && (battler.shiny? || battler.super_shiny?))
      @scene.pbThrowAndDeflect(ball, 1)
      pbDisplay(_INTL("You have already caught a Pokémon in this area!"))
      return
    elsif $player.rocketmode
      @scene.pbThrowAndDeflect(ball, 1)
      pbDisplay(_INTL("You cannot use Poké Balls due to your Rocket status!"))
      return
    end
    ...
    when 4
      ...
      # Gain Exp
      if Settings.pbGetGeneration >= 6 && !$player.levelone
      ...
      if GameData::Item.get(ball).is_snag_ball?
        ...
        GameData::Stat.each_main do |s|
          @iv[s.id]       = rand(Pokemon::IV_STAT_LIMIT + 1)
          @ev[s.id]       = 0
        end
      end
  ...
  def pbCaptureCalc(pkmn, battler, catch_rate, ball)
    ...
    y = 0 if $player.murphyslaw && y < 65536

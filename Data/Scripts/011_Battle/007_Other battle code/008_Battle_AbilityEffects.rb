...
Battle::AbilityEffects::StatusCure.add(:OBLIVIOUS,
  proc { |ability, battler|
    next if battler.effects[PBEffects::Attract] < 0 &&
            (battler.effects[PBEffects::Taunt] == 0 || Settings.pbGetGeneration <= 5)
    battler.battle.pbShowAbilitySplash(battler)
    ...
    if battler.effects[PBEffects::Taunt] > 0 && Settings.pbGetGeneration >= 6
...
Battle::AbilityEffects::PriorityChange.add(:GALEWINGS,
  proc { |ability, battler, move, pri|
    next pri + 1 if (Settings.pbGetGeneration <= 6 || battler.hp == battler.totalhp) &&
                    move.type == :FLYING
  }
)
...
Battle::AbilityEffects::PriorityBracketChange.add(:QUICKDRAW,
  proc { |ability, battler, battle|
    next 1 if battle.pbRandom(100) < ($player.murphyslaw ? battler.idxOwnSide == 0 ? 0 : 100 : 30)
  }
)
...
Battle::AbilityEffects::MoveImmunity.add(:SOUNDPROOF,
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    next false if Settings.pbGetGeneration >= 8 && user.index == target.index
    if show_message
      battle.pbShowAbilitySplash(target)
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("It doesn't affect {1}...", target.pbThis(true)))
...
Battle::AbilityEffects::MoveImmunity.add(:WONDERGUARD,
  proc { |ability, user, target, move, type, battle, show_message|
    ...
    if show_message
      battle.pbShowAbilitySplash(target) if !$player.wonderguard
      if Battle::Scene::USE_ABILITY_SPLASH && !$player.wonderguard
        battle.pbDisplay(_INTL("It doesn't affect {1}...", target.pbThis(true)))
      else
        ability_name = $player.wonderguard ? "Wonder Guard" : target.abilityName
        battle.pbDisplay(_INTL("{1} avoided damage with {2}!", target.pbThis, ability_name))
      end
      battle.pbHideAbilitySplash(target) if !$player.wonderguard
    end
    next true
  }
)
...
Battle::AbilityEffects::ModifyMoveBaseType.add(:NORMALIZE,
  proc { |ability, user, move, type|
    ...
    move.powerBoost = true if Settings.pbGetGeneration >= 7
    next :NORMAL
  }
)
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:TRUANT,
  proc { |ability, mods, user, target, move, type|
    mods[:accuracy_multiplier] *= 1000
  }
)
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:KEENEYE,
  proc { |ability, mods, user, target, move, type|
    mods[:evasion_stage] = 0 if mods[:evasion_stage] > 0 && Settings.pbGetGeneration >= 6
  }
)
...
Battle::AbilityEffects::AccuracyCalcFromTarget.add(:CYBERSPACE,
  proc { |ability, mods, user, target, move, type|
    mods[:evasion_multiplier] *= 1.25 if target.pbSpeed < user.pbSpeed
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:KENNYAURA,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:attack_multiplier] *= 1.75 if type == :DARK
  }
)

Battle::AbilityEffects::DamageCalcFromUser.add(:CHRISTMASSPIRIT,
  proc { |ability, user, target, move, mults, baseDmg, type|
...
Battle::AbilityEffects::OnBeingHit.add(:CURSEDBODY,
  proc { |ability, user, target, move, battle|
    ...
    next if battle.pbRandom(100) >= ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 100 : 30)
    battle.pbShowAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:FLAMEBODY,
  proc { |ability, user, target, move, battle|
    next if !move.pbContactMove?(user)
    next if user.burned? || battle.pbRandom(100) >= ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 100 : 30)
    battle.pbShowAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:POISONPOINT,
  proc { |ability, user, target, move, battle|
    next if !move.pbContactMove?(user)
    next if user.poisoned? || battle.pbRandom(100) >= ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 100 : 30)
    battle.pbShowAbilitySplash(target)
    if user.pbCanPoison?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
       user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      msg = nil
...
Battle::AbilityEffects::OnBeingHit.add(:STATIC,
  proc { |ability, user, target, move, battle|
    next if !move.pbContactMove?(user)
    next if user.paralyzed? || battle.pbRandom(100) >= ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 100 : 30)
    battle.pbShowAbilitySplash(target)
    if user.pbCanParalyze?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
       user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
      msg = nil
...
Battle::AbilityEffects::OnBeingHit.add(:WEAKARMOR,
  proc { |ability, user, target, move, battle|
    ...
    target.pbRaiseStatStageByAbility(:SPEED,
       (Settings.pbGetGeneration >= 7) ? 2 : 1, target, false)
    battle.pbHideAbilitySplash(target)
  }
)
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:PICKUP,
  proc { |ability, user, targets, move, battle|
    next if battle.pbAllFainted?(user.idxOpposingSide)
    numFainted = 0
    targets.each { |b| numFainted += 1 if b.damageState.fainted }
    ...
    if user.wildBattle? && !user.initialItem && fromBattler.initialItem == foundItem
      user.setInitialItem(foundItem)
      fromBattler.setInitialItem(nil)
    end
    battle.pbDisplay(_INTL("{1} found one {2}!", user.pbThis, user.itemName))
    battle.pbHideAbilitySplash(user)
    user.pbHeldItemTriggerCheck
  }
)
...
Battle::AbilityEffects::EndOfRoundHealing.add(:HEALER,
  proc { |ability, battler, battle|
    next if battle.pbRandom(100) >= ($player.murphyslaw ? battler.idxOwnSide == 0 ? 0 : 100 : 30)
...
Battle::AbilityEffects::EndOfRoundHealing.add(:SHEDSKIN,
  proc { |ability, battler, battle|
    next if battler.status == :NONE
    next unless battle.pbRandom(100) < ($player.murphyslaw ? battler.idxOwnSide == 0 ? 0 : 100 : 30)
    battle.pbShowAbilitySplash(battler)
    oldStatus = battler.status
    battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    if !Battle::Scene::USE_ABILITY_SPLASH
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:HARVEST,
  proc { |ability, battler, battle|
    ...
    if ![:Sun, :HarshSun].include?(battler.effectiveWeather)
      next unless battle.pbRandom(100) < ($player.murphyslaw ? battler.idxOwnSide == 0 ? 0 : 100 : 50)
    end
    battle.pbShowAbilitySplash(battler)
  ...
  proc { |ability, battler, battle, switch_in|
    ...
    battle.allOtherSideBattlers(battler.index).each do |b|
      b.eachMove do |m|
        ...
        if types.length > 0
          moveType = m.type
          if Settings.pbGetGeneration >= 6 && m.function_code == "TypeDependsOnUserIVs"   # Hidden Power
            moveType = pbHiddenPower(b.pokemon)[0]
          end
          eff = Effectiveness.calculate(moveType, *types)
          next if Effectiveness.ineffective?(eff) && @field.weather != :LightOasis
...
Battle::AbilityEffects::OnSwitchIn.add(:VENTILATION,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    battle.pbAnimation(:TAILWIND, battler, battler.pbDirectOpposing)
  ...
  proc { |ability, battler, battle, switch_in|
    ...
    battle.allOtherSideBattlers(battler.index).each do |b|
      b.eachMove do |m|
        ...
        power = 80 if Settings.pbGetGeneration <= 5 && m.function_code == "TypeDependsOnUserIVs"
...
Battle::AbilityEffects::OnSwitchIn.add(:HONEYGATHER,
  proc { |ability, battler, battle, switch_in|
    ...
    battle.allOtherSideBattlers(battler.index).each do |b|
      next if !b.near?(battler)
      b.pbLowerStatStage(:EVASION, 1, battler)
    end
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:ILLUMINATE,
  proc { |ability, battler, battle, switch_in|
    ...
    battle.allOtherSideBattlers(battler.index).each do |b|
      next if !b.near?(battler)
      b.pbLowerStatStage(:ACCURACY, 1, battler)
    end
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnIntimidated.add(:RATTLED,
  proc { |ability, battler, battle|
    next if Settings.pbGetGeneration < 8
    battler.pbRaiseStatStageByAbility(:SPEED, 1, battler)
  }
)

...
Battle::PokeBallEffects::ModifyCatchRate.add(:NETBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings.pbGetGeneration >= 7) ? 3.5 : 3
...
Battle::PokeBallEffects::ModifyCatchRate.add(:DUSKBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings.pbGetGeneration >= 7) ? 3 : 3.5
...
Battle::PokeBallEffects::ModifyCatchRate.add(:TWILIGHTBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings.pbGetGeneration >= 7) ? 3 : 3.5
  catchRate *= multiplier if battle.time == 3 || battle.time == 1
  next catchRate
})
...
Battle::PokeBallEffects::ModifyCatchRate.add(:LUREBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings.pbGetGeneration >= 7) ? 5 : 3

class Battle::Battler
  ...
  def pbObedienceCheck?(choice)
    if @battle.pbOwnedByPlayer?(@index)
      return pbDisobey(choice, 100, true) if $player.shinyonly && !(@pokemon.shiny? || @pokemon.super_shiny?)
      return pbDisobey(choice, 100, true) if $player.levelone && @level > 1
      return pbDisobey(choice, 100, true) if !$player.typeonly.nil? && !pbHasType?($player.typeonly)
    end
    ...
    badge_level = 10 * (@battle.pbPlayer.badge_count + 2)
  ...
  def pbDisobey(choice, badge_level, challenge=false)
    move = choice[2]
    PBDebug.log("[Disobedience] #{pbThis} disobeyed")
    @effects[PBEffects::Rage] = false
    if challenge
      @battle.pbDisplay(_INTL("{1} cannot be used due to a challenge.", pbThis))
      return false
    end
  ...
  def pbTryUseMove(choice, move, specialUsage, skipAccuracyCheck)
    ...
    when :FROZEN
      if !move.thawsUser?
        if (@battle.pbRandom(100) < 20 && (!$player.murphyslaw || idxOwnSide == 0)) || ($player.murphyslaw && idxOwnSide != 0)
          pbCureStatus
        else
          pbContinueStatus
    ...
    if @effects[PBEffects::Confusion] > 0
      ...
      if @effects[PBEffects::Confusion] <= 0
        ...
        threshold = (Settings.pbGetGeneration >= 7) ? 33 : 50   # % chance
        threshold = (idxOwnSide == 0 ? 100 : 0) if $player.murphyslaw
    ...
    # Paralysis
    if @status == :PARALYSIS && @battle.pbRandom(100) < ($player.murphyslaw ? idxOwnSide == 0 ? 100 : 0 : 25)
      pbContinueStatus
    ...
    if @effects[PBEffects::Attract] >= 0
      ...
      if @battle.pbRandom(100) < ($player.murphyslaw ? idxOwnSide == 0 ? 100 : 0 : 50)
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    ...
    # Pacifist
    if user.pbOwnedByPlayer? && move.damagingMove? && $player.pacifist
      PBDebug.log("[Move failed] #{user.pbThis} is a pacifist")
      @battle.pbDisplay(_INTL("{1} is a pacifist and can't use damaging moves!", user.pbThis))
      return false
    end
    ...
    if !(user.hasActiveAbility?(:UNSEENFIST) && move.contactMove?)
      ...
      if target.pbOwnSide.effects[PBEffects::WideGuard] && user.index != target.index &&
         ...
         (Settings.pbGetGeneration >= 7 || move.damagingMove?)
        if show_message
      ...
      if move.canProtectAgainst?
        ...
        if target.effects[PBEffects::KingsShield] && move.damagingMove?
          ...
          if move.pbContactMove?(user) && user.affectedByContactEffect? &&
            ...
            user.pbLowerStatStage(:ATTACK, (Settings.pbGetGeneration >= 8) ? 1 : 2, target)
          end
          return false
        end
        # Spiky Shield
        ...
        # Firewall
        if target.effects[PBEffects::Firewall]
          if show_message
            @battle.pbCommonAnimation("Firewall", target)
            @battle.pbDisplay(_INTL("{1} protected itself!", target.pbThis))
          end
          target.damageState.protected = true
          @battle.successStates[user.index].protected = true
    ...
    if Settings.pbGetGeneration >= 7 && user.effects[PBEffects::Prankster] &&
    ...
    if move.powderMove?
      if target.pbHasType?(:GRASS) && Settings.pbGetGeneration >= 6

class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    when :PARALYSIS
      hasImmuneType |= pbHasType?(:ELECTRIC) && Settings.pbGetGeneration >= 6
    when :FROZEN
  ...
  def pbSleepDuration(duration = -1)
    duration = 2 + ($player.murphyslaw ? idxOwnSide == 0 ? 3 : 0 : @battle.pbRandom(3)) if duration <= 0
  ...
  def pbCanConfuse?(user = nil, showMessages = true, move = nil, selfInflicted = false)
    ...
    # Terrains immunity
    if affectedByTerrain? && @battle.field.terrain == :Misty && Settings.pbGetGeneration >= 7
      @battle.pbDisplay(_INTL("{1} surrounds itself with misty terrain!", pbThis(true))) if showMessages
      return false
    end
  ...
  def pbConfusionDuration(duration = -1)
    duration = 2 + ($player.murphyslaw ? idxOwnSide == 0 ? 4 : 0 : @battle.pbRandom(4)) if duration <= 0
    return duration
  end

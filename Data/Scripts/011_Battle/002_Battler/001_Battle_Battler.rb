class Battle::Battler
  ...
  def pbSpeed
    ...
    if status == :PARALYSIS && !hasActiveAbility?(:QUICKFEET)
      speedMult /= (Settings.pbGetGeneration >= 7) ? 2 : 4
    end
    # Badge multiplier
  ...
  def abilityActive?(ignore_fainted = false, check_ability = nil)
    ...
    return false if $player.noability
    if Settings.pbGetGeneration >= 9
...
#      :FORECAST,                                          # This can be stopped
  ...
  def takesIndirectDamage?(showMsg = false)
    ...
    return false if $player.godmode && pbOwnedByPlayer?
  ...
  def affectedByPowder?(showMsg = false)
    return false if fainted?
    if pbHasType?(:GRASS) && Settings.pbGetGeneration >= 6
  ...
  def canHeal?
    return false if ($player.onehp || $player.nohealing) && pbOwnedByPlayer?
  ...
  def trappedInBattle?
    return true if $player.shadowtag
  ...
  def semiInvulnerable?
    return true if $player.godmode && pbOwnedByPlayer?

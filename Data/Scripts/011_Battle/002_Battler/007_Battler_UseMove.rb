class Battle::Battler
  ...
  def pbProcessTurn(choice, tryFlee = true)
    ...
    @battle.pbCalculatePriority if Settings.pbGetGeneration >= 8
    return true
  end
  ...
  def pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
    ...
    user.pbFaint if user.fainted?
    # Additional effect
    if !user.hasActiveAbility?(:SHEERFORCE) && !$player.nomoveeffects
      targets.each do |b|
        next if b.damageState.calcDamage == 0 && move.function_code != "LowerTargetAccuracy1ChanceToFlinch"
        chance = ($player.murphyslaw ? b.idxOwnSide == 0 ? 100 : 0 : move.pbAdditionalEffectChance(user, b))
        next if chance <= 0
    ...
    targets.each do |b|
      ...
      if $player.murphyslaw
        chance = 0 if b.idxOwnSide == 0 && chance < 100
        chance = 100 if b.idxOwnSide != 0 && chance > 0
      end
      next if chance <= 0

...
#===============================================================================
# Petrifies the target.
#===============================================================================
class PokeBattle_Move_176 < PokeBattle_PetrifyMove
end

#===============================================================================
# Purifies the target.
#===============================================================================
class PokeBattle_Move_177 < PokeBattle_PurifyMove
end

#===============================================================================
# Deals double damage on Ghost Types, removes a curse and perish song.
#===============================================================================
class PokeBattle_Move_178 < PokeBattle_Move
  ...
  def pbEffectAgainstTarget(user,target)
    ...
    user.effects[PBEffects::PerishSong]     = 0
    user.effects[PBEffects::Curse]     = false
...
#===============================================================================
# If this move KO's the target, omniboost.
# (Death Stalk)
#===============================================================================
class PokeBattle_Move_179 < PokeBattle_Move
  def pbEffectAfterAllHits(user,target)
    ...
    user.pbRaiseStatStage(:SPATK,1,user) if user.pbCanRaiseStatStage?(:SPATK,user,self)
    user.pbRaiseStatStage(:SPDEF,1,user) if user.pbCanRaiseStatStage?(:SPDEF,user,self)
  end
end
...
# Freezes the target.
#===============================================================================
class PokeBattle_Move_17A < PokeBattle_FreezeMove
  def pbEffectAgainstTarget(user,target)
    return if target.fainted? || target.damageState.substitute
    ...
    msg = _INTL("{1} was trapped in the permafrost!",target.pbThis)
...
# Also lowers the target's Defense by 1 stage.
# (Shell Crush)
#===============================================================================
...
#===============================================================================
# Move increases in damage based on the user's accuracy.
# (Clogged Sneeze)
#===============================================================================
class PokeBattle_Move_17C < PokeBattle_Move
  def pbBaseDamage(baseDmg,user,target)
    GameData::Stat.each_battle { mult = 1/user.stages[:ACCURACY] }
    return 70 * mult
  end
end

#===============================================================================
# Power is doubled if the target is burned. (Overcook)
#===============================================================================
...
#===============================================================================
# Raises and lowers stats based on nature. (Nature Boost)
#===============================================================================
class PokeBattle_Move_17E < PokeBattle_Move
  def pbEffectAfterAllHits(user,target)
    nature = user.nature.id
    case nature
    when :LONELY, :BRAVE, :ADAMANT, :NAUGHTY
      user.pbRaiseStatStage(:ATTACK,3,user) if user.pbCanRaiseStatStage?(:ATTACK,user,self)
    when :BOLD, :RELAXED, :IMPISH, :LAX
      user.pbRaiseStatStage(:DEFENSE,3,user) if user.pbCanRaiseStatStage?(:DEFENSE,user,self)
    when :MODEST, :MILD, :RASH, :QUIET
      user.pbRaiseStatStage(:SPATK,3,user) if user.pbCanRaiseStatStage?(:SPATK,user,self)
    when :CALM, :GENTLE, :CAREFUL, :SASSY
      user.pbRaiseStatStage(:SPDEF,3,user) if user.pbCanRaiseStatStage?(:SPDEF,user,self)
    when :TIMID, :HASTY, :JOLLY, :NAIVE
      user.pbRaiseStatStage(:SPEED,3,user) if user.pbCanRaiseStatStage?(:SPEED,user,self)
    else
      user.pbRaiseStatStage(:ATTACK,1,user) if user.pbCanRaiseStatStage?(:ATTACK,user,self)
      user.pbRaiseStatStage(:DEFENSE,1,user) if user.pbCanRaiseStatStage?(:DEFENSE,user,self)
      user.pbRaiseStatStage(:SPEED,1,user) if user.pbCanRaiseStatStage?(:SPEED,user,self)
    ...
    case nature
    when :BOLD, :MODEST, :CALM, :TIMID
      user.pbLowerStatStage(:ATTACK,3,user) if user.pbCanLowerStatStage?(:ATTACK,user,self)
    when :LONELY, :MILD, :GENTLE, :HASTY
      user.pbLowerStatStage(:DEFENSE,3,user) if user.pbCanLowerStatStage?(:DEFENSE,user,self)
    when :ADAMANT, :IMPISH, :CAREFUL, :JOLLY
      user.pbLowerStatStage(:SPATK,3,user) if user.pbCanLowerStatStage?(:SPATK,user,self)
    when :NAUGHTY, :LAX, :RASH, :NAIVE
      user.pbLowerStatStage(:SPDEF,3,user) if user.pbCanLowerStatStage?(:SPDEF,user,self)
    when :BRAVE, :RELAXED, :QUIET, :SASSY
      user.pbLowerStatStage(:SPEED,3,user) if user.pbCanLowerStatStage?(:SPEED,user,self)
    end
  end
end

#===============================================================================
# User and target swap stat changes, field boosts and items.
# Items remain swapped after wild battles.
# (Soul Swap)
#===============================================================================
class PokeBattle_Move_17F < PokeBattle_Move
  ...
  def pbEffectAgainstTarget(user,target)
    singularEffects = [PBEffects::StealthRock, PBEffects::StickyWeb]
    multipleEffects = [PBEffects::AuroraVeil, PBEffects::LightScreen,
      PBEffects::Reflect, PBEffects::Mist, PBEffects::Safeguard,
      PBEffects::Spikes, PBEffects::ToxicSpikes]
    ...
    # Swap stat changes with the target
    GameData::Stat.each_battle do |s|
      user.stages[s.id],target.stages[s.id] = target.stages[s.id],user.stages[s.id]
    end
    # IF NOT WORKS CHECK OUT "target.pbOpposingSide.effects[PBEffects::StickyWeb] = false if Settings::MECHANICS_GENERATION >= 6"
    # Swap field hazards with the target
    for i in singularEffects.length
      targetEffect = target.pbOwnSide.effects[i]
      userEffect = user.pbOwnSide.effects[i]
      user.pbOwnSide.effects[i] = targetEffect if targetEffect
      target.pbOwnSide.effects[i] = userEffect if userEffect
    end
    for i in multipleEffects.length
      targetEffect = target.pbOwnSide.effects[i]
      userEffect = user.pbOwnSide.effects[i]
      user.pbOwnSide.effects[i] = targetEffect if targetEffect>0
      target.pbOwnSide.effects[i] = userEffect if userEffect>0
    end
    @battle.pbDisplay(_INTL("{1} switched souls with its opponent!",user.pbThis))
    ...
    if user.hasActiveAbility?(:SOULMATE)

...
class PokeBattle_PetrifyMove < PokeBattle_Move
  def pbFailsAgainstTarget?(user,target)
    return false if damagingMove?
    return !target.pbCanPetrify?(user,true,self)
  end
...
class PokeBattle_PurifyMove < PokeBattle_Move
  def pbFailsAgainstTarget?(user,target)
    return false if damagingMove?
    return !target.pbCanPurify?(user,true,self)
  end
  ...
  def pbAdditionalEffect(user,target)
    return if target.damageState.substitute
    target.pbPurify if target.pbCanPurify?(user,false,self)
  end
end
...
# Weather-inducing move.
#===============================================================================
class PokeBattle_WeatherMove2 < PokeBattle_Move
  def initialize(battle,move)
    super
    @weatherType = :None
  end

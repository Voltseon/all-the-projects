class PokeBattle_Move
  ...
  def pbNumHits(user,targets)
    ...
    if user.effects[PBEffects::BugCry] > 0 && pbDamagingMove? &&
      !chargingTurnMove? && targets.length==1
     return 2
   end
    return 1
  ...
  def pbShowAnimation(id,user,targets,hitNum=0,showAnimation=true)
    ...
    if user.effects[PBEffects::ParentalBond]==1 || user.effects[PBEffects::BugCry] > 0
  ...
  def pbAnimateHitAndHPLost(user,targets,is_ohko=false)
    ...
    for side in 0...2   # side here means "allies first, then foes"
      targets.each do |b|
        ...
        effectiveness = 0
        if is_ohko; effectiveness = 5
        elsif Effectiveness.quad_effective?(b.damageState.typeMod);          effectiveness = 3
        elsif Effectiveness.quad_not_very_effective?(b.damageState.typeMod);          effectiveness = 4
        elsif Effectiveness.resistant?(b.damageState.typeMod);          effectiveness = 1
    ...
    # Break out of petrification if hit
    for side in 0...2   # side here means "allies first, then foes"
      targets.each do |b|
        if b.status == :PETRIFIED && !b.fainted? && b.damageState.hpLost > 0
          b.pbCureStatus(false)
          @battle.pbDisplay(_INTL("{1} was broken out of its petrification due to the attack!",b.pbThis))
        end
      end
    end
  end

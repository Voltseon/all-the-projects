...
class PokeBattle_Move_019 < PokeBattle_Move
  ...
  def pbAromatherapyHeal(pkmn,battler=nil)
    ...
    when :PETRIFIED
      @battle.pbDisplay(_INTL("{1} broke out of petrification.",curedName))
    when :PURIFY
      @battle.pbDisplay(_INTL("{1} has been fully purified.",curedName))
    end
  end
...
class PokeBattle_Move_01B < PokeBattle_Move
  ...
  def pbEffectAgainstTarget(user,target)
    ...
    when :PETRIFIED
      target.pbPetrify
      msg = _INTL("{1} broke out of petrification.",user.pbThis)
    when :PURIFY
      target.pbPurify
      msg = _INTL("{1} has been fully purified.",user.pbThis)
    end
    if msg!=""
      user.pbCureStatus(false)

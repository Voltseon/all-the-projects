...
ItemHandlers::UseFromBag.add(:SLEEPINGGEAR, proc { |item|
  next 2
})

ItemHandlers::UseFromBag.add(:FRONTIERPASSM, proc { |item|
  next 2
})

ItemHandlers::UseFromBag.copy(:FRONTIERPASSM, :FRONTIERPASSF, :FRONTIERPASSO)
...
ItemHandlers::UseFromBag.copy(:OLDROD, :GOODROD, :SUPERROD, :BESTROD)
...
ItemHandlers::UseFromBag.add(:HMEMULATOR, proc { |item|
  next 2
})
...
ItemHandlers::UseFromBag.addIf(:move_machines,
  ...
  proc { |item|
    if $player.metronome
      pbMessage(_INTL("It can't be used with the Metronome challenge."))
      next 0
    end
...
ItemHandlers::UseInField.add(:SACREDASH, proc { |item|
  ...
  pbFadeOutIn {
    ...
    $player.party.each_with_index do |pkmn, i|
      next if !pkmn.fainted? || $player.permadeath
      revived += 1
      pkmn.heal
...
ItemHandlers::UseInField.add(:BESTROD, proc { |item|
  notCliff = $game_map.passable?($game_player.x, $game_player.y, $game_player.direction, $game_player)
  if !$game_player.pbFacingTerrainTag.can_fish || (!$PokemonGlobal.surfing && !notCliff)
    pbMessage(_INTL("Can't use that here."))
    next false
...
ItemHandlers::UseInField.add(:HMEMULATOR, proc { |item|
  facing_event = $game_player.pbFacingEvent
  map_metadata = $game_map.metadata
  dive_check = $PokemonGlobal.diving ? pbCanSurface? : $game_player.terrain_tag.can_dive
  commands = []
  commands.push("Flash") if map_metadata&.dark_map && !$PokemonGlobal.flashUsed && vHI(:HM06)
  commands.push("Surf") if !facing_event && $game_player.can_ride_vehicle_with_follower? &&
    !$PokemonGlobal.surfing && $game_player.pbFacingTerrainTag.can_surf_freely && !map_metadata&.always_bicycle &&
    $game_map.passable?($game_player.x, $game_player.y, $game_player.direction, $game_player) && vHI(:HM03)
  commands.push("Rock Smash") if facing_event && facing_event.name[/smashrock/i] && vHI(:HM05)
  commands.push("Fly") if pbCanFly?
  commands.push("Cut") if facing_event && facing_event.name[/cuttree/i] && vHI(:HM01)
  commands.push("Strength") if !$PokemonMap.strengthUsed && pbBoulderInMap?
  commands.push("Fire Lash") if facing_event && facing_event.name[/lashrock/i] && vHI(:HM07) && $player.badge_count > 6
  commands.push("Dive") if !facing_event && $PokemonGlobal.surfing && vHI(:HM06) && map_metadata&.dive_map_id && dive_check
  ...
  if commands.length == 2
    decision = 0 if pbConfirmMessage("Would you like to use #{commands[0]}?")
  else
    decision = pbMessage("Emulate which move?", commands, commands.length)
  end
  next false if decision == commands.length || decision == nil
  case commands[decision]
  when "Flash" then pbFlash
  when "Surf"
    pbCancelVehicles
  ...
  when "Rock Smash"
    ...
    pbSmashEvent(facing_event)
    pbRockSmashRandomEncounter
  when "Fly"
    pbFadeOutIn {
      scene = PokemonRegionMap_Scene.new(0, false)
      screen = PokemonRegionMapScreen.new(scene)
  ...
  when "Fire Lash"
    $stats.fire_lash_count += 1
    pbSmashEvent(facing_event)
    pbFireLashRandomEncounter
  when "Dive" then pbDive
  end
  next true
})
...
ItemHandlers::UseInField.add(:SLEEPINGGEAR, proc { |item|
  params = ChooseNumberParams.new
  params.setMaxDigits(1)
  params.setDefaultValue(6)
  hours = pbMessageChooseNumber("Sleep for how many hours?", params)
  unless hours == 0
    pbFadeOutIn {
      UnrealTime.add_seconds(3600*hours)
      pbMessage("You slept for #{hours} hour(s).")
      woken_pkmn = []
      $player.party.each do |pkmn|
        next unless pkmn.status == :SLEEP
        woken_pkmn.push(pkmn.name)
        pkmn.status = :NONE
      end
      woken_pkmn_string = ""
      woken_pkmn.each_with_index { |pok, i| woken_pkmn_string << (i+1 == woken_pkmn.length && woken_pkmn.length > 1 ? " and #{pok}" : (i==0) ? "#{pok}" : ", #{pok}") }
      woken_pkmn.length > 1 ? pbMessage("#{woken_pkmn_string} have woken up from their sleep.") : pbMessage("#{woken_pkmn_string} woke up from its sleep.") if woken_pkmn.length > 0
    }
  end
  next true
})

ItemHandlers::UseInField.add(:FRONTIERPASSM, proc { |item|
  pbFrontierPass
  next true
})

ItemHandlers::UseInField.copy(:FRONTIERPASSM, :FRONTIERPASSF, :FRONTIERPASSO)
...
ItemHandlers::UseOnPokemon.copy(:POTION, :RAGECANDYBAR) if Settings.pbGetGeneration < 7

ItemHandlers::UseOnPokemon.add(:SUPERPOTION, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 60 : 50, scene)
})

ItemHandlers::UseOnPokemon.add(:HYPERPOTION, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 150 : 200, scene)
})
...
ItemHandlers::UseOnPokemon.add(:FRESHWATER, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 30 : 50, scene)
})

ItemHandlers::UseOnPokemon.add(:SODAPOP, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 50 : 60, scene)
})

ItemHandlers::UseOnPokemon.add(:LEMONADE, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 70 : 80, scene)
})
...
ItemHandlers::UseOnPokemon.copy(:FULLHEAL, :RAGECANDYBAR) if Settings.pbGetGeneration >= 7
...
ItemHandlers::UseOnPokemon.add(:REVIVE, proc { |item, qty, pkmn, scene|
  if !pkmn.fainted? || $player.permadeath
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  pkmn.hp = (pkmn.totalhp / 2).floor
  pkmn.hp = 1 if $player.onehp || $player.nohealing
  pkmn.heal_status
  scene.pbRefresh
...
ItemHandlers::UseOnPokemon.add(:MAXREVIVE, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_HP
  pkmn.heal_status
  pokemon.hp = 1 if $player.onehp || $player.nohealing
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}'s HP was restored.", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ENERGYPOWDER, proc { |item, qty, pkmn, scene|
  if pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 60 : 50, scene)
...
ItemHandlers::UseOnPokemon.add(:ENERGYROOT, proc { |item, qty, pkmn, scene|
  if pbHPItem(pkmn, (Settings.pbGetGeneration >= 7) ? 120 : 200, scene)
...
ItemHandlers::UseOnPokemonMaximum.add(:HPUP, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:HP, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:HPUP, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:HP, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemonMaximum.add(:PROTEIN, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:ATTACK, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:PROTEIN, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:ATTACK, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemonMaximum.add(:IRON, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:DEFENSE, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:IRON, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:DEFENSE, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemonMaximum.add(:CALCIUM, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:SPECIAL_ATTACK, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:CALCIUM, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:SPECIAL_ATTACK, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemonMaximum.add(:ZINC, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:SPECIAL_DEFENSE, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:ZINC, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:SPECIAL_DEFENSE, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemonMaximum.add(:CARBOS, proc { |item, pkmn|
  next pbMaxUsesOfEVRaisingItem(:SPEED, 10, pkmn, Settings.pbGetGeneration >= 8)
})

ItemHandlers::UseOnPokemon.add(:CARBOS, proc { |item, qty, pkmn, scene|
  next pbUseEVRaisingItem(:SPEED, 10, qty, pkmn, "vitamin", scene, Settings.pbGetGeneration >= 8)
})
...
ItemHandlers::UseOnPokemon.add(:RARECANDY, proc { |item, qty, pkmn, scene|
  ...
  if pkmn.level >= GameData::GrowthRate.max_level
    ...
    if Settings.pbGetGeneration < 8 || !new_species
      scene.pbDisplay(_INTL("It won't have any effect."))
      next false
    end
    # Check for evolution

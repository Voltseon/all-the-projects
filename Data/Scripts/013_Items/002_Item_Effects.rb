...
ItemHandlers::UseText.copy(:BICYCLE,:MACHBIKE,:ACROBIKE,:ELECTRICBIKE)
...
ItemHandlers::UseFromBag.copy(:BICYCLE,:MACHBIKE,:ACROBIKE,:ELECTRICBIKE)
...
ItemHandlers::UseInField.copy(:BICYCLE,:MACHBIKE,:ACROBIKE,:ELECTRICBIKE)
...
ItemHandlers::UseOnPokemon.add(:PETRIFYHEAL,proc { |item,pkmn,scene|
  if pkmn.fainted? || pkmn.status != :PETRIFIED
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} broke out of petrification.",pkmn.name))
  next true
})

ItemHandlers::UseOnPokemon.copy(:PETRIFYHEAL,:EMONBERRY)
...
ItemHandlers::UseOnPokemon.add(:ABILITYCANDY,proc { |item,pkmn,scene|
...
# Credit to Golisopod User from Gen 8 Project - Ability Patch
ItemHandlers::UseOnPokemon.add(:HIDDENABILITYCANDY,proc { |item,pkmn,scene|
  abils = pkmn.getAbilityList
  hiddenArr =[]
  for i in abils

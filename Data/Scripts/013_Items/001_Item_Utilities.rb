...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if pkmn.level == new_level || pkmn.shadowPokemon? || $player.levelone
...
def pbChangeExp(pkmn, new_exp, scene)
  ...
  if pkmn.exp == new_exp || $player.levelone
    if scene.is_a?(PokemonPartyScreen)
...
def pbGainExpFromExpCandy(pkmn, base_amt, qty, scene)
  if pkmn.level >= GameData::GrowthRate.max_level || pkmn.shadowPokemon? || $player.levelone
...
def pbHPItem(pkmn, restoreHP, scene)
  if !pkmn.able? || pkmn.hp == pkmn.totalhp || ($player.onehp || $player.nohealing) && pkmn.hp > 0
    scene.pbDisplay(_INTL("It won't have any effect."))
    return false
  end
...
def pbBattleHPItem(pkmn, battler, restoreHP, scene)
  return false unless battler.canHeal?
  if battler
  ...
  if pbConfirmMessage(_INTL("Should {1} forget a move to learn {2}?", pkmn_name, move_name), &block)
    loop do
      ...
      if move_index >= 0
        ...
        if by_machine && Settings.pbGetGeneration == 5

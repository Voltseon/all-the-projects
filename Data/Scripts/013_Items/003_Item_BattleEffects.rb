...
ItemHandlers::CanUseInBattle.add(:PETRIFYHEAL,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
  next pbBattleItemCanCureStatus?(:PETRIFIED, pokemon, scene, showMessages)
})

ItemHandlers::CanUseInBattle.copy(:PETRIFYHEAL,:EMONBERRY)
...
ItemHandlers::BattleUseOnPokemon.add(:PETRIFYHEAL,proc { |item,pokemon,battler,choices,scene|
  pokemon.heal_status
  battler.pbCureStatus(false) if battler
  name = (battler) ? battler.pbThis : pokemon.name
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} broke out of its petrification.",name))
})

ItemHandlers::BattleUseOnPokemon.copy(:PETRIFYHEAL,:EMONBERRY)

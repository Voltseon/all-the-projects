...
def pbPhoneRegisterNPC(ident, name, mapid, showmessage = true)
  ...
  pbMessage(_INTL("\\me[Register phone]Registered {1} in the P★DA.\\wtnp[30]", name)) if showmessage
end
...
def pbPhoneRegisterBattle(message, event, trainertype, trainername, maxbattles)
  ...
  pbMessage(_INTL("\\me[Register phone]Registered {1} in the P★DA.\\wtnp[30]", displayname))
end
...
#===============================================================================
def pbCallTrainer(trtype, trname, common_event_name)
  trainer = pbFindPhoneTrainer(trtype, trname)
  return if !trainer
  # Special NPC contacts
  if trainer.length != 8
    if !pbCommonEvent(common_event_name)
...
def pbEncounterSpecies(phonenum)
  ...
  if !species
    ...
    if !species
      ...
      if !species
        species = pbRandomEncounterSpecies(enc_tables[:LandDay])
      end
    end
  end
  return "" if !species
...
def pbPhoneCall(call, phonenum)
  pbMessage(_INTL("\\se[Phone ring]......\\wt[5] ......\\1"))
  ...
  gender = GameData::TrainerType.get(phonenum[1]).gender
  prefix = ((gender == 0) ? "\\b" : (gender == 1) ? "\\r" : "")
  prefix += _INTL("\\f[phone_{1}]",phonenum[1]) unless $game_temp.in_phone

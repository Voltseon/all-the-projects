...
class Window_AdvancedTextPokemon < SpriteWindow_Base
  ...
  def refresh
    ...
    (@drawncurchar + 1..numchars).each do |i|
      ...
      $current_drawing_char = @fmtchars[i][0]
    ...
    if self.active && @item_max > 0 && @index >= 0 && !@ignore_input
      if Input.repeat?(Input::UP)
        if @index >= @column_max ||
          ...
          if @index != oldindex
            self.is_a?(Window_Pokedex) ? pbSEPlay("Pokedex cursor") : pbPlayCursorSE
            update_cursor_rect
          end
        end
      elsif Input.repeat?(Input::DOWN)
        ...
        if @index > 0
          ...
          if @index != oldindex
            self.is_a?(Window_Pokedex) ? pbSEPlay("Pokedex jump") : pbPlayCursorSE

...
class ChooseNumberParams
  attr_accessor :messageSkin   # Set the full path for the message's window skin
  attr_reader :skin
...
def pbCreateMessageWindow(viewport = nil, skin = nil, character = nil)
  msgwindow = Window_AdvancedTextPokemon.new("")
  if viewport
    msgwindow.viewport = viewport
  else
  ...
  skin = "Graphics/Windowskins/character.png" if character
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  return if !msgwindow
  $game_temp.minimap.visible = false if $game_temp.minimap
  ...
  if $player
    text.gsub!(/\\cap/i, "#{(($game_switches && $game_switches[59]) ? "\\c[#{$player.suit+1}]" : "" )}\\gng[cap]\\pop[digital]")
    text.gsub!(/\\sonar/i, "#{(($game_switches && $game_switches[59]) ? "\\c[#{$player.suit == 1 ? 1 : 2}]" : "" )}\\gng[sonar]\\pop[digital]")
    text.gsub!(/\\pavo/i, "#{(($game_switches && $game_switches[59]) ? "\\c[#{$player.suit == 2 ? 1 : 3}]" : "" )}\\gng[pavo]\\pop[digital]")
    text.gsub!(/\\dr/i, "#{(($game_switches && $game_switches[59]) ? "\\c[#{$player.suit == 3 ? 1 : 4}]" : "" )}\\gng[cygnus]\\pop[digital]")
    text.gsub!(/\\luna/i, "#{(($game_switches && $game_switches[59]) ? "\\c[#{$player.suit == 4 ? 1 : 5}]" : "" )}\\gng[luna]\\pop[digital]")
  end
  text.gsub!(/\\[Ww]\[([^\]]*)\]/) {
    w = $1.to_s
    if w == ""
  ...
  text.gsub!(/\\[Gg][Nn][Gg]\[([^\]]*)\]/) {
    ...
    gang.bitmap = Bitmap.new("Graphics/Pictures/Speak/#{char}")
    gang.ox = gang.bitmap.width / 2
    gang.oy = gang.bitmap.height / 2
    ...
    charname.oy = charname.bitmap.height / 2
    ...
    name = "Captain" if char == "cap"
    name = "Dr. Cygnus" if char == "cygnus"
    pbDrawShadowText(charname.bitmap, 0, 0, 80, 32, name, MessageConfig::DARK_TEXT_MAIN_COLOR, MessageConfig::DARK_TEXT_SHADOW_COLOR, 1)
    next ""
  }
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|se|wt|wtnp|ch|pop)\[([^\]]*)\]|\\(g|cn|pt|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  letterpop_cooldown = 0
  loop do
    letterpop_cooldown += 1
    if signWaitCount > 0
      signWaitCount -= 1
      if atTop
    ...
    gang&.update
    charname&.update
    if letterpop_cooldown >= 2
      letterSound(character) if msgwindow.waitcount == 0 && msgwindow.busy? && !msgwindow.pausing?
      letterpop_cooldown = 0
    end
...
def pbMessage(message, commands = nil, cmdIfCancel = 0, skin = nil, defaultCmd = 0, &block)
  if message[/\\lgm/i]
    ...
    message.length.times do |i|
      ...
      case char.downcase
      when "a", "e", "i", "o", "u"
        (["a", "e", "i", "o", "u"].find_index(char.downcase)+1).times { new_message += "<icon=lgm0>" }
      when "b", "c", "d", "f", "g", "k", "l", "m"
        (["b", "c", "d", "f", "g", "k", "l", "m"].find_index(char.downcase)+1).times { new_message += "<icon=lgm1>" }
      when "n", "p", "q", "r", "s", "t", "h", "j"
        (["n", "p", "q", "r", "s", "t", "h", "j"].find_index(char.downcase)+1).times { new_message += "<icon=lgm2>" }
      when "v", "w", "x", "y", "z"
        (["v", "w", "x", "y", "z"].find_index(char.downcase)+1).times { new_message += "<icon=lgm3>" }
      else
        new_message += char
      end
      new_message += "-"
  ...
  ret = 0
  msgwindow = pbCreateMessageWindow(nil, skin, ["\\cap","\\sonar","\\pavo","\\dr","\\luna","\\gng"].any? { |word| message.downcase.include?(word) })
  if commands
...
def pbMessageChooseNumber(message, params, &block)
  msgwindow = pbCreateMessageWindow(nil, params.messageSkin, ["\\cap","\\sonar","\\pavo","\\dr","\\luna","\\gng"].any? { |word| message.downcase.include?(word) })
  ret = pbMessageDisplay(msgwindow, message, true,
                         proc { |msgwindow|
...
def pbShowCommandsWithHelp(msgwindow, commands, help, cmdIfCancel = 0, defaultCmd = 0)
  msgwin = msgwindow
  msgwin = pbCreateMessageWindow(nil, nil, ["\\cap","\\sonar","\\pavo","\\dr","\\luna","\\gng"].any? { |word| message.downcase.include?(word) }) if !msgwindow

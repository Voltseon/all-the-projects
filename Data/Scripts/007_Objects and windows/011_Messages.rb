...
#===============================================================================
def pbGetGoldString
  return _INTL("${1}", ($player) ? $player.money.decrypt.to_s_formatted : "0")
end
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  msgwindow.textspeed = -999 if $player&.nostory
  ret = nil
  commands = nil
  facewindow = nil
  goldwindow = nil
  ...
  text.gsub!(/\\pm/i,  _INTL("${1}", $player.money.decrypt.to_s_formatted)) if $player && $player.money
  text.gsub!(/\\n/i,   "\n")
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|-|se|wt|wtnp|ch)\[([^\]]*)\]|\\(g|mg|cn|pt|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  loop do
    ...
    controls.length.times do |i|
      ...
      when "mg"      # Display gold window
        momgoldwindow&.dispose
        momgoldwindow = pbDisplayMomGoldWindow(msgwindow)

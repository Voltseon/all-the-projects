...
def pbTextBitmap(text, maxwidth = Graphics.width, base = Color.new(248, 248, 248), shadow = Color.new(168, 184, 184), style = 0)
  tmp = Bitmap.new(maxwidth, Graphics.height)
  case style
  when 0 then pbSetSystemFont(tmp)
  when 1 then pbSetNarrowFont(tmp)
  when 2 then pbSetSmallFont(tmp)
  end
  drawFormattedTextEx(tmp, 0, 4, maxwidth, text, base, shadow)
  return tmp
end
...
class EventScene
  ...
  def addLabel(x, y, width, text, base = Color.new(248, 248, 248), shadow = Color.new(168, 184, 184), style = 0)
    addBitmap(x, y, pbTextBitmap(text, width, base, shadow, style))
  end

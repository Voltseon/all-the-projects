module Compiler
  ...
  def compile_planets(path = "PBS/planets.txt")
    compile_pbs_file_message_start(path)
    GameData::Planet::DATA.clear
    schema = GameData::Planet::SCHEMA
    planet_names = []
    planet_descriptions = []
    planet_maps = []
    planet_shows = []
    ...
    planet_frame_counts = []
    planet_gravities = []
    planet_connection_tops = []
    planet_connection_bottoms = []
    planet_connection_lefts = []
    planet_connection_rights = []
    planet_hash = nil
    # Read each line of planets.txt at a time and compile it into a planet
    idx = 0
    pbCompilerEachPreppedLine(path) { |line, line_no|
      echo "." if idx % 250 == 0
      idx += 1
      if line[/^\s*\[\s*(.+)\s*\]\s*$/]   # New section [planet_id]
        # Add previous planet's data to records
        GameData::Planet.register(planet_hash) if planet_hash
        # Parse planet ID
        planet_id = $~[1].to_sym
        if GameData::Planet.exists?(planet_id)
          raise _INTL("Planet ID '{1}' is used twice.\r\n{2}", planet_id, FileLineData.linereport)
        end
        # Construct planet hash
        planet_hash = {
          :id => planet_id
        }
      elsif line[/^\s*(\w+)\s*=\s*(.*)\s*$/]   # XXX=YYY lines
        ...
        property_value = pbGetCsvRecord($~[2], line_no, line_schema)
        # Record XXX=YYY setting
        planet_hash[line_schema[0]] = property_value
        case property_name
        when "Name"
          planet_names.push(planet_hash[:name])
        when "Description"
          planet_descriptions.push(planet_hash[:description])
        when "Map"
          planet_maps.push(planet_hash[:map])
        when "Shows on Map"
          planet_shows.push(planet_hash[:shows_on_map])
        when "Map X"
          planet_map_x.push(planet_hash[:map_x])
        when "Map Y"
          planet_map_y.push(planet_hash[:map_y])
        when "Frame Count"
          planet_frame_counts.push(planet_hash[:frame_count])
        when "Gravity"
          planet_gravities.push(planet_hash[:gravity])
        when "ConnectionTop"
          planet_connection_tops.push(planet_hash[:connection_top])
        when "ConnectionBottom"
          planet_connection_bottoms.push(planet_hash[:connection_bottom])
        when "ConnectionLeft"
          planet_connection_lefts.push(planet_hash[:connection_left])
        when "ConnectionRight"
          planet_connection_rights.push(planet_hash[:connection_right])
        end
      else   # Old format
        # Add previous planet's data to records
        GameData::Planet.register(planet_hash) if planet_hash
        # Parse planet
        line = pbGetCsvRecord(line, line_no, line_schema)
        planet_id = line[1].to_sym
        if GameData::Planet.exists?(planet_id)
          raise _INTL("Planet ID '{1}' is used twice.\r\n{2}", planet_id, FileLineData.linereport)
        end
        # Construct planet hash
        planet_hash = {
          ...
          :map_id            => line[:map_id],
          :shows_on_map      => line[:shows_on_map],
          :map_x             => line[:map_x],
          :map_y             => line[:map_y],
          :frame_count       => line[:frame_count],
          :gravity           => line[:gravity],
          :connection_top    => line[:connection_top],
          :connection_bottom => line[:connection_bottom],
          :connection_left   => line[:connection_left],
          :connection_right  => line[:connection_right]
        }
        # Add planet's data to records
        GameData::Planet.register(planet_hash)
        planet_names.push(planet_hash[:name])
        planet_descriptions.push(planet_hash[:description])
        planet_maps.push(planet_hash[:map])
        planet_shows.push(planet_hash[:shows_on_map])
    ...
    # Add last planet's data to records
    GameData::Planet.register(planet_hash) if planet_hash
    # Save all data
    GameData::Planet.save
    MessageTypes.setMessagesAsHash(MessageTypes::Planets, planet_names)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetDescriptions, planet_descriptions)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetMaps, planet_maps)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetShows, planet_shows)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetMapX, planet_map_x)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetMapY, planet_map_y)
    MessageTypes.setMessagesAsHash(MessageTypes::PlanetFrameCount, planet_frame_counts)
    #MessageTypes.setMessagesAsHash(MessageTypes::PlanetGravities, planet_gravities)
    process_pbs_file_message_end
  end

module Compiler
  ...
  #=============================================================================
  def compile_trainers(paths = ["PBS/trainers_easy.txt", "PBS/trainers.txt", "PBS/trainers_hard.txt", "PBS/trainers_insane.txt"])
    GameData::Trainer::clearData
    max_level = GameData::GrowthRate.max_level
      ...
      pbCompilerEachPreppedLine(path) { |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          if trainer_hash
            ...
            GameData::Trainer.register(trainer_hash, i)
          end
    ...
    if safeExists?(filename)
      File.open(filename, "rb") { |f|
        ...
        pbEachFileSection(f) { |section, name|
          ...
          newbegin = rsection[2].capitalize_good
          beginspeech.push(newbegin)
          newend = rsection[3].capitalize_good
          endspeechwin.push(newend)
          newendlose = rsection[4].capitalize_good
          endspeechlose.push(newendlose)
          sections.push(rsection)
        }
      }
    end
      ...
      pbEachFileSectionNumbered(f) { |contents, section_id|
        ...
        if section_id == 0   # Metadata
          ...
          metadata_hash = {
            ...
            :mount_charset     => contents["MountCharset"],

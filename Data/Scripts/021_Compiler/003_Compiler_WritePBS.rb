module Compiler
  ...
  def write_planets(path = "PBS/planets.txt")
    write_pbs_file_message_start(path)
    File.open(path, "wb") { |f|
      add_PBS_header_to_file(f)
      GameData::Planet.each do |planet|
        ...
        f.write(sprintf("[%s]\r\n", planet.id))
        f.write(sprintf("Name = %s\r\n", planet.real_name))
        f.write(sprintf("Description = %s\r\n", planet.real_description))
        f.write(sprintf("MapID = %s\r\n", planet.map_id))
        f.write(sprintf("ShowsOnMap = %s\r\n", planet.shows_on_map))
        f.write(sprintf("MapX = %s\r\n", planet.map_x))
        f.write(sprintf("MapY = %s\r\n", planet.map_y))
        f.write(sprintf("FrameCount = %s\r\n", planet.frame_count))
        f.write(sprintf("Gravity = %s\r\n", planet.gravity))
        f.write(sprintf("ConnectionTop = %s\r\n", planet.connection_top))
        f.write(sprintf("ConnectionBottom = %s\r\n", planet.connection_bottom))
        f.write(sprintf("ConnectionLeft = %s\r\n", planet.connection_left))
        f.write(sprintf("ConnectionRight = %s\r\n", planet.connection_right))
      end
    }
    process_pbs_file_message_end
  end

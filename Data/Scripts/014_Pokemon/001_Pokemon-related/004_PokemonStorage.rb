...
class PokemonStorage
  ...
  def allWallpapers
    return [
       # Basic wallpapers
       _INTL("Sky & Blue"),_INTL("Dark Souls"),_INTL("Star Wars"),_INTL("Field"),
       _INTL("Sprites"),_INTL("Golden Wind"),_INTL("Stone Ocean"),_INTL("Space Jam")
    ]
  end
  ...
  def boxes_almost_full?
    ...
    if full_boxes>Settings::NUM_STORAGE_BOXES*0.75
      return true
    end
    return false
  end
  ...
  def pbStoreCaught(pkmn)
    ...
    if $PokemonStorage.boxes_almost_full?
      i = 0
      while i < 16
        @boxes.push(PokemonBox.new(_INTL("Box {1}",Settings::NUM_STORAGE_BOXES+i+1),BOX_SIZE))
        @boxes[@boxes.length-1].background = i % BASICWALLPAPERQTY
        i+=1
      end
      Settings::NUM_STORAGE_BOXES += 16
    end
    if @currentBox>=0
      pkmn.time_form_set = nil
      pkmn.form          = 0 if pkmn.isSpecies?(:SHAYMIN)

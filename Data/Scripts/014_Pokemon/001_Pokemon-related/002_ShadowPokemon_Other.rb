...
#===============================================================================
def pbPurify(pkmn, scene, timeflute=false)
  return if (!pkmn.shadowPokemon? || pkmn.heart_gauge != 0) && !timeflute
  ...
  if pkmn.saved_exp && !$player.levelone
  ...
  pkmn.happiness = [pkmn.happiness+100,255].min
end
...
def pbRaiseHappinessAndReduceHeart(pkmn, scene, multiplier, show_fail_message = true)
  if !pkmn.shadowPokemon? || (pkmn.happiness == 255 && pkmn.heart_gauge == 0) || pkmn.isSpecies?(:NECROZMA)
...
def pbRaiseHappinessAndReduceHeartNoScene(pkmn, multiplier, show_fail_message = true)
  if !pkmn.shadowPokemon? || (pkmn.happiness == 255 && pkmn.heart_gauge == 0) || pkmn.isSpecies?(:NECROZMA)
    pbMessage(_INTL("It won't have any effect.")) if show_fail_message
    return false
  end
  old_gauge = pkmn.heart_gauge
  old_happiness = pkmn.happiness
  ...
  pkmn.change_heart_gauge("scent", multiplier)
  if pkmn.heart_gauge == old_gauge
    pbMessage(_INTL("{1} turned friendly.", pkmn.name))
  elsif pkmn.happiness == old_happiness
    pbMessage(_INTL("{1} adores you!\nThe door to its heart opened a little.", pkmn.name))
    pkmn.check_ready_to_purify
  else
    pbMessage(_INTL("{1} turned friendly.\nThe door to its heart opened a little.", pkmn.name))
    pkmn.check_ready_to_purify
  end
  return true
end
...
ItemHandlers::UseOnPokemon.add(:TIMEFLUTE, proc { |item, qty, pkmn, scene|
  if !pkmn.shadowPokemon? || pkmn.heart_gauge == 0 || pkmn.isSpecies?(:LUGIA) || pkmn.isSpecies?(:NECROZMA)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  pbPurify(pkmn, scene, true)
  next true
})

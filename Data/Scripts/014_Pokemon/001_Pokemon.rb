...
class Pokemon
  ...
  def tera_type
    generate_new_tera_type if !@tera_type
    return @tera_type
  end
  ...
  def hp=(value)
    return if @hp <= 0 && $player.permadeath
  ...
  def heal_HP
    return if egg?
    return if fainted? && $player.permadeath
    return if $player.nohealing
    @hp = ($player.onehp ? 1 : @totalhp)
  end
  ...
  def ability_id
    if !@ability
      ...
      if abil_index >= 2   # Hidden ability
        @ability = $player.species_abilities[sp_data.id][1][abil_index - 2]
        abil_index = (@personalID & 1) if !@ability
      end
      if !@ability   # Natural ability or no hidden ability defined
        @ability = $player.species_abilities[sp_data.id][0][abil_index] || $player.species_abilities[sp_data.id][0][0]
      end
    end
    return @ability
  end
  ...
  def getAbilityList
    ret = []
    sp_data = species_data
    $player.species_abilities[sp_data.id][0].each_with_index { |a, i| ret.push([a, i]) if a }
    $player.species_abilities[sp_data.id][1].each_with_index { |a, i| ret.push([a, i + 2]) if a }
    return ret
  end
  ...
  def compatible_with_move?(move_id)
    move_data = GameData::Move.try_get(move_id)
    return @ribbons.include?(:NATIONAL) if move_data && move_data.type == :SHADOW
    unless species_data.tutor_moves.length < 5
      return species_data.types.include?(move_data.type) || species_data.tutor_moves.include?(move_data.id) || species_data.egg_moves.include?(move_data.id) || species_data.moves.include?(move_data.id) || @first_moves.include?(move_data.id) if $player.randomizer_tms
      case move_id
      when :RECOVER then return true
      when :GASTROACID,:FIRELASH,:DUALCHOP,:DRAININGKISS,:MOONBLAST
        return species_data.types.include?(move_data.type) || species_data.tutor_moves.include?(move_data.id) || species_data.egg_moves.include?(move_data.id) || species_data.moves.include?(move_data.id) || @first_moves.include?(move_data.id)
      when :NIGHTSHADE
        return species_data.types.include?(move_data.type) || species_data.types.include?(:DARK) || species_data.types.include?(:PSYCHIC) || species_data.tutor_moves.include?(move_data.id) || species_data.egg_moves.include?(move_data.id) || species_data.moves.include?(move_data.id) || @first_moves.include?(move_data.id)
      when :EVOBURST,:EVOTHRUST
        return [:EEVEE,:FLAREON,:JOLTEON,:VAPOREON,:ESPEON,:UMBREON,:GLACEON,:LEAFEON,:SYLVEON].include?(species_data.species)
      end
    end
    return move_data && (species_data.tutor_moves.include?(move_data.id) || species_data.egg_moves.include?(move_data.id) || species_data.moves.include?(move_data.id) || @first_moves.include?(move_data.id))
  end
  ...
  def changeHappiness(method)
    ...
    @happiness = 70 if $player.murphyslaw
    @happiness = 0 if $player.hatemode
    @happiness = 255 if $player.lovemode
  end
  ...
  def check_evolution_internal
    ...
    evos = ($player.randomizer_evolutions) ? $player.species_evolutions[@species] : species_data.get_evolutions(true)
    evos.each do |evo|   # [new_species, method, parameter, boolean]
  ...
  def calc_stats
    ...
    @speed   = stats[:SPEED]
    @hp = oldhp if $player.nohealing
  end

  def generate_new_tera_type
    new_type = types.sample
    @tera_type = new_type
  end

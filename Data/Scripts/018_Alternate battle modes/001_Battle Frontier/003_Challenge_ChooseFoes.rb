...
def pbGenerateBattleTrainer(idxTrainer, rules)
  ...
  # Create the trainer
  is_custom = pbBattleChallenge.battleNumber%7 == 0
  if is_custom
    trainerdata = [:FRONTIERBRAIN_Optus, "Optus", "\\bAh, this will deem interesting...", "That sure was interesting...", "That deemed too interesting...", FrontierBrainList::BRAINS[0][3]] if pbBattleChallenge.currentChallenge.include?("palace")
    trainerdata = [:FRONTIERBRAIN_Tyler, "Tyler", "\\rI shall cut this battle short. No need to extend it.", "I might've cut off too much.", "A little over the top don't you think?", FrontierBrainList::BRAINS[1][3]] if pbBattleChallenge.currentChallenge.include?("arena")
    trainerdata = [:FRONTIERBRAIN_Radsel, "Radsel", "\\bProve to me you're strong enough...", "You disappoint me.", "Impressive, but you can do better.", FrontierBrainList::BRAINS[2][3]] if pbBattleChallenge.currentChallenge.include?("tower")
    trainerdata = [:FRONTIERBRAIN_Annie, "Annie", "\\rYay! Another strong trainer to debug!", "Ooh! I took you DOWN!", "That may require more fixing...", FrontierBrainList::BRAINS[3][3]] if pbBattleChallenge.currentChallenge.include?("factory")
    trainerdata = [:FRONTIERBRAIN_Hashil, "Hashil", "\\bOh... Another challenger?", "Hmm...? I won?", "I lost...?", FrontierBrainList::BRAINS[4][3]] if pbBattleChallenge.currentChallenge.include?("site")
    opponent = NPCTrainer.new(trainerdata[1], trainerdata[0])
    opponent.items.push(:MEGARING)
  else
    trainerdata = bttrainers[idxTrainer]
    opponent = NPCTrainer.new(
      pbGetMessageFromHash(MessageTypes::TrainerNames, trainerdata[1]).capitalize_good(false),
      trainerdata[0]
    )
  end
  ...
  if is_custom
    finalpkmn = []
    rules.ruleset.suggestedNumber.times do |i|
      addMon = pokemonnumbers.sample
      finalpkmn.push(addMon)
      pokemonnumbers.delete(addMon)
    end
    finalpkmn.each do |n|
      pkmn = Pokemon.new(n[0],level)
      pkmn.gender = n[1]==1 ? "Female" : "Male"
      pkmn.form = n[2]
      if pkmn.mega?
        pkmn.form = pkmn.species_data.unmega_form
        pkmn.item = pkmn.species_data.mega_stone
      end
      pkmn.shiny = n[3]

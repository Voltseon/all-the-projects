...
class BattleChallenge
  ...
  def pbBattle
    return @bc.extraData.pbBattle(self) if @bc.extraData && battleNumber%7!=0   # Battle Factory
    ...
    opponent.lose_text = pbGetMessageFromHash(MessageTypes::EndSpeechLose, trainerdata[4]).capitalize_good
    opponent.win_text = pbGetMessageFromHash(MessageTypes::EndSpeechWin, trainerdata[3]).capitalize_good
...
class BattleChallengeData
  ...
  def pbGainBP
    ...
    pbMessage(_INTL("\\me[RSE 313 Battle Points Get!]You received {1} BP!\\wtnp[20]",bpgain))
    $stats.battle_points_won += bpgain
    $player.battle_points += bpgain
    if @battleNumber%7 == 0
      type = 0
      type = 0 if pbBattleChallenge.currentChallenge.include?("palace")
      type = 1 if pbBattleChallenge.currentChallenge.include?("arena")
      type = 2 if pbBattleChallenge.currentChallenge.include?("tower")
      type = 3 if pbBattleChallenge.currentChallenge.include?("factory")
      type = 4 if pbBattleChallenge.currentChallenge.include?("site")
      newmedal = $player.frontier_medals[type]+1
      $player.set_medal(type, [newmedal,4].min)
      pbMessage(_INTL("\\me[RSE 315 Battle Symbol Get!]You received a Battle Symbol!\\wtnp[20]")) if newmedal < 5
    end
  end
  ...
  def pbRest
    ...
    @resting = true
    pbSaveInProgress
    pbMessage("Your game has been saved.")
  ...
  def pbSaveInProgress
    ...
    $game_player.moveto(@start[1], @start[2])
    $game_player.direction = 8   # facing up
    Game.save
    $game_map.map_id = oldmapid
    $game_player.moveto(oldx, oldy)

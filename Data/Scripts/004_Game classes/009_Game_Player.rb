...
class Game_Player < Game_Character
  ...
  def can_run?
    ...
    return false if $player.character_ID == 6
  ...
  def move_generic(dir, turn_enabled = true)
    ...
    if !$game_temp.encounter_triggered
      ...
      return if pbRocketBoots(x_offset, y_offset)
  ...
  def jump(x_plus, y_plus)
    ...
    if real_distance > 0   # Jumping to somewhere else
      ...
      @jump_count = (Game_Map::REAL_RES_X.to_f / jump_speed_real.to_f / $player.planet.gravity).round   # Number of frames to jump one tile
    end
    @stop_count = 0
    triggerLeaveTile
  end
  ...
  #-----------------------------------------------------------------------------
  def center_smooth(x, y, speed = 0.5)
  ...
  def moveto(x, y)
    super
    center(x, y)
    $scene.updateSpritesets(true) if $scene.is_a?(Scene_Map) && !$scene.map_renderer.nil?
  ...
  def update_move
    if !@moved_last_frame || @stopped_last_frame   # Started a new step
      if pbTerrainTag.ice
        ...
        faster = can_run? || $game_temp.alarms
        if $PokemonGlobal&.diving
...
def pbGetPlayerCharset(charset, trainer = nil, force = false)
  ...
  outfit = (trainer) ? trainer.outfit || :NOSUIT : 0
  suit = (trainer) ? trainer.suit || 0 : 0
  return nil if !force && $game_player&.charsetData &&
                ...
                $game_player.charsetData[3] == suit
  $game_player.charsetData = [trainer.character_ID, charset, outfit, suit] if $game_player
  ret = charset
  if pbResolveBitmap("Graphics/Characters/" + ret + "_" + outfit.to_s + "_" + suit.to_s)
    ret = ret + "_" + outfit.to_s + "_" + suit.to_s
  end
  return ret
end

...
class Game_Player < Game_Character
  ...
  def real_x=(value)
    @real_x = value
    @x = (@real_x / Game_Map::REAL_RES_X).round
  end

  def real_y=(value)
    @real_y = value
    @y = (@real_y / Game_Map::REAL_RES_Y).round
  end
  ...
  def set_movement_type(type)
    return if $game_temp.heart_swapped
    ...
    when :cycling, :cycling_fast, :cycling_jumping, :cycling_stopped, :mount
      if !@move_route_forcing
        self.move_speed = ($PokemonGlobal.mounted_pkmn > -1 ? 7 :  (type == :cycling_jumping) ? 3 : 5)
      end
      new_charset = ($PokemonGlobal&.mounted_pkmn > -1 ? pbGetPlayerCharset(meta.mount_charset) : pbGetPlayerCharset(meta.cycle_charset))
    when :running
    ...
    @character_name = new_charset if new_charset && !$game_switches[63]
  end
  ...
  def moveto_transfer(x, y, direction = self.direction)
    $game_temp.player_transferring = true
    $game_temp.player_new_map_id    = self.map.map_id
  ...
  def moved_last_frame; @moved_last_frame; end
...
def pbUpdateVehicle
  if $PokemonGlobal&.diving
    ...
    $game_player.set_movement_type(:mount)
  elsif $PokemonGlobal&.bicycle
...
def pbCanUseBike?(map_id)
  return false if $game_temp.heart_swapped

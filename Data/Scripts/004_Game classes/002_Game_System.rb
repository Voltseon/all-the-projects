...
################################################################################
  ...
  def bgm_play_internal2(name, volume, pitch, position, track = 0) # :nodoc:
    vol = volume
  ...
  def bgm_play_internal(bgm, position, track = 0) # :nodoc:
    if !track || track == 0
  ...
  def bgm_play_continue(name, volume = -1, pitch = -1)
    if @defaultBGM
      volume = @defaultBGM.volume if volume < 0
      pitch  = @defaultBGM.pitch  if pitch < 0
    else
      volume = 80 if volume < 0
      pitch  = 100 if pitch < 0
    end
    bgm_play_internal2("Audio/BGM/" + name, (volume * $PokemonSystem.bgmvolume / 100.0).to_i, pitch, Audio.bgm_pos, 1)
    Audio.bgm_stop(0)
  end

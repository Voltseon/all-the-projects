...
class PokemonMapFactory
  ...
  def setMapChanging(newID, newMap)
    ...
    EventHandlers.trigger(:on_map_or_spriteset_change, $scene, @mapChanged) if $scene.is_a?(Scene_Map)
  end
  ...
  def setSceneStarted(scene)
    ...
    @mapChanged = false
    #scene.spriteset.addUserSprite(LocationWindow.new($game_map.name)) if $game_map.metadata&.announce_location && !$game_temp.location_window_showing
  end

...
class Game_Map
  ...
  attr_accessor :shadows_name             # shadows file name
  attr_accessor :shadows_hue              # shadows hue
  attr_accessor :shadows_opacity          # shadows opacity level
  attr_accessor :shadows_blend_type       # shadows blending method
  attr_accessor :shadows_zoom             # shadows zoom rate
  attr_accessor :shadows_sx               # shadows sx
  attr_accessor :shadows_sy               # shadows sy
  attr_accessor :shadows_ox               # shadows x-coordinate starting point
  attr_accessor :shadows_oy               # shadows y-coordinate starting point
  attr_reader   :shadows_tone             # shadows color tone
  ...
  def setup(map_id)
    ...
    @shadows_ox           = 0
    @shadows_oy           = 0
    @shadows_tone         = Tone.new(0, 0, 0, 0)
    @shadows_tone_target  = Tone.new(0, 0, 0, 0)
    @fog_ox               = 0
    @fog_oy               = 0
  ...
  #-----------------------------------------------------------------------------
  def autoplay
    #$game_system.bgm_play_internal2(anonicalize("Audio/BGM/" + bgm_name), @map.bgm.volume, @map.bgm.pitch, 0, 0) if @map.autoplay_bgm
  ...
  def playerPassable?(x, y, d, self_event = nil)
    ...
    [2, 1, 0].each do |i|
      ...
      passage = @passages[tile_id]
      if terrain
        return false if ![:RawData, :DataPad, :Neutral].include?(terrain.id) && $player.character_ID == 6
        ...
        return $player.character_ID == 6 if terrain.id == :RawData
  ...
  def update
    ...
    # Update minimap
    if $game_temp.minimap
      $game_temp.minimap.pbUpdate
    end
  end
end

class Game_Character
  ...
  def pattern; return @pattern; end
  ...
  def move_speed=(val)
    ...
    self.move_speed_real = (val == 7) ? pbGetMountedSpeed : (val == 6) ? 64 : (val == 5) ? 32 : (2**(val + 1)) * 0.8
  end
  ...
  def passable?(x, y, d, strict = false)
    ...
    self.map.events.each_value do |event|
      ...
      return true if event.erased && (event.name[/smashrock/i] || event.name[/lashrock/i] || event.name[/cuttree/i])
  ...
  def screen_z(height = 0)
    return 99999 if self.is_a?(Game_Event) && self.name[/regulartone/i]
  ...
  def set_opacity(opacity); @opacity = opacity; end
  ...
  def jump(x_plus, y_plus)
    ...
    if real_distance > 0   # Jumping to somewhere else
      ...
      @jump_count = Game_Map::REAL_RES_X / jump_speed_real * ($PokemonGlobal.diving ? 2 : 1)  # Number of frames to jump one tile
    end
    @stop_count = 0
    triggerLeaveTile
  end

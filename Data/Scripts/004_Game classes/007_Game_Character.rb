class Game_Character
  ...
  def mirror_player(center_y, rect)
    self.character_name = $game_player.character_name
    pattern = $game_player.pattern
    self.pattern = pattern
    direction = $game_player.direction
    case direction
      when 2 then self.direction = 8
      when 4 then self.direction = 6
      when 6 then self.direction = 4
      when 8 then self.direction = 2
    end
    @real_x = $game_player.real_x
    center_real = center_y * Game_Map::REAL_RES_Y
    @real_y = (center_real - $game_player.real_y) + center_real
    mirror_frame = Rect.new(*rect)
    min_x = mirror_frame.x * Game_Map::REAL_RES_X
    max_x = mirror_frame.width * Game_Map::REAL_RES_X
    min_y = mirror_frame.y * Game_Map::REAL_RES_Y
    max_y = mirror_frame.height * Game_Map::REAL_RES_Y
    @opacity = @real_x > max_x || @real_x < min_x || @real_y > max_y || @real_y < min_y ? 0 : 255
  end
  ...
  def move_speed=(val)
    ...
    self.move_speed_real = ((val == 6) ? 64 : (val == 5) ? 32 : (2**(val + 1)) * 0.8).to_f * $player.planet.gravity
  end
  ...
  def jump_speed_real
    move_speed_real
    #self.jump_speed_real = ((2**(3 + 1)) * 0.8).to_f * $player.planet.gravity if !@jump_speed_real   # 3 is walking speed
  ...
  def move_toward_player_follow
    player_x = $game_player.x
    player_y = $game_player.y
    case $game_player.direction
    when 2 then player_y -= 1
    when 4 then player_x += 1
    when 6 then player_x -= 1
    when 8 then player_y += 1
    end
    sx = @x + (@width / 2.0) - (player_x + ($game_player.width / 2.0))
    sy = @y - (@height / 2.0) - (player_y - ($game_player.height / 2.0))
    return if sx == 0 && sy == 0
    abs_sx = sx.abs
    abs_sy = sy.abs
    if abs_sx == abs_sy
  ...
  def jump(x_plus, y_plus)
    ...
    @jump_peak = distance * Game_Map::TILE_HEIGHT * 3 / 8  * (1 / $player.planet.gravity)  # 3/4 of tile for ledge jumping

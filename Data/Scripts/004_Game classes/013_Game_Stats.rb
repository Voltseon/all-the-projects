...
class GameStats
  ...
  attr_accessor :fire_lash_count, :fire_lash_battles
  ...
  attr_accessor :coins_spent_at_marts
  ...
  def set_time_to_badge(number)
    $PokemonSystem.current_menu_theme = number+1
    @times_to_get_badges[number] = @play_time
  end

  def set_time_to_hall_of_fame
    $PokemonSystem.current_menu_theme = 9

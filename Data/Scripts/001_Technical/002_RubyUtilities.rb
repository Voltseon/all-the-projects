...
class String
  ...
  def capitalize_good(sentance=true)
    string = self.gsub(" .", ".").gsub(" !", "!").gsub(" ?", "?").gsub(" ,", ",")
    new_string = ""
    string.split("").each_with_index do |char, i|
      ...
      if i == 0 || ["!", ".", "?"].include?(string[[i-2,0].max]) || (char.downcase == "i" && string[[i-1,0].max] == " " && string[[i+1,string.length].min] == " ")
        new_string += char.upcase
      else
        new_string += char.downcase
      end
    end
    new_string += [".","!"].sample if !["!", ".", "?"].include?(string[string.length - 1]) && sentance
    return new_string
  end

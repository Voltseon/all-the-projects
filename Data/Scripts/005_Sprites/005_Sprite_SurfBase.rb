class Sprite_SurfBase
  ...
  def initialize(sprite, event, viewport = nil)
    ...
    RPG::Cache.retain("Graphics/Characters/Followers super shiny")
  ...
  def update
    ...
    if @sprite
      if $PokemonGlobal.surfing
        ...
        bmp = AnimatedBitmap.new(GameData::Species.ow_sprite_filename(pkmn.species, pkmn.form, pkmn.gender, pkmn.shiny?, pkmn.shadowPokemon?, pkmn.super_shiny?))
        @sprite.bitmap = bmp.bitmap
        cw = bmp.width / 4
        ch = bmp.height / 4
      end
      sx = @event.pattern_surf * cw
      sx = (sx + @event.pattern * cw)%bmp.width if $PokemonGlobal.mounted_pkmn > -1 && !$PokemonGlobal.surfing && !$PokemonGlobal.diving

...
class Spriteset_Map
  ...
  def initialize(map = nil)
    ...
    @shadowslayer = AnimatedPlane.new(@@viewport1)
    @shadowslayer.z = 10
  ...
  def update
    ...
    if @shadows_name != @map.shadows_name
      @shadows_name = @map.shadows_name
      @shadowslayer.set_shadows(nil) if !@shadowslayer.bitmap.nil?
      @shadowslayer.set_shadows(@shadows_name) if pbResolveBitmap(sprintf("Graphics/Shadows/" + @shadows_name))
      Graphics.frame_reset
    end
    ...
    if $game_temp
      if $game_temp.panorama_scroll_x != 0 || $game_temp.panorama_scroll_y != 0
        if $game_temp.panorama_scroll_x != 0
          @panorama.ox += $game_temp.panorama_scroll_x
        else
          @panorama.ox = tmox
        end
        if $game_temp.panorama_scroll_y != 0
          @panorama.oy += $game_temp.panorama_scroll_y
        else
          @panorama.oy = tmoy
        end
      else
    ...
    @shadowslayer.ox     = tmox + @map.shadows_ox
    @shadowslayer.oy     = tmoy + @map.shadows_oy

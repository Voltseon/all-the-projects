...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    shadow_color = Color.new(56,23,4)
    ...
    textpos=[
       [_INTL("Teach which move?"),16,2,0,base_color,shadow_color]
    ]
    imagepos=[]
    yPos=76
    for i in 0...VISIBLEMOVES
      ...
      if moveobject
        ...
        imagepos.push(["Graphics/Pictures/types", 12, yPos + 39, 0, type_number * 28, 64, 28])
        textpos.push([moveData.name,8,yPos,0,base_color,shadow_color])
        if moveData.total_pp>0
          textpos.push([_INTL("PP"),112,yPos+32,0,base_color,shadow_color])
          textpos.push([_INTL("{1}/{1}",moveData.total_pp),230,yPos+32,1,
             base_color,shadow_color])
        else
          textpos.push(["-",80,yPos,0,base_color,shadow_color])
          textpos.push(["--",228,yPos+32,1,base_color,shadow_color])
        end
      end
      yPos+=64
    end
    ...
    textpos.push([_INTL("CATEGORY"),272,108,0,base_color,shadow_color])
    textpos.push([_INTL("POWER"),272,142,0,base_color,shadow_color])
    textpos.push([basedamage<=1 ? basedamage==1 ? "???" : "---" : sprintf("%d",basedamage),
          468,142,2,base_color,shadow_color])
    textpos.push([_INTL("ACCURACY"),272,176,0,base_color,shadow_color])
    textpos.push([accuracy==0 ? "---" : "#{accuracy}%",
          468,176,2,base_color,shadow_color])
    pbDrawTextPositions(overlay,textpos)
    imagepos.push(["Graphics/Pictures/category",446,116,0,category*28,64,28])
    ...
    drawTextEx(overlay,272,220,230,5,selMoveData.description,
       base_color,shadow_color)
  end

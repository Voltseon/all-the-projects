...
class PokemonBag_Scene
  ...
  ITEMLISTSHADOWCOLOR   = Color.new(56,23,4)
  ITEMTEXTBASECOLOR     = Color.new(248,248,248)
  ITEMTEXTSHADOWCOLOR   = Color.new(21,50,67)
  POCKETNAMEBASECOLOR   = Color.new(248,248,248)
  POCKETNAMESHADOWCOLOR = Color.new(56,23,4)
  ITEMSVISIBLE          = 5
  ...
  def pbStartScene(bag,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sprites["bagsprite"] = IconSprite.new(Graphics.width/2-64,36,@viewport)
    @sprites["pocketicon"] = BitmapSprite.new(186,32,@viewport)
    @sprites["pocketicon"].x = 61
    @sprites["pocketicon"].y = 33
    @sprites["leftarrow"] = AnimatedSprite.new("Graphics/Pictures/leftarrow",8,40,28,2,@viewport)
    @sprites["leftarrow"].x       = Graphics.width/2-90
    @sprites["leftarrow"].y       = 86
    @sprites["leftarrow"].visible = (!@choosing || numfilledpockets>1)
    @sprites["leftarrow"].play
    @sprites["rightarrow"] = AnimatedSprite.new("Graphics/Pictures/rightarrow",8,40,28,2,@viewport)
    @sprites["rightarrow"].x       = Graphics.width/2+50
    @sprites["rightarrow"].y       = 86
    @sprites["rightarrow"].visible = (!@choosing || numfilledpockets>1)
    @sprites["rightarrow"].play
    @sprites["itemlist"] = Window_PokemonBag.new(@bag,@filterlist,lastpocket,203,166,279,69+ITEMSVISIBLE*32)
    ...
    @sprites["itemicon"] = ItemIconSprite.new(29,149,nil,@viewport)
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize("",
       -12, 164, 250, 256, @viewport)
  ...
  def pbRefresh
    ...
    @sprites["pocketicon"].bitmap.blt(0,2,@pocketbitmap.bitmap,Rect.new((@sprites["itemlist"].pocket-1)*28,0,28,28))
    if @sprites["itemlist"].pocket < 5
      @sprites["pocketicon"].x = 61
      @sprites["pocketicon"].y = 33+(@sprites["itemlist"].pocket-1)*34
    else
      @sprites["pocketicon"].x = 422
      @sprites["pocketicon"].y = 33+(@sprites["itemlist"].pocket-5)*34
    end
  ...
  def pbRefreshIndexChanged
    ...
    pbDrawTextPositions(overlay,[
       [PokemonBag.pocketNames[@bag.lastpocket],Graphics.width/2,-2,2,POCKETNAMEBASECOLOR,POCKETNAMESHADOWCOLOR]
    ])
    # Draw slider arrows
    showslider = false
    if itemlist.top_row>0
      overlay.blt(470,191,@sliderbitmap.bitmap,Rect.new(0,0,36,38))
      showslider = true
    end
    if itemlist.top_item+itemlist.page_item_max<itemlist.itemCount
      overlay.blt(470,334,@sliderbitmap.bitmap,Rect.new(0,38,36,38))
      showslider = true
    end
    # Draw slider box
    if showslider

...
class PokemonEvolutionScene
  ...
  def pbGenerateMetafiles(s1x,s1y,s2x,s2y)
    ...
    totaltempo   = 0
    currenttempo = 100
    maxtempo = 12*Graphics.frame_rate
    while totaltempo<maxtempo
      for j in 0...currenttempo
        ...
        sprite.angle  += [0.3*totaltempo,10].max
        sprite2.angle  += [0.3*totaltempo,10].max
        if totaltempo>maxtempo/2
          sprite2.opacity += 10
        end
        sprite.update
        sprite2.update
  ...
  def pbEvolution(cancancel=true)
    ...
    @sprites["rsprite2"].angle = 0
    pbFlashInOut(canceled,oldstate,oldstate2)
    @sprites["rsprite2"].color = @sprites["rsprite1"].color
    if canceled
      pbMessageDisplay(@sprites["msgwindow"],

...
class PokemonEggHatch_Scene
  def pbStartScene(pokemon)
    ...
    @sprites["pokemon"].setSpeciesBitmap(@pokemon.species, @pokemon.gender,
                                         ...
                                         false, false, true, @pokemon.super_shiny?)   # Egg sprite
  ...
  def pbMain
    ...
    @pokemon.name = nil
    waittime = ($GameSpeed == 0 ? 80 : 20)
    pbMessage(_INTL("\\se[]{1} hatched from the Egg!\\wt[#{waittime}]", @pokemon.name)) { update }
...
def pbHatch(pokemon)
  ...
  pokemon.timeEggHatched = Time.now

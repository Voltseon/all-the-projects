...
class PokemonSystem
  ...
  def initialize
    ...
    @footstepvolume = 50    # Volume of footstep sound effects
...
class Window_PokemonOption < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    when SliderOption
      value = sprintf(" %d", @options[index].highest_value * 2)
      ...
      value = sprintf("%d", (@options[index].lowest_value + self[index]) * 2)
...
MenuHandlers.add(:options_menu, :footstep_volume, {
  "name"        => _INTL("Footstep Volume"),
  "order"       => 21,
  "type"        => SliderOption,
  "parameters"  => [0, 100, 5],   # [minimum_value, maximum_value, interval]
  "description" => _INTL("Adjust the volume of sound effects."),
  "get_proc"    => proc { next $PokemonSystem.footstepvolume },
  "set_proc"    => proc { |value, _scene|
    next if $PokemonSystem.footstepvolume == value
    $PokemonSystem.footstepvolume = value
    play_footstep_se(:default)
  }
})

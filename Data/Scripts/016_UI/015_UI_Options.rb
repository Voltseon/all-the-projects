...
class PokemonSystem
  ...
  attr_accessor :owpkmn_volume
  attr_accessor :textinput
  attr_accessor :loadtransition
  attr_accessor :owpkmnenabled
  attr_accessor :asknewmoves
  ...
  def initialize
    ...
    @owpkmn_volume = 50    # Volume of overworld pokémon
    @textinput     = 1     # Text input mode (0=cursor, 1=keyboard)
    @owpkmnenabled = 0     # Whether Overworld Pokémon appear (0=on, 1=off)
    @asknewmoves   = 0     # Whether it should ask for a new move (0=on, 1=off)
  end
end
...
class PokemonOption_Scene
  ...
  attr_reader :audio_settings

  def pbStartScene(in_load_screen = false, audio_settings = false)
    ...
    @hashes = []
    if @audio_settings
      MenuHandlers.each_available(:options_menu_audio) do |option, hash, name|
        @options.push(
    ...
    @sprites["title"] = Window_UnformattedTextPokemon.newWithSize(
      _INTL(@audio_settings ? "Audio Options" : "Options"), 0, -16, Graphics.width, 64, @viewport
    )
    ...
    pbActivateWindow(@sprites, "option") {
      ...
      loop do
        ...
        if Input.trigger?(Input::BACK)
          @audio_settings = false if @audio_settings
          break
          ...
          if @sprites["option"].index == @options.length
            ...
            unless @audio_settings
              pbPlayDecisionSE
              System.show_settings
            end
          elsif @sprites["option"].index == 0
            unless @audio_settings
              pbPlayDecisionSE
              pbFadeOutIn do
                scene = PokemonOption_Scene.new
                screen = PokemonOptionScreen.new(scene)
                screen.pbStartScreen(@in_load_screen, true)
              end
            end
          end
        end
...
class PokemonOptionScreen
  ...
  def pbStartScreen(in_load_screen = false, audio_settings = false)
    @scene.pbStartScene(in_load_screen, audio_settings)
    @scene.pbOptions
    @scene.pbEndScene
  end
end
...
MenuHandlers.add(:options_menu, :audio_settings, {
  "name"        => _INTL("Audio Options"),
  "order"       => 10,
  "type"        => EnumOption,
  "parameters"  => [_INTL(""), _INTL("")],
  "description" => _INTL("Open the Audio settings."),
  "get_proc"    => proc {  },
  "set_proc"    => proc {  }
})
...
# Audio Settings
#===============================================================================
MenuHandlers.add(:options_menu_audio, :bgm_volume, {
...
MenuHandlers.add(:options_menu_audio, :se_volume, {
...
MenuHandlers.add(:options_menu_audio, :owpkmn_volume, {
  "name"        => _INTL("Overworld Pokémon Volume"),
  "order"       => 22,
  "type"        => SliderOption,
  "parameters"  => [0, 100, 5],   # [minimum_value, maximum_value, interval]
  "description" => _INTL("Adjust the volume of overworld Pokémon."),
  "get_proc"    => proc { next $PokemonSystem.owpkmn_volume },
  "set_proc"    => proc { |value, _scene|
    next if $PokemonSystem.owpkmn_volume == value
    $PokemonSystem.owpkmn_volume = value
    pbPlayCursorSE
  }
})
...
MenuHandlers.add(:options_menu, :send_to_boxes, {
  ...
  "condition"   => proc { next Settings.pbGetGeneration >= 7 },
...
MenuHandlers.add(:options_menu, :asknewmoves, {
  "name"        => _INTL("Learn Moves"),
  "order"       => 115,
  "type"        => EnumOption,
  "parameters"  => [_INTL("On"), _INTL("Off")],
  "description" => _INTL("Choose whether you can learn a new move upon level up."),
  "get_proc"    => proc { next $PokemonSystem.asknewmoves },
  "set_proc"    => proc { |value, _scene| $PokemonSystem.asknewmoves = value }
})
...
MenuHandlers.add(:options_menu, :open_controls, {

  ...
  def pbSelectBoxInternal(_party)
    ...
    loop do
      ...
      if Input.press?(Input::CTRL)
        ...
        @storage.boxes[@storage.currentBox].pokemon.each_with_index do |pokemon, i|
          if Input.press?(Input::ALT)
            if pokemon.nil? || pokemon.egg?
              pbDrawShadowText(@sprites["overlay"].bitmap, 210+(i%6).floor*48 , 80+(i/6).floor*48 , 120, 32, "#{@storage.currentBox*30 + i + 1}", Color.new(248, 248, 248), Color.new(40, 48, 48))
            else
              pbDrawShadowText(@sprites["overlay"].bitmap, 210+(i%6).floor*48 , 80+(i/6).floor*48 , 120, 32, "#{@species_list.index(pokemon.species)}", Color.new(248, 248, 128), Color.new(40, 48, 24))
            end
          else
            pbDrawShadowText(@sprites["overlay"].bitmap, 210+(i%6).floor*48 , 80+(i/6).floor*48 , 120, 32, "#{@storage.currentBox*30 + i + 1}", Color.new(248, 248, 248), Color.new(40, 48, 48))
          end
      ...
      pbUpdateOverlay(selection) if Input.release?(Input::CTRL)
      key = -1
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    if !pokemon.egg?
      ...
      if pokemon.shiny?
        ...
        imagepos.push(["Graphics/Pictures/shadow", 156, 198])
      end
...
class PokemonStorageScreen
  ...
  def pbPlace(selected)
    ...
    if Settings.pbGetGeneration >= 8 && box >= 0

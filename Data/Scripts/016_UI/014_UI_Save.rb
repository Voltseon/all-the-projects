def pbEmergencySave
  ...
  return if !$player
  if SaveData.exists?
    File.open(SaveData::FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata"), "rb") do |r|
      File.open(SaveData::FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata") + ".bak", "wb") do |w|
        loop do
          s = r.read(4096)
          break if !s
          w.write(s)
...
class PokemonSave_Scene
  def pbStartScreen
    ...
    textColor = ["0070F8,78B8E8", "E82010,F8A8B8", "209808,90F090"][$player.gender]

...
def pbReceiveMysteryGift(id)
  ...
  if gift[1] == 0   # Pokémon
    ...
    if pbAddPokemonSilent(gift[2])
      ...
      if Settings.pbGetGeneration >= 7 && !was_owned && $player.has_pokedex
    ...
    if $bag.can_add?(item, qty)
      ...
      if item == :LEFTOVERS
        pbMessage(_INTL("\\me[RSE 220 Obtained an Item!]You obtained some \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
      elsif itm.is_machine?   # TM or HM
        pbMessage(_INTL("\\me[RSE 153 Obtained a TM!]You obtained \\c[1]{1} {2}\\c[0]!\\wtnp[30]", itemname,
                        GameData::Move.get(itm.move).name))
      elsif qty > 1
        pbMessage(_INTL("\\me[RSE 220 Obtained an Item!]You obtained {1} \\c[1]{2}\\c[0]!\\wtnp[30]", qty, itemname))
      elsif itemname.starts_with_vowel?
        pbMessage(_INTL("\\me[RSE 220 Obtained an Item!]You obtained an \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
      else
        pbMessage(_INTL("\\me[RSE 220 Obtained an Item!]You obtained a \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
      end
      $player.mystery_gifts[index] = [id]
      return true
    end

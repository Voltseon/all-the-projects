...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(as_editor = false, fly_map = false)
    ...
    Settings::ROAMING_SPECIES.length.times do |index|
      pkmn = pbGetRoamer(index)
      ...
      @sprites["roaming#{index}"].x = -SQUARE_WIDTH/2+(roamingPosition[1]*SQUARE_WIDTH)+(Graphics.width-@sprites["map"].bitmap.width)/2
      @sprites["roaming#{index}"].y = -SQUARE_HEIGHT/2+(roamingPosition[2]*SQUARE_HEIGHT)+(Graphics.height-@sprites["map"].bitmap.height)/2
    end

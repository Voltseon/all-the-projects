...
class ReadyMenuButton < Sprite
  ...
  def initialize(index, command, selected, side, viewport = nil)
    ...
    if @command[2]
      ...
      if GameData::Item.get(@command[0]).has_flag?("PDA")
        @button = AnimatedBitmap.new("Graphics/Pictures/Ready Menu/icon_pdabutton")
      else
...
class PokemonReadyMenu
  ...
  def pbStartReadyMenu(moves, items, mountables)
    commands = [[], []]   # Moves, items
    moves.each do |i|
      commands[0].push([i[0], GameData::Move.get(i[0]).name, true, i[1]])
    end
    commands[0].sort! { |a, b| a[1] <=> b[1] }
    mountables.each do |i|
      commands[0].push([:MOUNT, "Mount", true, i])
    end
    items.each do |i|
    ...
    loop do
      ...
      if command[0] == 0   # Use a move
        ...
        if move == :FLY
          ...
          pbHideMenu
          pbMountPkmn(commands[0][command[1]][3], false)
        ...
        if GameData::Item.get(item).has_flag?("PDA")
          MenuHandlers.each_available(:pokegear_menu) do |option, hash, name|
            if hash["pdaitem"] == item
              pbHideMenu
              $game_temp.in_menu = false
              hash["effect"].call(@scene)
              return @scene.pbEndScene
            end
          end
        else  
...
def pbUseKeyItem
  moves = [:CUT, :DEFOG, :DIG, :DIVE, :FLASH, :FLY, :HEADBUTT, :ROCKCLIMB,
           ...
           :WATERFALL, :WHIRLPOOL, :FIRELASH]
  real_moves = []
  mountables = []
  $player.party.each_with_index do |pkmn, i|
    mountables.push(i) if pbIsMountable(i)
  end
  moves.each do |move|
    $player.party.each_with_index do |pkmn, i|
  ...
  $bag.registered_items.each do |i|
    ...
    real_items.push(itm) if $bag.has?(itm) || GameData::Item.get(i).has_flag?("PDA")
  end
  if real_items.length == 0 && real_moves.length == 0
    ...
    sscreen.pbStartReadyMenu(real_moves, real_items, mountables)
    $game_temp.in_menu = false
  end
end

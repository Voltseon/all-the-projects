...
def pbDrawGauge(bitmap, rect, color, value, maxValue)
  ...
  if rect.width >= 4 && rect.height >= 4
    bitmap.fill_rect(rect.x + 2, rect.y + 2, rect.width - 4, rect.height - 4, Color.new(255, 255, 255))
...
#    end
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set) == 0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(_INTL("Place at least one regular Pokémon to proceed the purification of {1}.", pkmn.name))
    end
  end
    ...
    loop do
      ...
      if cmd[0] == 0
        ...
        if curpkmn || heldpkmn
          ...
          if pbCheckPurify
            @scene.pbDisplay(_INTL("There is a Pokémon that is ready to open its heart!"))
...
class PurifyChamberSetView < Sprite
  ...
  def initialize(chamber, set, viewport = nil)
    ...
    @__sprites = []
    @sprites__ = {}
    @sprites__["back0"] = IconSprite.new(0, 0, viewport)
    @sprites__["back0"].setBitmap("Graphics/Pictures/purifychamberball")
    @__sprites[0] = PokemonIconSprite.new(nil, viewport)
    @__sprites[0].setOffset
    (PurifyChamber::SETSIZE * 2).times do |i|
      @sprites__["back#{i + 1}"] = IconSprite.new(0, 0, viewport)
      @sprites__["back#{i + 1}"].setBitmap("Graphics/Pictures/purifychamberball")
  ...
  def refresh
    ...
    if pkmn
      if pkmn.types.length == 1
        textpos.push([_INTL("{1}  Lv.{2}  {3}", pkmn.name, pkmn.level,
                      ...
                      2, 6, 0, Color.new(255, 255, 255), Color.new(148, 198, 61)])
      else
      ...
      textpos.push([_INTL("FLOW"), 2 + (@info.bitmap.width / 2), 30, 0,
                    Color.new(255, 255, 255), Color.new(148, 198, 61)])
      # draw heart gauge
    ...
    @sprites__["back0"].x = 306
    @sprites__["back0"].y = 158
    @sprites__["back0"].visible = !pkmn.nil?
    @sprites__["back0"].z = 2
    ...
    (PurifyChamber::SETSIZE * 2).times do |i|
      ...
      @sprites__["back#{i + 1}"].x = point[0]
      @sprites__["back#{i + 1}"].y = point[1]
      @sprites__["back#{i + 1}"].visible = !pkmn.nil?
      @sprites__["back#{i + 1}"].z = 2
...
class PurifyChamberScene
  ...
  def pbOpenSet(set)
    ...
    @sprites["bg0"].setBitmap("Graphics/Pictures/Pokegear/bg_plus")
    ...
    @sprites["setwindow"].windowskin = nil
    @sprites["setwindow"].selarrow = AnimatedBitmap.new("Graphics/Pictures/selarrow_purify")
    @sprites["setwindow"].baseColor = Color.new(255, 255, 255)
    @sprites["setwindow"].shadowColor = Color.new(148, 198, 61)

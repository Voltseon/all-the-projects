...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
       [pagename,8,4,0,base,shadow],
       [@pokemon.name,46,56,0,base,shadow],
       [@pokemon.level.to_s,46,86,0,Color.new(248,248,248),Color.new(56,23,4)],
       [_INTL("Item"),86,308,0,base,shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name,16,346,0,Color.new(192,200,208),Color.new(56,23,4)])
    else
      textpos.push([_INTL("None"),16,346,0,Color.new(192,200,208),Color.new(56,23,4)])
    end
    # Write the gender symbol
    if @pokemon.male?
      textpos.push([_INTL("♂"),178,56,0,Color.new(68,162,234),Color.new(47,40,49)])
    elsif @pokemon.female?
      textpos.push([_INTL("♀"),178,56,0,Color.new(239,79,76),Color.new(91,26,6)])
    end
    # Draw all text
  ...
  def drawPageOne
    ...
    base   = Color.new(248,248,248)
    shadow = Color.new(56,23,4)
    dexNumBase   = (@pokemon.shiny?) ? Color.new(216,147,36) : Color.new(156,210,135)
    dexNumShadow = (@pokemon.shiny?) ? Color.new(98,24,4) : Color.new(56,23,4)
    ...
    # Write various bits of text
    textpos = [
       [_INTL("Dex No."),258,58,0,base,shadow],
       [_INTL("Species"),258,96,0,base,shadow],
       [@pokemon.speciesName,435,100,2,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Type"),258,132,0,base,shadow],
       [_INTL("OT"),258,170,0,base,shadow],
       [_INTL("ID No."),258,208,0,base,shadow],
    ]
    ...
    if dexnum<=0
      textpos.push(["???",435,64,2,dexNumBase,dexNumShadow])
    else
      dexnum -= 1 if dexnumshift
      textpos.push([sprintf("%03d",dexnum),435,64,2,dexNumBase,dexNumShadow])
    end
    # Write Original Trainer's name and ID number
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("RENTAL"),435,172,2,Color.new(156,210,135),Color.new(56,23,4)])
      textpos.push(["?????",435,172,2,Color.new(156,210,135),Color.new(56,23,4)])
    else
      ownerbase   = Color.new(156,210,135)
      ownershadow = Color.new(56,23,4)
      case @pokemon.owner.gender
      when 0
        ownerbase = Color.new(68, 162, 234)
        ownershadow = Color.new(47, 40, 49)
      when 1
        ownerbase = Color.new(239, 79, 76)
        ownershadow = Color.new(91, 26, 6)
      end
      textpos.push([@pokemon.owner.name,435,172,2,ownerbase,ownershadow])
      textpos.push([sprintf("%05d",@pokemon.owner.public_id),435,210,2,Color.new(156,210,135),Color.new(56,23,4)])
    end
    ...
    if @pokemon.shadowPokemon?
      ...
      memo = sprintf("<c3=F3F2F1,381704>%s\n",heartmessage)
      drawFormattedTextEx(overlay,234,304,264,memo)
    else
      endexp = @pokemon.growth_rate.minimum_exp_for_level(@pokemon.level + 1)
      textpos.push([_INTL("Exp. Points"),238,248,0,base,shadow])
      textpos.push([@pokemon.exp.to_s_formatted,488,282,1,Color.new(156,210,135),Color.new(56,23,4)])
      textpos.push([_INTL("To Next Lv."),238,298,0,base,shadow])
      textpos.push([(endexp-@pokemon.exp).to_s_formatted,488,330,1,Color.new(156,210,135),Color.new(56,23,4)])
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw Pokémon type(s)
    ...
    if @pokemon.type1==@pokemon.type2
      overlay.blt(402,144,@typebitmap.bitmap,type1rect)
    else
      overlay.blt(370,144,@typebitmap.bitmap,type1rect)
      overlay.blt(436,144,@typebitmap.bitmap,type2rect)
    end
    # Draw Exp bar
  ...
  def drawPageOneEgg
    ...
    if @pokemon.timeReceived
      ...
      year  = @pokemon.timeReceived.year-40
      memo += _INTL("<c3=F3F2F1,381704>{1} {2}, {3}\n",date,month,year)
    end
    # Write map name egg was received on
    mapname = pbGetMapNameFromId(@pokemon.obtain_map)
    mapname = @pokemon.obtain_text if @pokemon.obtain_text && !@pokemon.obtain_text.empty?
    if mapname && mapname != ""
      memo += _INTL("<c3=F3F2F1,381704>A mysterious Pokémon Egg received from <c3=F83820,E09890>{1}<c3=F3F2F1,381704>.\n",mapname)
    else
      memo += _INTL("<c3=F3F2F1,381704>A mysterious Pokémon Egg.\n",mapname)
    end
    memo += "\n" # Empty line
    # Write Egg Watch blurb
    memo += _INTL("<c3=F3F2F1,381704>\"The Egg Watch\"\n")
  ...
  def drawPageTwo
    ...
    if showNature
      ...
      memo += _INTL("<c3=7BCE5A,413410>{1}<c3=F3F2F1,381704> nature.\n",natureName)
    end
    # Write date received
    if @pokemon.timeReceived
      date  = @pokemon.timeReceived.day
    ...
    memo += sprintf("<c3=EF4F4C,5A1906>%s\n",mapname)
  ...
  def drawPageThree
    ...
    if !@pokemon.shadowPokemon? || @pokemon.heartStage > 3
      @pokemon.nature_for_stats.stat_changes.each do |change|
        statshadows[change[0]] = Color.new(90,25,6) if change[1] > 0
        statshadows[change[0]] = Color.new(60,60,43) if change[1] < 0
      end
    end
    # Write various bits of text
    textpos = [
       [_INTL("HP"),292,64,2,base,statshadows[:HP]],
       [sprintf("%d/%d",@pokemon.hp,@pokemon.totalhp),462,70,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Attack"),248,110,0,base,statshadows[:ATTACK]],
       [sprintf("%d",@pokemon.attack),456,114,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Defense"),248,146,0,base,statshadows[:DEFENSE]],
       [sprintf("%d",@pokemon.defense),456,146,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Sp. Atk"),248,178,0,base,statshadows[:SPECIAL_ATTACK]],
       [sprintf("%d",@pokemon.spatk),456,184,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Sp. Def"),248,212,0,base,statshadows[:SPECIAL_DEFENSE]],
       [sprintf("%d",@pokemon.spdef),456,218,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Speed"),248,246,0,base,statshadows[:SPEED]],
       [sprintf("%d",@pokemon.speed),456,250,1,Color.new(156,210,135),Color.new(56,23,4)],
       [_INTL("Ability"),234,278,0,base,shadow]
    ]
    # Draw ability name and description
    ability = @pokemon.ability
    if ability
      textpos.push([ability.name,374,278,0,Color.new(123,206,90),Color.new(56,23,4)])
      drawTextEx(overlay,224,320,282,2,ability.description,Color.new(243,147,79),Color.new(56,23,4))
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw HP bar
    if @pokemon.hp>0
      ...
      imagepos = [
         ["Graphics/Pictures/Summary/overlay_hp",369,110,0,hpzone*6,w,6]
      ]
      pbDrawImagePositions(overlay,imagepos)
    end
  end
  ...
  def drawPageFour
    ...
    moveShadow = Color.new(56,23,4)
    ppBase   = [moveBase,                # More than 1/2 of total PP
                Color.new(247,213,76),    # 1/2 of total PP or less
                Color.new(247,150,81),   # 1/4 of total PP or less
                Color.new(239,79,76)]    # Zero PP
    ppShadow = [moveShadow,             # More than 1/2 of total PP
                Color.new(86,34,6),   # 1/2 of total PP or less
                Color.new(86,34,6),   # 1/4 of total PP or less
                Color.new(99,25,6)]   # Zero PP
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    imagepos = [["Graphics/Pictures/category", 170, 122, 0, selected_move.category * 28, 64, 28]]
    pbDrawImagePositions(overlay, imagepos)
...
# Made compatable for v19 by Voltseon
# =====================================================================
def drawPageFive
  overlay = @sprites["overlay"].bitmap
   base   = Color.new(248,248,248)
   ...
   textpos = [
    [_INTL("EV"),evColumn,56,2,base,shadow],
    [_INTL("IV"),ivColumn,56,2,base,shadow],
    [_INTL("HP"),textColumn,86,2,base,shadow],
    [sprintf("%d",@pokemon.ev[:HP]),evColumn,86,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:HP]),ivColumn,86,2,base,shadow],
    [_INTL("Attack"),textColumn,118,2,base,statshadows[:ATTACK]],
    [sprintf("%d",@pokemon.ev[:ATTACK]),evColumn,118,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:ATTACK]),ivColumn,118,2,base,shadow],
    [_INTL("Defense"),textColumn,150,2,base,statshadows[:DEFENSE]],
    [sprintf("%d",@pokemon.ev[:DEFENSE]),evColumn,150,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:DEFENSE]),ivColumn,150,2,base,shadow],
    [_INTL("Sp. Atk"),textColumn,182,2,base,statshadows[:SPECIAL_ATTACK]],
    [sprintf("%d",@pokemon.ev[:SPECIAL_ATTACK]),evColumn,182,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:SPECIAL_ATTACK]),ivColumn,182,2,base,shadow],
    [_INTL("Sp. Def"),textColumn,214,2,base,statshadows[:SPECIAL_DEFENSE]],
    [sprintf("%d",@pokemon.ev[:SPECIAL_DEFENSE]),evColumn,214,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:SPECIAL_DEFENSE]),ivColumn,214,2,base,shadow],
    [_INTL("Speed"),textColumn,246,2,base,statshadows[:SPEED]],
    [sprintf("%d",@pokemon.ev[:SPEED]),evColumn,246,2,base,shadow],
    [sprintf("%d",@pokemon.iv[:SPEED]),ivColumn,246,2,base,shadow],
    [_INTL("Ability"),234,278,0,base,shadow],
   ]
   # Draw ability name and description
    ability = @pokemon.ability
  ...
  def pbOptions
    ...
    if !@pokemon.egg?
      ...
      commands[cmdPokedex = commands.length]  = _INTL("View The Book of Ethos") if $Trainer.has_pokedex
    end

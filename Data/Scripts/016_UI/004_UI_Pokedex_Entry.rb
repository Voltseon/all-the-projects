...
class PokemonPokedexInfo_Scene
  ...
  def drawPageInfo
    ...
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(51, 49, 49)
    shadow = Color.new(175, 172, 170)
    imagepos = []
    if @brief
    ...
    textpos = [
       [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
          246, 36, 0, Color.new(51, 49, 49), Color.new(175, 172, 170)],
       [_INTL("Height"), 314, 152, 0, base, shadow],
       [_INTL("Weight"), 314, 184, 0, base, shadow]
    ]
    if $Trainer.owned?(@species)
      # Write the category
      textpos.push([_INTL("{1} Pokémon", species_data.category), 246, 70, 0, base, shadow])
    ...
    # Write the evolution method 
    myEvolutions = species_data.get_evolutions
    if myEvolutions[0]
      case myEvolutions[0][1]
      when :Level then evolutionMethod = "at level"
      when :Ninjask then evolutionMethod = "at level"
      when :Happiness then evolutionMethod = "with happiness"
      when :HappinessNight then evolutionMethod = "with happiness"
      when :Location then evolutionMethod = "with level up at"
      when :Item then evolutionMethod = "with a"
      when :HasMove then evolutionMethod = "while knowing"
      end
    ...
    evolutionMethodX = 40 
    evolutionMethodY = 340
    if !$Trainer.owned?(@species) && $PokemonSystem.difficulty > 0
      textpos.push([_INTL("????????????"), evolutionMethodX, evolutionMethodY, 0, base, shadow])
    elsif !myEvolutions[0]
      textpos.push([_INTL("Does not evolve."), evolutionMethodX, evolutionMethodY, 0, base, shadow])
    elsif !evolutionParam
      textpos.push([_INTL("Evolves {1}",evolutionMethod), evolutionMethodX, evolutionMethodY, 0, base, shadow])
    else
      textpos.push([_INTL("Evolves {1} {2}",evolutionMethod, evolutionParam), evolutionMethodX, evolutionMethodY, 0, base, shadow])
    end
    # Draw all text
  ...
  def drawPageArea
    ...
    pointcolorhl = Color.new(196, 196, 196)
    ...
    for j in 0...points.length
      if points[j]
        ...
        if j-mapwidth<0 || !points[j-mapwidth]
          @sprites["areahighlight"].bitmap.fill_rect(x,y-1,sqwidth,1,pointcolorhl)
        end
        if j+mapwidth>=points.length || !points[j+mapwidth]
          @sprites["areahighlight"].bitmap.fill_rect(x,y+sqheight,sqwidth,1,pointcolorhl)
        end
        if j%mapwidth==0 || !points[j-1]
          @sprites["areahighlight"].bitmap.fill_rect(x-1,y,1,sqheight,pointcolorhl)
        end
        if (j+1)%mapwidth==0 || !points[j+1]
          @sprites["areahighlight"].bitmap.fill_rect(x+sqwidth,y,1,sqheight,pointcolorhl)
        end
      end
    end
    # Set the text

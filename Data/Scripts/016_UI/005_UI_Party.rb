  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if !@pokemon.egg?
        ...
        # Draw gender symbol
        if @pokemon.male?
          textpos.push([_INTL("♂"),224,10,0,Color.new(68,162,234),Color.new(47,40,49)])
        elsif @pokemon.female?
          textpos.push([_INTL("♀"),224,10,0,Color.new(239,79,76),Color.new(91,26,6)])
        end
        # Draw shiny icon
        if @pokemon.shiny?
      ...
      if @text && @text.length>0
        pbSetSystemFont(@overlaysprite.bitmap)
        posX = 166
        posX -= 20 if @text == "NOT ABLE"
        pbDrawTextPositions(@overlaysprite.bitmap,[
           [@text,posX,52,0,basecolor,shadowcolor]
        ])
      end
    end
    @refreshing = false

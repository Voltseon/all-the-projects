...
class PokemonPartyPanel < Sprite
  ...
  def draw_shiny_icon
    if @pokemon.shadowPokemon?
      yoffset = (@active) ? -4 : -18
      pbDrawImagePositions(@overlaysprite.bitmap,
                         [["Graphics/Pictures/shadow", 38, 48+yoffset, 0, 0, 16, 16]])
    end
  ...
  def draw_annotation
    ...
    if @active
      pbDrawTextPositions(@overlaysprite.bitmap,
        [[@text, 48, 68, 0, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    else
      pbDrawTextPositions(@overlaysprite.bitmap,
                          [[@text, 160, 12, 0, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    end
  end
...
class PokemonParty_Scene
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    @sprites["storagetext"].shadowColor = Color.new(57, 41, 104)
  ...
  def pbSwitchBegin(oldid, newid)
    ...
    timeTaken.times do
      oldsprite.x += (oldid < 1) ? -distancePerFrame : distancePerFrame
      newsprite.x += (newid < 1) ? -distancePerFrame : distancePerFrame
      Graphics.update
      Input.update
      self.update
    end
      ...
      if !pkmn.egg?
        ...
        pkmn.moves.each_with_index do |move, i|
          next if !HiddenMoveHandlers.hasHandler(move.id) &&
                  ![:MILKDRINK, :SOFTBOILED,:SHADOWGEYSER].include?(move.id)
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          loop do
            ...
            if party_idx == old_party_idx
              ...
              pbDisplay(_INTL("{1} can't be used on an Egg!", movename))
            elsif newpkmn.fainted? || newpkmn.shadowPokemon?
              pbDisplay(_INTL("{1} can't be used on that Pokémon.", movename))
            elsif shadowCheckSpecific(newpkmn)
              pbDisplay(_INTL("{1} is not capable of closing its heart.", movename))
            else
              change_form_animation("SHADOW", party_idx, newpkmn.form)
              newpkmn.makeShadow
              newpkmn.adjustHeart(100-newpkmn.heart_gauge)
              newpkmn.update_shadow_moves(true)
              @scene.pbDisplay(_INTL("{1} closed its heart.", newpkmn.name))
              pbRefresh
            end
            break if pkmn.shadowPokemon?
          end
          @scene.pbSelect(old_party_idx)
          pbRefresh
...
MenuHandlers.add(:party_menu, :moves, {
  "name"      => _INTL("Moves"),
  "order"     => 35,
  "condition" => proc { |screen, party, party_idx| next !party[party_idx].egg? || $player.metronome },
  "effect"    => proc { |screen, party, party_idx|
    if $player.metronome
      pbMessage(_INTL("Only Metronome is allowed!"))
      next
    end
    pkmn = party[party_idx]
    commands = []
    commands.push("Teach") if screen.scene.pbCheckTM($bag,pkmn)
    commands.push("Relearn") if pkmn.can_relearn_move?
    commands.push("Forget") if pkmn.moves.length > 1
    commands.push("Cancel")
    command = screen.scene.pbShowCommands(_INTL("Do what with {1}'s moveset?",pkmn.name),commands)
    case commands[command]
    when "Teach"
      item = screen.scene.pbUseTM($bag,pkmn) {
        screen.scene.pbSetHelpText(($player.party.length>1) ? _INTL("Choose a Pokémon.") : _INTL("Choose Pokémon or cancel."))
      }
      if item
        screen.pbUseItemOnPokemon(item, pkmn, screen)
        screen.pbRefreshSingle(party_idx)
      end
    when "Relearn"
      pbRelearnMoveScreen(pkmn)
    when "Forget"
      moveindex = screen.pbChooseMoveF(pkmn, _INTL("Forget which move?"),0)
      if moveindex >= 0
        movename = pkmn.moves[moveindex].name
        if screen..pbConfirmMessageSerious(_INTL("Are you sure you want {1} to forget how to use {2}?", pkmn.name, movename))
          pkmn.forget_move_at_index(moveindex)
          screen.pbMessage(_INTL("1, 2, and...\\wt[16] ...\\wt[16] ... Poof!\\se[Battle ball drop]\1"))
          screen..pbMessage(_INTL("{1} forgot how to use {2}.", pkmn.name, movename))
          screen.pbRefreshSingle(party_idx)
        end
      end
    end

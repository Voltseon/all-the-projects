...
class PokemonPhoneScene
  ...
  def start
    ...
    @sprites["header"] = Window_UnformattedTextPokemon.newWithSize(
      _INTL("Phone"), 8, -8, 128, 64, @viewport
    )
    @sprites["header"].windowskin = nil
    @sprites["header"].baseColor   = TEXT_BASE_COLOR
    @sprites["header"].shadowColor = Color.new(59, 63, 81)
    mapname = (@trainers[0][2]) ? pbGetMapNameFromId(@trainers[0][2]) : ""
    @sprites["bottom"] = Window_AdvancedTextPokemon.newWithSize(
      "", 162, Graphics.height - 60, Graphics.width - 158, 64, @viewport
    )
    @sprites["bottom"].windowskin = nil
    @sprites["bottom"].baseColor = TEXT_BASE_COLOR
    @sprites["bottom"].shadowColor = TEXT_SHADOW_COLOR
    ...
    @sprites["info"].baseColor = TEXT_BASE_COLOR
    @sprites["info"].shadowColor = TEXT_SHADOW_COLOR
    ...
    if $player.has_pdaplus
      ...
      @sprites["bg_overlay"].setBitmap("Graphics/Pictures/phoneplusbg")
    else
      @sprites["bg"].setBitmap("Graphics/Pictures/Pokegear/bg")
      @sprites["bg_overlay"].setBitmap("Graphics/Pictures/phonebg")
    end
    @sprites["icon"] = IconSprite.new(70, 102, @viewport)
    filename = sprintf("Graphics/Pictures/phone_%s", @trainers[0][0])
    ...
    pbActivateWindow(@sprites, "list") {
      ...
      loop do
        ...
        if @sprites["list"].index != oldindex
          ...
          filename = sprintf("Graphics/Pictures/phone_%s", @trainers[@sprites["list"].index][0])
          @sprites["icon"].setBitmap(filename)
          @sprites["icon"].x        = 48
          @sprites["icon"].y        = 90
        ...
        if Input.trigger?(Input::BACK)
          ...
          pbCallTrainer(@trainers[index][0], @trainers[index][1], commands[index] + " phone") if index >= 0
        end
        if @sprites["bg"].y > -1
          @sprites["bg"].y = -128
        else
    ...
    @viewport.dispose
    $game_temp.in_phone = false
  end
end

class Game_Temp
  attr_accessor :in_phone

  def in_phone
    @in_phone = false if !@in_phone
    return @in_phone
  end
end

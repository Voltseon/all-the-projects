module GameData
  class Species
    ...
    def moves(real = false)
      return [[1, :METRONOME]] if $player&.metronome
      return @moves if real
      return ($player) ? $player.species_learnsets[@id] : @moves
    end
    ...
    def compatible_with_move?(move_id)
      ...
      return false if $player.metronome && move_id != :METRONOME
      unless @tutor_moves.length < 5
        case move_id
        when :RECOVER then return true
        when :GASTROACID,:FIRELASH,:DUALCHOP,:DRAININGKISS,:MOONBLAST
          return @types.include?(move_data.type)
        when :NIGHTSHADE
          return @types.include?(move_data.type) || @types.include?(:DARK) || @types.include?(:PSYCHIC)
        when :EVOBURST,:EVOTHRUST
          return [:EEVEE,:FLAREON,:JOLTEON,:VAPOREON,:ESPEON,:UMBREON,:GLACEON,:LEAFEON,:SYLVEON].include?(@species)
        end
      end
      return move_data && (@tutor_moves.include?(move_data.id) || @egg_moves.include?(move_data.id) || @moves.include?(move_data.id))
    end
  end
end

module GameData
  class Item
    ...
    def description
      return pbGetMessageFromHash(MessageTypes::MoveDescriptions, GameData::Move.get(self.move).description) if @move
      return pbGetMessageFromHash(MessageTypes::ItemDescriptions, @real_description)
    end

    def move
      return nil if !@move
      tm_number = @id.to_s.gsub("TM","").to_i
      return @move if tm_number == 0 || tm_number > 100
      return $player.tms[tm_number-1] if self.is_TM?
      return @move
    end
    ...
    def is_important?
      return true if is_key_item? || is_HM? || is_TM? || is_mega_stone? || is_memory?
      return false
    end

    def can_hold?;           return !(is_important? && !is_mega_stone? && !is_memory?); end

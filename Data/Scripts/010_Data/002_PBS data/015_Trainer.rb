module GameData
  class Trainer
    ...
    $data = [{}] * 4
    DATA_FILENAMES = ["trainers_easy.dat", "trainers.dat", "trainers_hard.dat", "trainers_insane.dat"]
    ...
    def self.load
      DATA_FILENAMES.each_with_index { |data, i| $data[i] = load_data("Data/#{data}") }
    end

    def self.save(index)
      self::save_data($data[index], "Data/#{DATA_FILENAMES[index]}")
    end

    def self.clearData
      4.times { |i| $data[i].clear }
    end

    def self.register(hash, index)
      $data[index][hash[:id]] = $data[index][hash[:id_number]] = self.new(hash)
    end

    # Returns the array of keys for the data.
    # @return [Array]
    def self.keys
      return $data[self::getDifficulty].keys
    end

    # Yields all data in order of their id_number.
    def self.each
      sorted_keys = $data[self::getDifficulty].keys
      sorted_keys.each { |key| yield $data[self::getDifficulty][key] if !key.is_a?(Integer) }
    end

    def self.count
      return $data[self::getDifficulty].length / 2
    end

    def self.getDifficulty
      return $player.difficulty if $player && !$player.difficulty.nil?
      return 1
    end
    ...
    def self.exists?(tr_type, tr_name, tr_version = 0)
      ...
      return !$data[self::getDifficulty][key].nil?
    end
    ...
    def self.get(tr_type, tr_name, tr_version = 0)
      ...
      validate tr_name => [String]
      key = [tr_type.to_sym, tr_name, tr_version]
      raise "Unknown trainer #{tr_type} #{tr_name} #{tr_version}." unless $data[self::getDifficulty].has_key?(key)
      return $data[self::getDifficulty][key]
    end
    ...
    def self.try_get(tr_type, tr_name, tr_version = 0)
      ...
      validate tr_name => [String]
      key = [tr_type.to_sym, tr_name, tr_version]
      return ($data[self::getDifficulty].has_key?(key)) ? $data[self::getDifficulty][key] : nil
    end
    ...
    def to_trainer
      ...
      echoln $player.randomizer_trainer
      if $player.randomizer_trainer
        my_version = @version || 0
        my_pokemon = $player.trainer_pokemon[[trainer.trainer_type, tr_name, my_version]] || @pokemon
      else
        my_pokemon = @pokemon
      end
      my_pokemon.each do |pkmn_data|
        ...
        if pkmn_data[:moves] && pkmn_data[:moves].length > 0 && !$player.metronome

module GameData
  class Move
    ...
    def physical?
      ...
      return @category == 0 if !$player.nospecialphysicalsplit
      return GameData::Type.get(@type).physical?
    end

    def special?
      return false if @base_damage == 0
      return @category == 1 if !$player.nospecialphysicalsplit
    ...
    def display_type(pkmn, move = nil)
      ...
      when "TypeIsUserFirstType", "TypeDependsOnUserIncreasesSpAtk", "TypeDependsOnUserIncreasesAttack"
        return pkmn.types[0]
      end
      return @type
    end
      ...
      when "TypeAndPowerDependOnUserBerry"
        ...
        if pkmn.hasItem?
          damage_array.each do |dmg, items|
            ...
            ret = dmg
            ret += 20 if Settings.pbGetGeneration >= 6
            return ret
          end
        end
...
=end
    ...
    def convert_move_data(data)
      ...
      when "003"
        ...
        elsif data[:id] == :DARKVOID && Settings.pbGetGeneration >= 7
      ...
      when "048"
        if data[:id] == :SWEETSCENT && Settings.pbGetGeneration >= 6
      ...
      when "070"
        ...
        elsif data[:id] == :SHEERCOLD && Settings.pbGetGeneration >= 7
          new_code = "OHKOIce"
        else
          new_code = "OHKO"
        end

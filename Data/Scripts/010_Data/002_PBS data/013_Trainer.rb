  ...
  class Trainer
    ...
    def to_trainer
      ...
      @pokemon.each do |pkmn_data|
        ...
        newLevel = pkmn_data[:level] * levelModif
        pkmn = Pokemon.new(species, newLevel.round, trainer, false)
        ...
        GameData::Stat.each_main do |s|
          if pkmn_data[:iv]
            ...
            pkmn.iv[s.id] = [newLevel.round / 2, Pokemon::IV_STAT_LIMIT].min
          end
          if pkmn_data[:ev]
            pkmn.ev[s.id] = pkmn_data[:ev][s.id]
          else
            pkmn.ev[s.id] = [newLevel.round * 3 / 2, Pokemon::EV_LIMIT / 6].min
          end
        end

...
class Player < Trainer
  ...
  # @return [Planet] the planet the player is currently on
  attr_accessor :planet
  # @return [Array<Symbol>] the planets the player has visited
  attr_accessor :visited_planets
  # @return [Integer] the suit the player picked
  attr_accessor :suit
  # @return [Hash<Symbol, Integer>] the player's inventory
  attr_accessor :items
  # @return [Integer] the player's carry capacity
  attr_writer   :carry_capacity
  # @return [Hash<Symbol, Integer>] the storage's inventory
  attr_accessor :storage
  # @return [Integer] the storage's capacity
  attr_writer   :storage_capacity
  # @return [Array<Symbol>] the items that the player has researched
  attr_accessor :researched_items
  # @return [Array<Symbol>] the crafting recipes that the player has
  ...
  # @return [Planet] the planet the player is currently on
  def planet
    return @planet || GameData::Planet.get(:SHUTTLE)
  end

  def planet=(value)
    if value.is_a?(Symbol)
      value = GameData::Planet.get(value)
    end
    @planet = value
  ...
  def carry_capacity
    return CARRY_CAPACITIES[@carry_capacity]
  end

  def increase_carry_capacity
    @carry_capacity = [@carry_capacity + 1, CARRY_CAPACITIES.length - 1].min
  end

  def increase_storage_capacity
    @storage_capacity = [@storage_capacity + 1, STORAGE_CAPACITIES.length - 1].min
  end

  def storage_capacity
    return STORAGE_CAPACITIES[@storage_capacity || 0]
  end

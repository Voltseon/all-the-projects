...
class Player < Trainer
  ...
  # @return [Boolean] whether the PDA+ has been obtained
  attr_accessor :has_pdaplus
  # @return [Boolean] whether the pokeradar has been obtained
  attr_accessor :has_pokeradar
  ...
  # @return [String] the name of the save file
  ...
  def money
    @money = @money.encrypt if @money.is_a?(Integer)
    return @money
  end
  ...
  def money=(value)
    value = value.decrypt if value.is_a?(String)
    validate value => Integer
    @money = value.clamp(0, Settings::MAX_MONEY).encrypt
  end
  ...
  def initialize_challenges
    if @challenges.nil?
      ...
      MenuHandlers.each_available(:challenge) do |option, hash, name|
        @challenges[option] = hash["default"]
      end
    else
      MenuHandlers.each_available(:challenge) do |option, hash, name|
        next if @challenges.has_key?(option) && !@challenges[option].nil?
        @challenges[option] = hash["default"]
      end
    end
  end

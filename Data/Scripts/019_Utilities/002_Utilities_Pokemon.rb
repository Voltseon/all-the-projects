...
def pbAffectMurphysLaw(pkmn)
  # Ensure murphy's law
  if $player.murphyslaw
    # Minimize stats
    GameData::Stat.each_main { |s| pkmn.ev[s.id] = 0; pkmn.iv[s.id] = 0 }
    # Change nature
    lowest_offensive = (pkmn.species_data.base_stats[:SPECIAL_ATTACK] < pkmn.species_data.base_stats[:ATTACK] ? :SPECIAL_ATTACK : :ATTACK)
    if pkmn.species_data.base_stats[:SPEED] < 75
      pkmn.nature = (lowest_offensive == :SPECIAL_ATTACK ? :BOLD : :CAREFUL)
    else
      if (pkmn.species_data.base_stats[:ATTACK] - pkmn.species_data.base_stats[:SPECIAL_ATTACK]).abs <= 10
        pkmn.nature = :QUIET
      else
        pkmn.nature = (lowest_offensive == :ATTACK ? :ADAMANT : :MODEST)
      end
    end
    # Recalculate stats
    pkmn.calc_stats
...
def pbAddPokemon(pkmn, level = 1, see_form = true)
  ...
  if Settings.pbGetGeneration >= 7 && see_form && !was_owned && $player.has_pokedex

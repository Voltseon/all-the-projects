...
def pbNoticePlayer(event,should_wait=true)
  if !pbFacingEachOther(event, $game_player)
    pbExclaim(event)
  end
  pbTurnTowardEvent($game_player, event)
  pbMoveTowardPlayer(event,should_wait)
end
...
def pbSaveScreenshot
  filestart = "./Graphics/Pictures/lastsave#{$save_suffix}.png"
  Graphics.screenshot(filestart)
end

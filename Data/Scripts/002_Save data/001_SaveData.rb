...
module SaveData
  ...
  def self.exists?
    return File.file?(FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata"))
  end
  ...
  def self.delete_file
    File.delete(FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata"))
    File.delete(FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata") + ".bak") if File.file?(FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata") + ".bak")
  end
  ...
  def self.move_old_windows_save
    return if File.file?(FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata"))
    ...
    File.move(old_file, FILE_PATH.gsub(".rxdata","#{$save_suffix}.rxdata"))
  end

  def self.load_last_save
    data = ""
    file_path = System.data_directory + "LastSave.rxdata"
    return "" unless File.file?(file_path)
    File.open(file_path) do |file|
      data = Marshal.load(file)
    end
    return "" if (data == " debug" && !$DEBUG)
    return data
  end

  def self.save_last_save
    File.open(System.data_directory + "LastSave.rxdata", "wb") { |file| Marshal.dump($save_suffix, file) }
  end
end

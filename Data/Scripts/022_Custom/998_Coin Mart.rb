def pbCasinoPokemon
  pbCallBub
  options = []
  prices = [250, 500, 1000, 2500, 5000, 5000, 9999]
  $player.special_pokemon[:CASINO].each do |pokemon|
    options.push("#{GameData::Species.get(pokemon).name} - #{prices[options.length].to_s_formatted} coins")
  end
  options.push(_INTL("Cancel"))
  result = pbMessage(_INTL("\\CN\\rWhich Pokémon would you like to buy?"), options, options.length)
  return false if result == options.length - 1
  pbSet(1, result + 1)
  pbSet(2, prices[result])
  return true
end
...
#===============================================================================
class PokemonMartAdapter_Coins
  def getCoins
    return $player.coins
  end

  def getCoinsString
    return ($player.coins).to_s
  end
  ...
  def getPrice(item, selling = false)
    ...
    return 5 if item == :GIMMIGHOULCOIN
    return 25 if GameData::Item.get(item).sell_price == 0 && selling
    return 50 if GameData::Item.get(item).price == 0
    return GameData::Item.get(item).sell_price / 10 if selling
    return GameData::Item.get(item).price / 10
  end
...
# Pokémon Mart
#===============================================================================
class Window_PokemonMart_Coins < Window_DrawableCommand
...
class PokemonMart_Scene_Coins
  ...
  def pbRefresh
    ...
    @sprites["coinswindow"].text = _INTL("Coins:\r\n<r>{1}", @adapter.getCoinsString)
  end
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    winAdapter = buying ? BuyAdapter_Coins.new(adapter) : SellAdapter_Coins.new(adapter)
    @sprites["itemwindow"] = Window_PokemonMart_Coins.new(
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.text = _INTL("x{1}<r> {2} coins", curnumber, (curnumber * itemprice).to_s_formatted)
...
#
#===============================================================================
class PokemonMartScreen_Coins
  def initialize(scene, stock)
    @scene = scene
    @stock = stock
    @adapter = PokemonMartAdapter_Coins.new
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getCoins < price
        pbDisplayPaused(_INTL("You don't have enough coins."))
        next
      end
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("So you want {1}?\nIt'll be {2} coins. All right?",
                            itemname, price.to_s_formatted))
        quantity = 1
      else
        maxafford = (price <= 0) ? Settings::BAG_MAX_PER_SLOT : @adapter.getCoins / price
      ...
      if added == quantity
        $stats.coins_spent_at_marts += price
        $stats.mart_items_bought += quantity
        @adapter.setCoins(@adapter.getCoins - price)
  ...
  def pbSellScreen
    ...
    loop do
      ...
      price *= qty
      if pbConfirm(_INTL("I can pay {1} coins.\nWould that be OK?", price.to_s_formatted))
        old_coins = @adapter.getCoins
        @adapter.setCoins(@adapter.getCoins + price)
        $stats.coins_earned_at_marts += @adapter.getCoins - old_coins
        qty.times { @adapter.removeItem(item) }
        sold_item_name = (qty > 1) ? itemnameplural : itemname
        pbDisplayPaused(_INTL("You turned over the {1} and got {2}coins.",
                              sold_item_name, price.to_s_formatted)) { pbSEPlay("mart_buy") }
        @scene.pbRefresh
      end
      @scene.pbHideCoins
    end
...
#
#===============================================================================
def pbPokemonMart_Coins(stock, speech = nil, cantsell = false)
  stock.delete_if { |item| GameData::Item.get(item).is_important? && checkItemAll(item) }
  commands = []
  cmdBuy  = -1
  cmdSell = -1
  ...
  pbCallBub
  cmd = pbMessage(speech || _INTL("\\rWelcome! How may I help you?"), commands, cmdQuit + 1)
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      scene = PokemonMart_Scene_Coins.new
      screen = PokemonMartScreen_Coins.new(scene, stock)
      screen.pbBuyScreen
      ...
      pbCallBub
      pbMessage(_INTL("\\rDo come again!"))
      break
    end
    pbCallBub
    cmd = pbMessage(_INTL("\\rIs there anything else I can do for you?"), commands, cmdQuit + 1)
  end

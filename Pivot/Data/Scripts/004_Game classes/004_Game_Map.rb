...
class Game_Map
  ...
  def bgm_name
    ...
    audiopack = AudioPack.get($PokemonGlobal.audio_pack)
    return @map.bgm.name if !audiopack || audiopack.name == ""
    bgmname = AudioPack.parse(audiopack.get_arena(Arena.get_by_mapid(@map_id)), audiopack)
    return bgmname
  end
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  def autoplayAsCue
    #if $game_system.playing_bgm || $game_system.playing_bgs
    #  pbBGMFade(0.2) if $game_system.playing_bgm
    #  pbBGSFade(0.2) if $game_system.playing_bgs
    #  pbWait(12)
    #end
  ...
  def passable?(x, y, d, self_event = nil)
    ...
    events.each_value do |event|
      ...
      return false if event.name[/partner/i] && !event.name[/partner_follower/i]
    ...
    char = AI.ais[self_event&.id]&.character if AI.ais[self_event&.id].is_a?(AI)
    [2, 1, 0].each do |i|
      tile_id = data[x, y, i]
      terrain = GameData::TerrainTag.try_get(@terrain_tags[tile_id])
      can_stay_on_water = true
      if char
        can_stay_on_water = char.movement_type == :FLYING
        if char.movement_type == :FLYING
          return true if terrain.can_surf || terrain.can_phase
        elsif char.movement_type == :PHASE
          return true if terrain.can_phase
        end
      end
      # If already on water, only allow movement to another water tile
      if self_event && terrain.can_surf_freely && can_stay_on_water
        [2, 1, 0].each do |j|
  ...
  def playerPassable?(x, y, d, self_event = nil)
    ...
    [2, 1, 0].each do |i|
      ...
      passage = @passages[tile_id]
      if terrain
        if $player.character.movement_type == :FLYING
          return true if terrain.can_surf || terrain.can_phase
        elsif $player.character.movement_type == :PHASE
          return true if terrain.can_phase
        end

class Game_Character
  ...
  def real_x; return @real_x || 0; end
  def real_y; return @real_y || 0; end

  def real_x=(value); @real_x = value; end
  def real_y=(value); @real_y = value; end

  def always_on_top; return @always_on_top; end

  def moved_this_frame; @moved_this_frame; end
  ...
  def move_speed=(val)
    ...
    self.move_speed_real = ((3*val)*0.9)
  end
  ...
  def fullPattern
    case @direction
    when 1..2 then return self.pattern
    when 3..4 then return self.pattern + 4
    when 6..7 then return self.pattern + 8
    when 8..9 then return self.pattern + 12
    end
    return 0
  end

  def direction
    return (@direction%2==1 ? @direction > 5 ? @direction-1 : @direction+1 : @direction)
  end
  ...
  def directional_offset
    return Game_Character.directional_offset(@direction)
  end

  def self.directional_offset(direction)
    ret = [0,0]
    case direction
  ...
  def character_name
    return (@direction%2==1 ? @character_name+"_diagonal" : @character_name )
  end
  ...
  #=============================================================================
  def screen_x(should_round = true)
    ret = (@real_x.to_f - self.map.display_x) / Game_Map::X_SUBPIXELS
    ret = ret.round if should_round
    ret += @width * Game_Map::TILE_WIDTH / 2
    ret += self.x_offset
    return ret
  end

  def screen_y_ground(should_round = true)
    ret = (@real_y.to_f - self.map.display_y) / Game_Map::Y_SUBPIXELS
    ret = ret.round if should_round
    ret += Game_Map::TILE_HEIGHT
    return ret
  end

  def screen_y(should_round = true)
    ret = screen_y_ground(should_round)
    if jumping?
      if @jump_count > 0
  ...
  def move_generic(dir, turn_enabled = true)
    ...
    if can_move_in_direction?(dir)
      turn_generic(dir)
      @x += (dir == 4 || dir == 7 || dir == 1) ? -1 : (dir == 6 || dir == 3 || dir == 9) ? 1 : 0
      @y += (dir == 8 || dir == 7 || dir == 9) ? -1 : (dir == 2 || dir == 3 || dir == 1) ? 1 : 0
      increase_steps
    else
  ...
  def move_random
    ...
    when 4 then move_upper_left
    when 5 then move_lower_right
    when 6 then move_upper_right
    when 7 then move_lower_left
    end
  end
  ...
  def move_towards_location(xpos, ypos)
    return if moving?
    sx = @x + (@width / 2.0) - xpos
    sy = @y - (@height / 2.0) - ypos
    return if sx == 0 && sy == 0
    angle = sy.to_f/sx.to_f
    if (angle.abs <= 0.5 && angle.abs != Float::INFINITY) || sx == 0 || sy == 0
      if sx.abs > sy.abs
      ...
      if sx > 0
        (sy < 0) ? move_lower_left : move_upper_left
      else
        (sy > 0) ? move_upper_right : move_lower_right
      end
    end
  end

  def move_away_from_location(xpos, ypos)
    return if moving?
    sx = @x + (@width / 2.0) - xpos
    sy = @y - (@height / 2.0) - ypos
    return if sx == 0 && sy == 0
  ...
  def move_towards_event(event)
    return move_towards_location(event.x + (event.width / 2.0), event.y - (event.height / 2.0))
  end

  def move_away_from_event(event)
    return move_away_from_location(event.x + (event.width / 2.0), event.y - (event.height / 2.0))
  end

  def move_toward_player
    move_towards_event($game_player)
  end

  def move_away_from_player
    move_away_from_event($game_player)
  end
  ...
  def move_forward
    ...
    when 7 then move_upper_left(false)
    when 8 then move_up(false)
    when 9 then move_upper_right(false)
    end
  end
  ...
  def move_backward
    ...
    when 7 then move_lower_right(false)
    when 8 then move_down(false)
    when 9 then move_lower_left(false)
    end
  ...
  def jumpForward
    case self.direction
    when 1 then jump(-1, 1)   # lower left
    when 2 then jump(0, 1)    # down
    when 3 then jump(1, 1)    # lower right
    when 4 then jump(-1, 0)   # left
    when 6 then jump(1, 0)    # right
    when 7 then jump(-1, -1)  # upper left
    when 8 then jump(0, -1)   # up
    when 9 then jump(1, -1)   # upper right
    end
  end
  ...
  def turn_toward_event(event, is_ai=false)
    ...
    if angle.abs > 0.5 && angle.abs != Float::INFINITY
      if sx > 0
        (sy > 0) ? turn_upper_left : turn_lower_left
      else
        (sy > 0) ? turn_upper_right : turn_lower_right
      end
    else
      if sx.abs > sy.abs
        if is_ai
  ...
  def turn_toward_player
    turn_toward_event($game_player)
  end

  def turn_away_from_player
    turn_away_from_event($game_player)
  end
...
#    return if @jump_count > 0   # Don't animate if jumping on the spot
    ...
    frames_per_pattern = ((move_speed == 4) ? 6 : Game_Map::REAL_RES_X / (real_speed * 2.0))

...
class GameStats
  ...
  attr_accessor :games_drawn
  attr_accessor :total_take_downs
  attr_accessor :total_faints
  attr_accessor :times_played_characters
  attr_accessor :takedown_record
  attr_accessor :total_damage_dealt
  ...
  def initialize
    ...
    @total_take_downs              = 0
    @total_faints                  = 0
    @times_played_characters       = {}
    @takedown_record               = 0
    @total_damage_dealt            = 0
  end

  def games_lost
    return @games_played-@games_won-@games_drawn
  end

  def times_played_characters
    if @times_played_characters.is_a?(Array)
      # Old save data compatibility
      old_array = @times_played_characters
      @times_played_characters = {}
      Character.each_with_index { |c, i| @times_played_characters[c.internal] = old_array[i] }
    end
    Character.each { |c| @times_played_characters[c.internal] = 0 if !@times_played_characters[c.internal] }
    return @times_played_characters
  end

  def most_played_character
    # Returns Array [character, amount]
    key = self.times_played_characters.key(self.times_played_characters.values.max)
    return ["None", 0] if self.times_played_characters[key] == 0
    return [Character.get(key).name, self.times_played_characters[key]]
  end

  def tdf_ratio
    return @total_faints==0 ? @total_take_downs : @total_take_downs.to_f/@total_faints.to_f
  end
...
module Graphics
  ...
  def self.update
    ...
    $stats.play_time += self.delta_s if $stats
  end
end

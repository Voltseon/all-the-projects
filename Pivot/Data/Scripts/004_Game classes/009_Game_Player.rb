...
class Game_Player < Game_Character
  ...
  def refresh_charset
    ...
    @character_name = "#{$player.character_id}/#{skin}/idle"
  end
  ...
  def center_smooth(x, y)
    ...
    until (self.map.display_x-target[0]).abs < 1 && (self.map.display_y-target[1]).abs < 1
      self.map.display_x = ease_in_out(self.map.display_x, target[0], 0.2)
      self.map.display_y = ease_in_out(self.map.display_y, target[1], 0.2)
      Graphics.update
      $scene.update
      target = [(x * Game_Map::REAL_RES_X) - SCREEN_CENTER_X,(y * Game_Map::REAL_RES_Y) - SCREEN_CENTER_Y]
    end
  ...
  def update_command_new
    ...
    unless pbMapInterpreterRunning? || $game_temp.message_window_showing ||
           $game_temp.in_mini_update || $game_temp.in_menu || $scene.active_hud.aiming || $player.has_state || $game_temp.character_lock || $player.current_hp < 1

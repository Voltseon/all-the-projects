...
module Keys
  ...
  def self.default_controls
    return [
      ...
      ControlConfig.new("Scoreboard", "Tab"),
      #ControlConfig.new("Scroll Down", "Page Down"),
      ControlConfig.new("Guard", "Shift")
    ]
  end 
    ...
    def buttonToKey(button)
      ...
      case button
        ...
        when Input::JUMPUP # A, Q, Page Up
          return $PokemonSystem.game_control_code("Scoreboard")
        when Input::JUMPDOWN # S, Page Down
          return $PokemonSystem.game_control_code("Scroll Down")
        when Input::SPECIAL # F, F5
          return $PokemonSystem.game_control_code("Guard")
...
class Window_PokemonControls < Window_DrawableCommand
  ...
  def control_description(control_action)
    ...
    hash["Scoreboard"   ] = _INTL("Open the scoreboard.")
    hash["Scroll Down" ] = hash[ "Scroll Up"]
    hash["Guard"       ] = _INTL("Open Ready Menu, with registered items and available field moves.")
  ...
  def drawItem(index,count,rect)
    ...
    name = case index-@controls.length
      ...
      when EXIT_EXTRA_INDEX      ; _INTL("Close")
      else                       ; @controls[index].control_action
    end
    width = (Graphics.width/2)-50
    pbDrawShadowText(
      self.contents,(index-@controls.length == DEFAULT_EXTRA_INDEX || index-@controls.length == EXIT_EXTRA_INDEX) ? (Graphics.width/4)+18 : 0,rect.y,width,rect.height,
      name,(index == @index) ? Window_PokemonOption::SEL_NAME_BASE_COLOR : MainMenu::BASE_COLOR,
      (index == @index) ? Window_PokemonOption::SEL_NAME_SHADOW_COLOR : MainMenu::SHADOW_COLOR, (index-@controls.length == DEFAULT_EXTRA_INDEX || index-@controls.length == EXIT_EXTRA_INDEX) ? 1 : 2
    )
    return if index>=@controls.length
    value = _INTL(@controls[index].key_name)
    pbDrawShadowText(
      self.contents,(Graphics.width/2)+38,rect.y,width,rect.height,
      value,(index == @index) ? (@reading_input) ? Window_PokemonOption::SEL_VALUE_BASE_COLOR : Window_PokemonOption::SEL_NAME_BASE_COLOR : MainMenu::BASE_COLOR,
      (index == @index) ? (@reading_input) ? Window_PokemonOption::SEL_VALUE_SHADOW_COLOR : Window_PokemonOption::SEL_NAME_SHADOW_COLOR : MainMenu::SHADOW_COLOR, 0
    )
  end
  ...
  def update
    oldindex=self.index
    Mouse.update
    if Mouse.over_area?(self.x,self.y,self.width,self.height)
      new_index = (Mouse.y-self.y-16)/32
      if new_index != self.index
        self.index = new_index
        if $april_fools
          case self.index
          when 0
            pbAnnounce(:fools_controls_up)
          when 1
            pbAnnounce(:fools_controls_left)
          when 2
            pbAnnounce(:fools_controls_down)
          when 3
            pbAnnounce(:fools_controls_right)
          when 4
            pbAnnounce(:fools_controls_action)
          when 5
            pbAnnounce(:fools_controls_caencel)
          when 6
            pbAnnounce(:fools_controls_menu)
          when 7
            #pbAnnounce(:fools_controls_scoreboard)
          when 8
            pbAnnounce(:fools_controls_guard)
          when 9
            pbAnnounce(:fools_controls_reset)
          when 10
            pbAnnounce(:fools_controls_close)
          end
        end
      end
    else
      self.index = @controls.length+2
    end
    self.index=0 if self.index<0
    super
    do_refresh=self.index!=oldindex
    if self.active && self.index<=@controls.length
      if Input.trigger?(Input::C) || Mouse.trigger?
        if on_default_index?
          pbPlayDecisionSE()
...
class PokemonControls_Scene
  def start_scene
    ...
    addBackgroundOrColoredPlane(@sprites, "bg", "/Menu/bg", Color.new(248, 248, 248), @viewport)
    ...
    @sprites["back"] = ButtonSprite.new(self,"",MainMenu::PATH+"back",MainMenu::PATH+"back_highlight",proc{@disposed = true},0,24,80,@viewport,nil,proc{pbAnnounce(:fools_back) if $april_fools})
    game_controls = $PokemonSystem.game_controls.map{|c| c.clone}
    @sprites["controlwindow"]=Window_PokemonControls.new(
      game_controls,0,@sprites["top_bar"].height+80,Graphics.width,
      Graphics.height-@sprites["top_bar"].height-80
    )
    @sprites["controlwindow"].viewport=@viewport
    @sprites["controlwindow"].visible=true
    @sprites["controlwindow"].setSkin("Graphics/Windowskins/choice 0")
    @changed = false
    pbDeactivateWindows(@sprites)
    update
    @sprites["controlwindow"].reading_input = false
  end
  ...
  def main
    ...
    loop do
      ...
      textpos.push(["Controls",Graphics.width/2+48, 24, 1, MainMenu::BASE_COLOR, MainMenu::SHADOW_COLOR])
      pbDrawTextPositions(@overlay.bitmap, textpos)
      should_refresh_text = @sprites["controlwindow"].index!=last_index
      if @sprites["controlwindow"].reading_input
        @sprites["controlwindow"].refresh
        @sprites["controlwindow"].set_new_input(Keys.detect_key)
        should_refresh_text = true
        @changed = true
      else
        if Input.trigger?(Input::B) || (
          (Input.trigger?(Input::C) || Mouse.press?) && @sprites["controlwindow"].on_exit_index?
        )
          if(
            ...
            if @sprites["controlwindow"].controls.find{|c| c.key_code == 0}
              pbMessage(_INTL("Fill all fields!"))
              should_refresh_text = false
            else

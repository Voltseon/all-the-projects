...
class Window_CharacterEntry < Window_DrawableCommand
  ...
  def initialize(charset, viewport = nil)
    ...
    @othercharset = ""
    super(0, 96, 480, 192)
    self.baseColor = Color.new(254,255,255)
    self.shadowColor = Color.new(219,97,15)
    self.columns = XSIZE
    refresh
  end
  ...
  def pbStartScene(helptext, minlength, maxlength, initialText, subject = 0, pokemon = nil)
    ...
    if USEKEYBOARD
      @sprites["entry"] = Window_TextEntry_Keyboard.new(
        initialText, (Graphics.width/2)-300, 0, 600, 96, helptext, true
      )
      Input.text_input = true
    else
      @sprites["entry"] = Window_TextEntry.new(initialText, 0, 0, 400, 96, helptext, true)
    end
    @sprites["entry"].x = (Graphics.width / 2) - 300
...
class PokemonEntryScene2
  ...
  def pbStartScene(helptext, minlength, maxlength, initialText, subject = 0, pokemon = nil)
      ...
      COLUMNS.times do |y|
        ROWS.times do |x|
          ...
          textPos.push([@@Characters[i][0][pos], 44 + (x * 32), 24 + (y * 38), 2,
                        Color.new(254,255,255), Color.new(219,97,15)])
        end
      end
    ...
    underline_bitmap.fill_rect(2, 2, 22, 4, Color.new(254,255,255))
    underline_bitmap.fill_rect(0, 0, 22, 4, Color.new(219,97,15))
  ...
  def pbDoUpdateOverlay
    ...
    textPositions = [
      [@helptext, 160, 18, false, Color.new(254, 255, 255), Color.new(219,97,15)]
    ]
    chars = @helper.textChars
    x = 166
    chars.each do |ch|
      textPositions.push([ch, x, 54, false, Color.new(254, 255, 255), Color.new(219,97,15)])
      x += 24
    end

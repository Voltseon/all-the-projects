...
module Game
  ...
  def self.load(save_data)
    ...
    self.load_map
    pbAutoplayOnSave
    if $april_fools && rand(100) < 15
      pbAnnounce(:fools_story)
      frame = 0
      # Add a rect bar at the bottom of the screen that slowly fills up until frame is 8560
      ...
      loop do
        Graphics.update
        frame += 1
        overlay.bitmap.fill_rect(0, Graphics.height-20, ((Graphics.width * frame) / 8545), 20, Color.new(255, 255, 255, 255))
        if frame >= 8545
          pbBGMStop
          overlay.dispose
          viewport.dispose
  ...
  def self.load_map
    ...
    magic_number_matches = false
    if !magic_number_matches || $PokemonGlobal.safesave
      if pbMapInterpreterRunning?
        pbMapInterpreter.setup(nil, 0)
      end
      begin
        $map_factory.setup($data_system.start_map_id)
      rescue Errno::ENOENT
        if $DEBUG
          pbMessage(_INTL("Map {1} was not found.", $data_system.start_map_id))
          map = pbWarpToMap
          exit unless map
      ...
      $map_factory.setMapChanged($data_system.start_map_id)
    end

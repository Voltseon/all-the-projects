...
class Player < Trainer
  ...
  def metadata
    return GameData::PlayerMetadata.get(character_ID || 1)
  end
  ...
  def battle_points=(value)
    validate value => Integer
    @battle_points = value.clamp(0, 9999)
  end
  ...
  def initialize(name, trainer_type)
    ...
    while pbCheckIDAvailability(@id) == "taken" || @id > 2147483647
      @id = rand(2**16) | (rand(2**16) << 16)
    end

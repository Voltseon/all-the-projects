...
module RPG
  class Sprite < ::Sprite
    ...
    def damage(value, critical, centered = false, by=0)
      damage_colors = [
        [239, 115, 41],
        [254, 255, 255],
        [90, 198, 189]
      ]
      colorSelect = damage_colors[by]
      dispose_damage
      damage_string = (value.is_a?(Numeric)) ? value.abs.to_s : value.to_s
      bitmap = Bitmap.new(160, 48)
      bitmap.font.name = "Power Green"
      bitmap.font.size = 27
      bitmap.font.color.set(colorSelect[0]/8,colorSelect[1]/8,colorSelect[2]/8)
      (0..2).each do |x|
        (12..14).each do |y|
          next if x == 0 && y == 12
          bitmap.draw_text(x, y, 160, 36, damage_string, 1)
        end
      end
      bitmap.font.color.set(colorSelect[0],colorSelect[1],colorSelect[2])
      ...
      @_damage_sprite.ox     = self.width+20 if centered
      @_damage_sprite.oy     = self.height/4 if centered
      @_damage_sprite.x      = self.x + (centered ? self.width : 0)
    ...
    def update
      ...
      if @_damage_duration > 0
        ...
        newY = 4 if @_damage_duration < 28
        @_damage_sprite.y = self.y + newY - (self.oy / 2)

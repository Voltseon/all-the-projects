...
# Uses physfs to test if a file exists
def safeExists?(f)
  return System.file_exists?(f)
end
...
# ONly works if the bitmap is a png because that's the only audio format that matters idiots
def pbResolveBitmap(path)
  return nil if !path
  filename = path.gsub(/\.(bmp|png|gif|jpg|jpeg)$/, "") + ".png"
  return System.file_exists?(filename) ? filename : nil
end
...
module RTP
  ...
  def self.getImagePath(filename)
    return self.getPath(filename, [".png"])   # ".jpg", ".jpeg", ".bmp", for my sanity's sake ".gif" was removed too
  end

  def self.getAudioPath(filename)
    return self.getPath(filename, [".wav", ".ogg"])   # ".mp3", for my sanity's sake ".mid", ".midi" and ".wma" were removed too
  end
...
module FileTest
  ...
  AUDIO_EXTENSIONS = [".ogg", ".wav"]   # ".mp3", ".wma", ".mid", ".midi"
...
def pbGetFileChar(file)
  p "#{file} => #{System.file_exists?(file)}"
  return System.file_exists?(file)

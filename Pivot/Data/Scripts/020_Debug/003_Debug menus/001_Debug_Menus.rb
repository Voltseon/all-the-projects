...
def pbDebugMenu(show_all = true)
  ...
  ListHandlers.each_available(:debug_menu) do |option, hash, name|
  ...
  loop do
    ...
    if commands.hasSubMenu?(cmd)
      ...
      return if ListHandlers.call(:debug_menu, cmd, "effect", sprites, viewport)
    else
      ListHandlers.call(:debug_menu, cmd, "effect")
    end
  end
  pbPlayCloseMenuSE
...
module PokemonDebugMixin
  def pbPokemonDebug(pkmn, pkmnid, heldpoke = nil, settingUpBattle = false)
    # Get all commands
    commands = CommandMenuList.new
    ListHandlers.each_available(:pokemon_debug_menu) do |option, hash, name|
    ...
    loop do
      ...
      if command < 0
        ...
        if commands.hasSubMenu?(cmd)
          commands.currentList = cmd
          command = 0
        elsif ListHandlers.call(:pokemon_debug_menu, cmd, "effect", pkmn, pkmnid, heldpoke, settingUpBattle, self)
          break
        end
      end
    end

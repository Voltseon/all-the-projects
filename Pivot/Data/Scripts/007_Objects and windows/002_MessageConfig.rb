module MessageConfig
  LIGHT_TEXT_MAIN_COLOR   = Color.new(254,255,255)
  LIGHT_TEXT_SHADOW_COLOR = Color.new(64,64,64)
  DARK_TEXT_MAIN_COLOR    = Color.new(254,255,255)
  DARK_TEXT_SHADOW_COLOR  = Color.new(64,64,64)
  ...
  def self.pbDefaultSystemFrame
    if $PokemonSystem
      return pbResolveBitmap("Graphics/Windowskins/choice 1")
    else
      return pbResolveBitmap("Graphics/Windowskins/choice 1")
    end
  end
  ...
  def self.pbDefaultSpeechFrame
    if $PokemonSystem
      return pbResolveBitmap("Graphics/Windowskins/speech hgss 1")
    else
      return pbResolveBitmap("Graphics/Windowskins/speech hgss 1")
    end
  end
...
def pbRepositionMessageWindow(msgwindow, linecount = 2)
  msgwindow.height = (22 * linecount) + msgwindow.borderY
  msgwindow.y = (Graphics.height) - (msgwindow.height)
  msgwindow.width = 580
  msgwindow.x = (Graphics.width/2) - (msgwindow.width/2)
  if $game_system
...
def pbTransparentAndHide(sprites)
  visiblesprites = {}
  numFrames = (Graphics.frame_rate * 0.4).floor
  pbDeactivateWindows(sprites) {
    (0..numFrames).each do |j|
...
def pbTransparentAndShow(sprites, visiblesprites = nil)
  return if sprites.nil? || sprites[0].nil?
  sprites = sprites[0].merge(sprites[1]) if sprites.is_a?(Array)
  if visiblesprites
    visiblesprites.each do |i|
      if i[1] && sprites[i[0]] && !pbDisposed?(sprites[i[0]])
        sprites[i[0]].visible = true

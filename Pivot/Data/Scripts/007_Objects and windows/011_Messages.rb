...
def pbDisplayTooltipWindow(viewport, text)
  tooltip = Window_AdvancedTextPokemon.new(text)
  pbSetSmallFont(tooltip.contents)
  tooltip.contents.text_offset_y = 2
  tooltip.border_widths = [2, -8]
  #if header_text != ""
    #tooltip.contents.text_offset_y = -2
    #tooltip.lineHeight = 24
    #tooltip.border_widths = [2, 8]
  #end
  tooltip.setSkin("Graphics/Windowskins/choice 1")
  tooltip.resizeToFit(tooltip.text, 368)
  tooltip.width = 120 if tooltip.width <= 120
  tooltip.viewport = viewport
  tooltip.z = viewport.z + 1
  return tooltip
end
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|se|wt|wtnp|ch|char)\[([^\]]*)\]|\\(g|cn|pt|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  controls.length.times do |i|
    ...
    when "f"
      facewindow&.dispose
      facewindow = PictureWindow.new("Graphics/Pictures/Faces/#{param}")
    when "ff"
      facewindow&.dispose
...
#===============================================================================
def pbMessage(message, commands = nil, cmdIfCancel = 0, skin = "Graphics/Windowskins/speech hgss 1", defaultCmd = 0, &block)
  ret = 0

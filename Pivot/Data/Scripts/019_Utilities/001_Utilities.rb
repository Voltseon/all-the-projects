...
def pbChangePlayer(id)
  ...
  $player.character_id = Character.get(id-1).internal
  $stats.times_played_characters[$player.character_id] += 1 unless $game_temp.training || $player.transformed != :NONE || $player.character.get_prevos > 0
  $game_player.refresh_charset
  return true
end

def pbTrainerName(name = nil, outfit = 1)
  pbChangePlayer(1) if $player.character_ID < 1
  if name.nil?
    name = pbEnterPlayerName(_INTL("My name is...?"), 0, Settings::MAX_PLAYER_NAME_SIZE, "")
    if name.nil? || name.empty? || !checkName(name)
      pbMessage("That name is not valid.")
      pbTrainerName
    end
  end
  $player.name   = name
...
def pbChangePlayerName(name = nil)
  pbChangePlayer(1) if $player.character_ID < 1
  $player.name = "" if $player.name == "Unnamed"
  initialName = $player.name || ""
  if name.nil?
    name = pbEnterPlayerName(_INTL("My name is...?"), 0, Settings::MAX_PLAYER_NAME_SIZE, initialName)
    name = initialName if name == ""
    if (!pbCheckUsernameAvailability(name) && !$game_temp.training)
      pbMessage("That username is already taken.")
      pbChangePlayerName
    elsif (name.nil? || name.empty? || !checkName(name)) && name != initialName
      pbMessage("That name is not valid.")
      pbChangePlayerName
    else
      $player.name = name
      pbUpdatePlayerName($player.id, name) unless $game_temp.training
      Game.save unless $game_temp.training
    end
  end
end

def checkName(name)
  banned_words = HTTPLite::JSON.parse(pbDownloadToString("https://raw.githubusercontent.com/ENLSLive/profane-words/master/words.json"))
  banned_words.each do |word|
    return false if name.downcase.include?(word)
  end
  space_count = name.scan(/\s/).length.to_f
  return false if space_count/name.length.to_f > 0.35
  return false if ["&","^","*","#","@",",",":"].any? { |c| name.include?(c) }
  return true
end
...
def pbScreenCapture
  ...
  Dir.mkdir("Screenshots/") if !FileTest.directory?("Screenshots/")
  name = $game_map.name
  name = "Unknown" if ["?","<",">","'",",","?","[","]","}","{","=","-",")","(","*","&","^","%","$","#","`","~","{","}"].any? { |word| name.include?(word) }
  filestart = "./Screenshots/" + name + " - " + filestart + ".png"
  Graphics.screenshot(filestart)
  pbSEPlay("Pkmn exp full") if FileTest.audio_exist?("Audio/SE/Pkmn exp full")
end

def pbSaveScreenshot(blur = false)
  filestart = "./Graphics/Pictures/lastsave.png"
  bitmap = Graphics.snap_to_bitmap
  bitmap.blur(4) if blur
  bitmap.to_file(filestart)
end
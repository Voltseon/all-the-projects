...
class Spriteset_Map
  ...
  def pbDamagePartner(partner, dmg, by_me)
    @character_sprites.each { |sprite| next if sprite.name != "partner#{partner}"; sprite.damage(dmg, false, true, (by_me ? 0 : 1)) }
  end

  def pbDamageEvent(event, dmg, by_me)
    @character_sprites.each { |sprite| next if sprite.character.id != event; sprite.damage(dmg, false, true, (by_me ? 0 : 1)) }
  end

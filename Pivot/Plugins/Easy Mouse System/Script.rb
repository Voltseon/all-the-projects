...
module Mouse
  ...
  class << self
    ...
    def start
      ...
      @mouse_bitmap.ox = 4
      @mouse_bitmap.oy = 10
      @mouse_in_window_last_frame = false
    end
    #---------------------------------------------------------------------------
    ...
    def update
      @x, @y = getMousePos
      if Input.mouse_in_window? && self.active?
        showCursor if !@mouse_in_window_last_frame
        @mouse_bitmap.x = @x.to_i
        @mouse_bitmap.y = @y.to_i
      else
        hideCursor
      end
      ...
      @mouse_in_window_last_frame = Input.mouse_in_window?
      @ui = UIHandler.new(@viewport) if !@ui
      @ui.update
    end
    #---------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    def hideCursor
      @mouse_bitmap.visible = false
    end
    #---------------------------------------------------------------------------
    #  shows the mouse cursor
    #---------------------------------------------------------------------------
    def showCursor
      @mouse_bitmap.visible = true
    end
    #---------------------------------------------------------------------------
    #  get the mouse cursor bitmap
    #---------------------------------------------------------------------------
    def cursorBitmap
      @mouse_bitmap.bitmap
    end
    ...
    def setCursorBitmap(value)
      @mouse_bitmap.setBitmap(value)
    end
    #---------------------------------------------------------------------------
    #  returns the visiblity of the mouse cursor
    #---------------------------------------------------------------------------
    def cursor_visible
      return @mouse_bitmap.visible
    end
    #---------------------------------------------------------------------------
    #  returns scroll direction (-1 down, 0 stationary, 1 up)
    #---------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    def active?
      return false if !Input.mouse_in_window?
    ...
    #---------------------------------------------------------------------------
    #  check if mouse is being pressed and held down for a shorter specific period of time (1/10th as long)
    #---------------------------------------------------------------------------
    def hold_short?(object = nil, button = :left)
      return false unless press?(object, button)

      Input.time?(input_map(button)) > CLICK_TIMEOUT * 100000
    end
    #---------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    #  check if mouse is clicking briefly on object (75% as long)
    #---------------------------------------------------------------------------
    def click_short?(object = nil, button = :left)
      return false if object && !over?(object)

      return @hold = 0 || true if !Input.press?(input_map(button)) && @hold.between?(1, CLICK_TIMEOUT * 750000)

      @hold = Input.time?(input_map(button))
      false
    end
    #---------------------------------------------------------------------------
    #  check if mouse is triggering on object
    #---------------------------------------------------------------------------
    def trigger?(object = nil, button = :left)
      return Input.trigger?(input_map(button))
    end
    #---------------------------------------------------------------------------
  end
end

def pbInteractOverworldEncounter
  ...
  return if evt.character_name == "[FW] Balls"
  evt.lock
  pkmn = evt.variable.pokemon
...
def get_viable_tile(enc_type=:Land)
  possible_tiles = []
  ...
  (($game_player.x-possible_distance)..($game_player.x+possible_distance)).each do |x|
    ...
    (($game_player.y-possible_distance)..($game_player.y+possible_distance)).each do |y|
      ...
      next unless $game_map.passable?(x, y, 0) || enc_type == :Water
      ...
      next unless (enc_type == :Water && $game_map.terrain_tag(x, y).can_surf_freely) || (enc_type == :Land && (VoltseonsOverworldEncounters::GRASS_TILES.include?($game_map.terrain_tag(x, y).id) || $PokemonEncounters.has_cave_encounters?))
...
def pbDestroyOverworldEncounter(event,animation=true,play_sound=false)
  ...
  echoln "Despawning #{event.variable.pokemon.name}" if VoltseonsOverworldEncounters::LOG_SPAWNS
  if play_sound
  ...
  spriteset&.addUserAnimation(VoltseonsOverworldEncounters::SPAWN_ANIMATION[(event.variable.terrain == :Land ? 0 : 1)], event.x, event.y, true, 1) if animation
  ...
  if VoltseonsOverworldEncounters::DELETE_EVENTS
    Rf.delete_event(event.variable.r_event)
  else
...
def pbPokemonIdle(evt)
  ...
  return if evt.character_name == "[FW] Balls"
  if rand(50)==1 || (!VoltseonsOverworldEncounters::GRASS_TILES.include?($game_map.terrain_tag(evt.x, evt.y).id) && !$PokemonEncounters.has_cave_encounters? && evt.variable.terrain == :Land) || (evt.variable.terrain == :Water && (!$game_map.terrain_tag(evt.x, evt.y).can_surf_freely || rand(20) == 1))
    unless evt.variable.pokemon.shiny?
      pbDestroyOverworldEncounter(evt)
      return
    end
  end
  case evt.variable.terrain
  when :Water
    possible_movements = []
    [[0,1], [-1,0], [1,0], [0,-1]].each_with_index do |movement, i|
      next unless $game_map.terrain_tag(evt.x + movement[0], evt.y + movement[1]).can_surf_freely
      possible_movements.push((i*2)+2)
    end
    evt.move_generic(possible_movements.sample)
  when :Land
    evt.move_random
  end
  dist = (((evt.x - $game_player.x).abs + (evt.y - $game_player.y).abs) / 4).floor
  pbDestroyOverworldEncounter(evt) if pbDistanceToPlayer(evt) > VoltseonsOverworldEncounters::MAX_DISTANCE && !evt.variable.pokemon.shiny?
  GameData::Species.play_cry_from_pokemon(evt.variable.pokemon, [75, 65, 55, 40, 27, 22, 15][dist]) if dist <= 6 && dist >= 0 && rand(20) == 1
end

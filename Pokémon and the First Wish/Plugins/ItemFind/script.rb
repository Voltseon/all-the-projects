...
class PokemonItemFind_Scene
  def pbStartScene
    ...
    @viewport.z = 99999
    @sprites = {}
    skin = WINDOWSKIN_NAME == "" ? MessageConfig.pbDefaultSpeechFrame : "Graphics/Windowskins/" + WINDOWSKIN_NAME
    
    
    @sprites["background"] = Window_UnformattedTextPokemon.newWithSize("", (Graphics.width/2)-290, 0, 580, 0, @viewport)
    ...
    @sprites["itemicon"] = ItemIconSprite.new((Graphics.width/2)-242, Graphics.height - 48, :NONE, @viewport)
    @sprites["itemicon"].visible = false
    @sprites["itemicon"].z = @viewport.z + 2
	
    @sprites["descwindow"] = Window_UnformattedTextPokemon.newWithSize("", (Graphics.width/2)-228, 0, 480, 64, @viewport)
    ...
    @sprites["titlewindow"] = Window_UnformattedTextPokemon.newWithSize("", 0, 0, 580, 16, @viewport)
  ...
  def pbShow(item)
    ...
    descwindow.resizeToFit(description, 480)
    descwindow.height += 8
    ...
    titlewindow.y = Graphics.height - descwindow.height - 36
    titlewindow.x = (Graphics.width/2)-278
    titlewindow.visible = true

    background = @sprites["background"]
    background.height = descwindow.height + 36

...
class PokemonBag
  ...
  def add(item, qty = 1)
    ...
    # Convert 10 Relic Copper to 1 Relic Silver, and 10 Relic Silver to 1 Relic Gold
    if item_data.id == :RELICCOPPER && quantity(item_data.id) >= 100
      while quantity(item_data.id) >= 100
        remove(item_data.id, 100)
        add(:RELICSILVER, 1)
      end
    elsif item_data.id == :RELICSILVER && quantity(item_data.id) >= 10
      while quantity(item_data.id) >= 10
        remove(item_data.id, 10)
        add(:RELICGOLD, 1)
      end
    end
    $scene.active_hud.redraw
    return ret
  end

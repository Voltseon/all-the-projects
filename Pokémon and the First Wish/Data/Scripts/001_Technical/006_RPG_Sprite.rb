class SpriteAnimation
  ...
  ["x", "y", "ox", "oy", "viewport", "flash", "src_rect", "opacity", "tone", "zoom_x", "zoom_y"].each do |def_name|
    eval <<-__END__
  ...
  def animation_set_sprites(sprites, cell_data, position, quick_update = false)
    ...
    if position == 3
      ...
      sprite_x = self.x + ((self.src_rect.width / 2) - self.ox) * TilemapRenderer.zoom_x
      sprite_y = self.y - self.oy * TilemapRenderer.zoom_y
    ...
    16.times do |i|
      ...
      when 1 then sprite.z = sprite.y + (Game_Map::TILE_HEIGHT * TilemapRenderer.zoom_y * 3 / 2) + 1
      when 2 then sprite.z = sprite.y + (Game_Map::TILE_HEIGHT * TilemapRenderer.zoom_y * 3) + 1
      else        sprite.z = 2000
      end
      sprite.ox         = 96
      sprite.oy         = 96

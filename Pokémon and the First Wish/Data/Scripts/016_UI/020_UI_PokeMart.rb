...
class PokemonMartAdapter
  ...
  def getDisplayPrice(item, selling = false)
    price = pbGoldString(getPrice(item, selling))
    return _INTL("<ar>{1}</ar>", price)
  end
...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    if index == count - 1
      ...
      xQty = rect.x + rect.width - sizeQty
      ...
      drawFormattedTextEx(self.contents, rect.x, ypos + 2, rect.width, qty, self.baseColor, self.shadowColor)
    end
...
class PokemonMart_Scene
  ...
  def pbRefresh
    ...
    @sprites["moneywindow"].text = _INTL("{1}", @adapter.getMoneyString)
  end
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    @sprites["itemwindow"] = Window_PokemonMart.new(
      stock, winAdapter, 180, 10, 330 + 16, Graphics.height - 24
    )
    ...
    @sprites["moneywindow"].resizeToFit(@adapter.getMoneyString)
  ...
  def pbConfirm(msg)
    ...
    loop do
      ...
      if (Input.trigger?(Input::USE) || Mouse.trigger?) && dw.resume && !dw.busy?
        cw.dispose
        @sprites["helpwindow"].visible = false
        return (cw.index == 0)
      end
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.text = _INTL("x{1}<r>{2}", curnumber, pbGoldString((curnumber * itemprice)))
...
class PokemonMartScreen
      ...
      if GameData::Item.get(item).is_important?
        ...
        if quantity > 1
          next if !pbConfirm(_INTL("So you want {1} {2}?\nThey'll be {3}. All right?",
                                   quantity, itemnameplural, pbGoldString(price)))
        elsif quantity > 0
          next if !pbConfirm(_INTL("So you want {1} {2}?\nIt'll be {3}. All right?",
                                   quantity, itemname, pbGoldString(price)))
        end
      end
      if @adapter.getMoney < price
        pbDisplayPaused(_INTL("You don't have enough money."))
  ...
  def pbSellScreen
    ...
    loop do
      ...
      price /= 2
      price *= qty
      if pbConfirm(_INTL("I can pay {1}.\nWould that be OK?", pbGoldString(price)))
        ...
        pbDisplayPaused(_INTL("You turned over the {1} and got {2}.",
                              sold_item_name, pbGoldString(price))) { pbSEPlay("Mart buy item") }
        @scene.pbRefresh
      end
      @scene.pbHideMoney
    end

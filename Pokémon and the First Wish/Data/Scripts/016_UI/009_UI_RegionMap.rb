...
class MapBottomSprite < Sprite
  ...
  def refresh
    ...
    textpos = [
      ...
      [@maplocation,                 18, 552, 0, TEXT_MAIN_COLOR, TEXT_SHADOW_COLOR],
      [@mapdetails, Graphics.width - 16, 552, 1, TEXT_MAIN_COLOR, TEXT_SHADOW_COLOR]
    ]
...
class PokemonRegionMap_Scene
  ...
  def pbMapScene
    ...
    loop do
      Graphics.update
      Input.update
      @sprites["cursor"].x = Input.mouse_x if Input.mouse_in_window?
      @map_x = (@sprites["cursor"].x / SQUARE_WIDTH)
      @sprites["cursor"].y = Input.mouse_y if Input.mouse_in_window?
      @map_y = (@sprites["cursor"].y / SQUARE_HEIGHT) -1
      #echoln "#{@map_x}, #{@map_y}"
      pbUpdate
      @sprites["mapbottom"].maplocation = pbGetMapLocation(@map_x, @map_y)
      @sprites["mapbottom"].mapdetails  = pbGetMapDetails(@map_x, @map_y)
      ...
      elsif (Input.trigger?(Input::USE) || Mouse.trigger?) && @mode == 1   # Choosing an area to fly to
      ...
      elsif (Input.trigger?(Input::USE) || Mouse.trigger?) && @editor   # Intentionally after other USE input check

...
class PokemonLoad_Scene
  def pbStartScene(commands, show_continue, trainer, frame_count, stats, map_id)
    ...
    commands.length.times do |i|
      ...
      @sprites["panel#{i}"].x = 301
      @sprites["panel#{i}"].y = y
      @sprites["panel#{i}"].pbRefresh
      y += (show_continue && i == 0) ? 228 : 52
    end
  ...
  def pbChoose(commands)
    ...
    loop do
      ...
      Input.update
      commands.length.times do |i|
        break unless Mouse.active?
        if Mouse.over_area?(@sprites["panel#{i}"].x+16, @sprites["panel#{i}"].y, 384, 46)
          @sprites["panel#{@sprites["cmdwindow"].index}"].selected = false
          @sprites["cmdwindow"].index = i
          @sprites["panel#{i}"].selected = true
          pbUpdate
          break
        elsif i != @sprites["cmdwindow"].index
          @sprites["panel#{i}"].selected = false
        end
      end
      pbUpdate
      if Input.trigger?(Input::USE) || Mouse.trigger?
        return @sprites["cmdwindow"].index if @sprites["panel#{@sprites["cmdwindow"].index}"].selected
      end
    end
  end

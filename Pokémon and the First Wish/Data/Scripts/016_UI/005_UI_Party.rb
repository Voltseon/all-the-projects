...
class PokemonParty_Scene
  ...
  def pbDisplay(text)
    ...
    loop do
      ...
      elsif Input.trigger?(Input::BACK) || Input.trigger?(Input::USE) || Mouse.trigger?
        break
      end
    end
    @sprites["messagebox"].visible = false
      ...
      loop do
        ...
        if !@sprites["messagebox"].busy?
          if Input.trigger?(Input::BACK)
            ret = false
            break
          elsif (Input.trigger?(Input::USE) || Mouse.trigger?) && @sprites["messagebox"].resume
...
MenuHandlers.add(:party_menu, :evolve, {
  "name"      => _INTL("Evolve"),
  "order"     => 15,
  "condition" => proc { |screen, party, party_idx| next party[party_idx].check_evolution_on_level_up },
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    command = screen.scene.pbShowCommands(_INTL("Would you like to evolve {1}?",pkmn.name),[_INTL("Yes"), _INTL("No")])
    next unless command == 0
...
MenuHandlers.add(:party_menu_item, :move, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    loop do
      ...
      if newitem == :LEFTOVERS || newitem == :MUD

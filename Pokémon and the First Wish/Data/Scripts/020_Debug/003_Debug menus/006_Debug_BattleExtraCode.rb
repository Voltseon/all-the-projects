...
#    PBEffects::FocusPunch - only applies to use of specific move, not suitable for setting via debug
    ...
    PBEffects::Brandish       => { name: "Brandish power multiplier 2**x (0-4)",            default: 0, max: 4 },
...
#    PBEffects::ParentalBond - only applies during use of move, not suitable for setting via debug
    ...
    PBEffects::NoxiousPulse   => { name: "Will explode if self tries to recover health",    default: false },
...
#    PBEffects::Type3 - set elsewhere
  ...
  SIDE_EFFECTS = {
    ...
    PBEffects::DrowsyMist         => { name: "Drowsy Mist layers (0-2)",               default: 0, max: 2 },
  ...
  FIELD_EFFECTS = {
    ...
    PBEffects::ExtortionRoom   => { name: "Extortion Room duration",          default: 0 }
  }

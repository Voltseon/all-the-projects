...
#===============================================================================
def pbGetGoldString
  # Convert money to Relic Gold(1000), Silver(100) and Copper(1)
  gold = $player.money / 1000
  silver = ($player.money - gold * 1000) / 100
  copper = ($player.money - gold * 1000 - silver * 100) / 1
  string = ""
  string += _INTL("<icon=gc>{1}\n", gold) if gold > 0
  string += _INTL("<icon=sc>{1}\n", silver) if silver > 0
  string += _INTL("<icon=cc>{1}\n", copper) if copper > 0
  return string
end

def pbGoldString(value, journal = false)
  # Convert money to Relic Gold(1000), Silver(100) and Copper(1)
  gold = value / 1000
  silver = (value - gold * 1000) / 100
  copper = (value - gold * 1000 - silver * 100) / 1
  string = ""
  string += _INTL("<icon=gc#{(journal) ? "_1" : ""}>{1} ", gold) if gold > 0
  string += _INTL("<icon=sc#{(journal) ? "_1" : ""}>{1} ", silver) if silver > 0
  string += _INTL("<icon=cc#{(journal) ? "_1" : ""}>{1} ", copper) if copper > 0
  return string
end
...
def pbDisplayGoldWindow(msgwindow)
  ...
  echoln moneyString
  goldwindow = Window_AdvancedTextPokemon.new(_INTL("{1}", moneyString))
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|se|cry|wt|wtnp|ch|char)\[([^\]]*)\]|\\(g|cn|pt|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  loop do
    ...
    controls.length.times do |i|
      ...
      when "cry"    # Play cry
        pbSEPlay(pbStringToAudioFile("Cries/#{param}"))
      when "char"
    ...
    if Input.trigger?(Input::USE) || Input.trigger?(Input::BACK) || Mouse.trigger?
      if msgwindow.busy?
        msgwindow.resume
...
def pbDisplayMessageBrief(message, frames=20, letterbyletter = false)

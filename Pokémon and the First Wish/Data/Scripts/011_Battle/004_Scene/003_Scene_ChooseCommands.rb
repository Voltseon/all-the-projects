class Battle::Scene
  ...
  def pbCommandMenuEx(idxBattler, texts, mode = 0)
    ...
    loop do
      ...
      cw.buttons.each_with_index do |button, i|
        cw.index = i if Mouse.over_area?(button.x, button.y, button.src_rect.width, button.src_rect.height)
      end
      pbPlayCursorSE if cw.index != oldIndex
      # Actions
      if Input.trigger?(Input::USE) || Mouse.trigger?                 # Confirm choice
        pbPlayDecisionSE
        ret = cw.index
  ...
  def pbFightMenu(idxBattler, megaEvoPossible = false)
    ...
    loop do
      ...
      elsif Input.trigger?(Input::UP) || Mouse.scroll_direction < 0
        cw.index -= 2 if (cw.index & 2) == 2
      elsif Input.trigger?(Input::DOWN) || Mouse.scroll_direction > 0
        if battler.moves[cw.index + 2]&.id && (cw.index & 2) == 0
          cw.index += 2
        end
      end
      cw.buttons.each_with_index do |button, i|
        break if !battler.moves[cw.index + i]&.id
        cw.index = i if Mouse.over_area?(button.x, button.y, button.src_rect.width, button.src_rect.height)
      end
      pbPlayCursorSE if cw.index != oldIndex
      # Actions
  ...
  def pbChooseTarget(idxBattler, target_data, visibleSprites = nil)
    ...
    loop do
      ...
      if mode == 0   # Choosing just one target, can change index
        ...
        elsif ((Input.trigger?(Input::UP) || Mouse.scroll_direction < 0) && cw.index.even?) ||
              ((Input.trigger?(Input::DOWN) || Mouse.scroll_direction > 0) && cw.index.odd?)
        ...
        cw.buttons.each_with_index do |button, i|
          next if button.nil? || nil_or_empty?(cw.texts[i])
          cw.index = i if Mouse.over_area?(button.x, button.y, button.src_rect.width, button.src_rect.height)
        end
      ...
      if Input.trigger?(Input::USE) || Mouse.trigger?   # Confirm
        ret = cw.index
        pbPlayDecisionSE
        break

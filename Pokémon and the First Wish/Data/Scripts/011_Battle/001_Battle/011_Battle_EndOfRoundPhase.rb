...
#    when :StrongWinds then pbDisplay(_INTL("The wind is strong."))
  ...
  def pbEORWishHealing
    @positions.each_with_index do |pos, idxPos|
      ...
      pbDisplay(_INTL("{1}'s wish came true!", wishMaker)) if @battlers[idxPos].pbRecoverHP(pos.effects[PBEffects::WishAmount]) > 0
    end
  end
  ...
  def pbEORTerrainHealing(battler)
    ...
    if @field.terrain == :Grassy && battler.affectedByTerrain? && battler.canHeal?
      ...
      pbDisplay(_INTL("{1}'s HP was restored.", battler.pbThis)) if battler.pbRecoverHP(battler.totalhp / 16) > 0
    end
  end
  ...
  def pbEORHealingEffects(priority)
    ...
    priority.each do |battler|
      ...
      hpGain = (hpGain * 1.3).floor if battler.hasActiveAbility?(:DRAINBOOST)
      pbDisplay(_INTL("Aqua Ring restored {1}'s HP!", battler.pbThis(true))) if battler.pbRecoverHP(hpGain) > 0
    end
    # Ingrain
    priority.each do |battler|
      ...
      hpGain = (hpGain * 1.3).floor if battler.hasActiveItem?(:BIGROOT)
      hpGain = (hpGain * 1.3).floor if battler.hasActiveAbility?(:DRAINBOOST)
      pbDisplay(_INTL("{1} absorbed nutrients with its roots!", battler.pbThis)) if battler.pbRecoverHP(hpGain) > 0
    end
    # Leech Seed
    priority.each do |battler|
  ...
  def pbEOREndFieldEffects(priority)
    ...
    # Extortion Room
    pbEORCountDownFieldEffect(PBEffects::ExtortionRoom,
                              _INTL("Extortion Room wore off, and Attack and Sp. Atk stats returned to normal!"))              
    # Magic Room
  ...
  def pbEndOfRoundPhase
    ...
    allBattlers.each do |battler|
      ...
      battler.effects[PBEffects::NoxiousPulse]     = false

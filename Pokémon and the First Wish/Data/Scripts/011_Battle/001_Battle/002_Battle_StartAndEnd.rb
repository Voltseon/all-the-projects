class Battle
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      if moneyGained > 0
        ...
        pbDisplayPaused(_INTL("You got {1} for winning!", pbGoldString(moneyGained)))
      end
    end
    ...
    if @field.effects[PBEffects::PayDay] > 0
      ...
      if moneyGained > 0
        $stats.battle_money_gained += moneyGained
        pbDisplayPaused(_INTL("You picked up {1}!", pbGoldString(moneyGained)))
      end
    end
  end
  ...
  def pbLoseMoney
    ...
    if moneyLost > 0
      ...
      if trainerBattle?
        pbDisplayPaused(_INTL("You gave {1} to the winner...", pbGoldString(moneyLost)))
      else
        pbDisplayPaused(_INTL("You panicked and dropped {1}...", pbGoldString(moneyLost)))
      end
    end
  end

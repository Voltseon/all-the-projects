class Battle
  ...
  def pbEntryHazards(battler)
    ...
    # Drowsy Mist
    if battler_side.effects[PBEffects::DrowsyMist] > 0 && !battler.fainted? && !battler.airborne? &&
      battler.pbCanSleep?(nil, false) && !battler.hasActiveItem?(:HEAVYDUTYBOOTS) && !battler.pbHasType?(:BUG)
      if battler_side.effects[PBEffects::DrowsyMist] > 1
        battler.pbSleep(nil, _INTL("{1} fell asleep due to the drowsy mist!", battler.pbThis))
      else
        pbDisplay(_INTL("{1} became drowsy!", battler.pbThis))

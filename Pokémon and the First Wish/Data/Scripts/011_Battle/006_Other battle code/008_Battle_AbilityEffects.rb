...
Battle::AbilityEffects::SpeedCalc.add(:WEATHERING,
  proc { |ability, battler, mult|
    next mult * 1.2 if [:Sandstorm, :Hail].include?(battler.effectiveWeather)
  }
)
...
Battle::AbilityEffects::MoveImmunity.add(:WEATHERING,
  proc { |ability, user, target, move, type, battle, show_message|
    next target.pbMoveImmunityStatRaisingAbility(user, move, type,
...
Battle::AbilityEffects::MoveImmunity.add(:WEATHERING,
  proc { |ability, user, target, move, type, battle, show_message|
    next target.pbMoveImmunityStatRaisingAbility(user, move, type,
       :WATER, :SPECIAL_DEFENSE, 1, show_message)
  }
)
...
Battle::AbilityEffects::MoveImmunity.add(:WEATHERING,
  proc { |ability, user, target, move, type, battle, show_message|
    next target.pbMoveImmunityStatRaisingAbility(user, move, type,
       :GROUND, :SPECIAL_DEFENSE, 1, show_message)
  }
)
...
Battle::AbilityEffects::MoveImmunity.add(:COMPOST,
  proc { |ability, user, target, move, type, battle, show_message|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:DEFENDER,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if user.lastFoeAttacker.length > 0
...
Battle::AbilityEffects::DamageCalcFromUser.add(:BLAZE,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if user.hp <= user.totalhp / 3 && type == :FIRE || move.function == "ErupticMeteors"
      mults[:attack_multiplier] *= 1.5
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:FLASHFIRE,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if user.effects[PBEffects::FlashFire] && type == :FIRE || move.function == "ErupticMeteors"
      mults[:attack_multiplier] *= 1.5
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SPECIALPOWER,
  proc { |ability, user, target, move, mults, baseDmg, type|
...
Battle::AbilityEffects::DamageCalcFromUser.copy(:SNIPER, :PREDATOR)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:MASSSTRENGTH,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:final_damage_multiplier] *= 1 + 0.5 * user.allAllies.length if user.allAllies.length > 0
  }
)
...
Battle::AbilityEffects::DamageCalcFromAlly.add(:MASSSTRENGTH,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:final_damage_multiplier] *= 1 + 0.5 * user.allAllies.length if user.allAllies.length > 0
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:DRYSKIN,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:base_damage_multiplier] *= 1.25 if type == :FIRE || move.function == "ErupticMeteors"
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:FLUFFY,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:final_damage_multiplier] *= 2 if move.calcType == :FIRE || move.function == "ErupticMeteors"
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:HEATPROOF,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:base_damage_multiplier] /= 2 if type == :FIRE || move.function == "ErupticMeteors"
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:THICKFAT,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:base_damage_multiplier] /= 2 if [:FIRE, :ICE].include?(type) || move.function == "ErupticMeteors"
  }
)

Battle::AbilityEffects::DamageCalcFromTarget.add(:WATERBUBBLE,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:final_damage_multiplier] /= 2 if type == :FIRE || move.function == "ErupticMeteors"
  }
)
...
Battle::AbilityEffects::DamageCalcFromTargetAlly.add(:WEATHERING,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:final_damage_multiplier] *= 0.8 if [:Sandstorm, :Hail].include?(battler.effectiveWeather)
  }
)
...
Battle::AbilityEffects::CriticalCalcFromUser.add(:PREDATOR,
  proc { |ability, user, target, c|
    next c + 1
  }
)
...
Battle::AbilityEffects::OnBeingHit.add(:RECOVERYBATON,
  proc { |ability, user, target, move, battle|
    next if !target.fainted?
    next if target.allAllies.length == 0
    battle.pbShowAbilitySplash(target)
    target.allAllies.each do |b|
      if b.pbRecoverHP(b.totalhp) > 0
        if Battle::Scene::USE_ABILITY_SPLASH
          battle.pbDisplay(_INTL("{1}'s HP was restored.", b.pbThis))
        else
          battle.pbDisplay(_INTL("{1}'s {2} restored {3}'s HP.", target.pbThis, target.abilityName, b.pbThis))
        end
      end
...
Battle::AbilityEffects::OnDealingHit.add(:VOLCANICPRESENCE,
  proc { |ability, user, target, move, battle|
...
Battle::AbilityEffects::OnSwitchIn.add(:TWISTEDENTRANCE,
  proc { |ability, battler, battle, switch_in|
    if battle.field.effects[PBEffects::WonderRoom] > 0
      battle.pbShowAbilitySplash(battler)
      battle.field.effects[PBEffects::WonderRoom] = 0
      battle.pbDisplay(_INTL("{1} made the battlefield normal!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    else
      battle.pbShowAbilitySplash(battler)
      battle.pbCommonAnimation("WonderRoom", battler)
      battle.field.effects[PBEffects::WonderRoom] = 5
      battle.pbDisplay(_INTL("{1} made the battlefield weird!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    end
  }
)

Battle::AbilityEffects::OnSwitchIn.add(:EXTORTINGENTRANCE,
  proc { |ability, battler, battle, switch_in|
    if battle.field.effects[PBEffects::ExtortionRoom] > 0
      battle.pbShowAbilitySplash(battler)
      battle.field.effects[PBEffects::ExtortionRoom] = 0
      battle.pbDisplay(_INTL("{1} made the battlefield normal!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    else
      battle.pbShowAbilitySplash(battler)
      battle.pbCommonAnimation("ExtortionRoom", battler)
      battle.field.effects[PBEffects::ExtortionRoom] = 5
      battle.pbDisplay(_INTL("{1} made the battlefield ominous!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    end
  }
)
...
Battle::AbilityEffects::OnSwitchIn.copy(:PRIMORDIALSEA, :SEARULER)

module Battle::CatchAndStoreMixin
  ...
  def pbRecordAndStoreCaughtPokemon
    @caughtPokemon.each do |pkmn|
      ...
      # Store caught Pokémon
      pbStorePokemon(pkmn)
      # Get item from Pokémon
      if rand(3) == 1
        item = pbSelectPokedrop(pkmn.species)
        quantity = rand(2)+1
        item_name = GameData::Item.get(item).name
        item_name_plural = GameData::Item.get(item).name_plural
        $bag.add(item, quantity)
        pbSEPlay("PLA Item Get")
        pbDisplayPaused(_INTL("You picked up {1} from {2}.", (quantity > 1 ? ("#{quantity} #{item_name_plural}") : ("#{item_name}")), pkmn.name))
      end
    end
    @caughtPokemon.clear
  end

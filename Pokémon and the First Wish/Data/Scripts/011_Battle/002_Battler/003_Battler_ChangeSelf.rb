class Battle::Battler
  ...
  def pbRecoverHP(amt, anim = true, anyAnim = true)
    return 0 if pbTakeDamageNoxiousPulse
    amt = amt.round
  ...
  def pbTakeDamageNoxiousPulse
    if self.effects[PBEffects::NoxiousPulse]
      @battle.pbCommonAnimation("NoxiousPulse", self)
      @battle.pbDisplay(_INTL("The Pokémon became overwhelmed!"))
      self.lastMoveFailed = true
      if self.takesIndirectDamage?
        self.pbTakeEffectDamage((self.totalhp / 2.0).round, false) { |hp_lost|
          @battle.pbDisplay(_INTL("The Pokémon became overwhelmed!"))
        }
  ...
  def pbRecoverHPFromDrain(amt, target, msg = nil)
    return false if pbTakeDamageNoxiousPulse
    if target.hasActiveAbility?(:LIQUIDOOZE)
      @battle.pbShowAbilitySplash(target)
      amt = (amt * 1.3).floor if hasActiveAbility?(:DRAINBOOST)
      pbReduceHP(amt)
  ...
  def pbFaint(showMessage = true)
    ...
    @pokemon.faint_count = 0 if @pokemon.faint_count.nil?
    @pokemon.faint_count += 1
    pbInitEffects(false)
    # Reset status
    self.status      = :NONE
    self.statusCount = 0

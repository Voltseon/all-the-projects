  ...
  def pbMoveImmunityHealingAbility(user, move, moveType, immuneType, show_message)
    ...
    if show_message
      ...
      if canHeal? && pbRecoverHP(@totalhp / 4) > 0
        ...
        @battle.pbDisplay(_INTL("It doesn't affect {1}...", pbThis(true))) if !pbTakeDamageNoxiousPulse
      else
        @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", pbThis, abilityName, move.name)) if !pbTakeDamageNoxiousPulse
      end
      @battle.pbHideAbilitySplash(self)
    end
    return true

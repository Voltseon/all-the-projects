class Battle::Battler
  ...
  def attack
    return @spatk if @battle.field.effects[PBEffects::ExtortionRoom] > 0
    return @attack
  end

  def spatk
    return @attack if @battle.field.effects[PBEffects::ExtortionRoom] > 0
    return @spatk
  end
...
#      :FORECAST,                                          # This can be stopped
  ...
  def takesSandstormDamage?
    ...
    return false if hasActiveAbility?([:OVERCOAT, :SANDFORCE, :SANDRUSH, :SANDVEIL, :SANDSTREAM, :WEATHERING])
  ...
  def takesHailDamage?
    ...
    return false if inTwoTurnAttack?("TwoTurnAttackInvulnerableUnderground",
                                     "TwoTurnAttackInvulnerableUnderwater")
    return false if hasActiveAbility?([:OVERCOAT, :ICEBODY, :SNOWCLOAK, :SNOWWARNING, :WEATHERING])
    return false if hasActiveItem?(:SAFETYGOGGLES)
    return true
  end
  ...
  def resetBelched
    @battle.belch[@index & 1][@pokemonIndex] = false
  end

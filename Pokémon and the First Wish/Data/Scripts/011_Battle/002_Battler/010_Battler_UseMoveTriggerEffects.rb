class Battle::Battler
  ...
  def pbEffectsOnMakingHit(move, user, target)
    if target.damageState.calcDamage > 0 && !target.damageState.substitute
      # Record damage taken / dealt
      user.pokemon.damage_dealt = 0 if user.pokemon.damage_dealt.nil?
      target.pokemon.damage_taken = 0 if target.pokemon.damage_taken.nil?
      target.pokemon.damage_taken += target.damageState.hpLost
      user.pokemon.damage_dealt += target.damageState.hpLost
      # Target's ability
      ...
      # Revenge Toxins
      if target.fainted? && target.allAllies.length > 0 && user.status == :NONE
        rt = nil
        target.allAllies.each do |b|
          next if b.ability != :REVENGETOXINS
          rt = b
          break
        end
        if rt
          @battle.pbShowAbilitySplash(rt)
          rt.pbPoison(user,nil,true)
          @battle.pbHideAbilitySplash(rt)
        end
      end
  ...
  def pbEffectsAfterMove(user, targets, move, numHits)
    ...
    if move.damagingMove?
      targets.each do |b|
        ...
        #       target (in Gen 6+).
        if move.calcType == :FIRE || move.function == "ErupticMeteors" || (Settings::MECHANICS_GENERATION >= 6 && move.thawsUser?)
          b.pbCureStatus
        end
      end
    end
    ...
    @pokemon.kill_count = 0 if @pokemon.kill_count.nil?
    @pokemon.kill_count += numFainted

class Battle::Battler
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    # Store move usage data
    if !move.pbMoveFailed?(user, targets)
      @pokemon.move_usages = {} if @pokemon.move_usages.nil?
      if @pokemon.move_usages[move.id]
        @pokemon.move_usages[move.id] += 1
      else
        @pokemon.move_usages[move.id] = 1
      end
    end
    ...
    # Powder
    if user.effects[PBEffects::Powder] && (move.calcType == :FIRE || move.function == "ErupticMeteors")
    ...
    if move.damagingMove?
      ...
      when :HeavyRain
        if move.calcType == :FIRE || move.function == "ErupticMeteors"
  ...
  def pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
    ...
    # the move)
    if [:FIRE, :WATER].include?(move.calcType) || move.function == "ErupticMeteors"
      targets.each do |b|
        next if b.damageState.unaffected
        next if b.damageState.calcDamage == 0 || b.damageState.substitute

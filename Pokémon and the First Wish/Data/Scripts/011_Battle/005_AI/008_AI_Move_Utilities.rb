class Battle::AI
  ...
  def pbCheckMoveImmunity(score, move, user, target, skill)
    ...
    if skill >= PBTrainerAI.mediumSkill
      case type
      when :GROUND
        return true if target.hasActiveAbility?(:WEATHERING) || (target.airborne? && !move.hitsFlyingTargets?)
      when :FIRE
        return true if target.hasActiveAbility?(:FLASHFIRE)
      when :WATER
        return true if target.hasActiveAbility?([:DRYSKIN, :STORMDRAIN, :WATERABSORB, :WEATHERING])
      when :GRASS
        return true if target.hasActiveAbility?([:SAPSIPPER, :COMPOST])
      when :ELECTRIC
        return true if target.hasActiveAbility?([:LIGHTNINGROD, :MOTORDRIVE, :VOLTABSORB])
      when :POISON
        return true if target.hasActiveAbility?(:COMPOST)
      end
  ...
  def pbMoveBaseDamage(move, user, target, skill)
    ...
    when "OHKO", "OHKOIce", "OHKOHitsUndergroundTarget"
      baseDmg = 200
    when "CounterPhysicalDamage", "CounterSpecialDamage", "CounterDamagePlusHalf", "GuardianOfSteel"
      baseDmg = 60
    ...
    when "HitTwoToFiveTimes", "ErupticMeteors"   # Fury Attack
      if user.hasActiveAbility?(:SKILLLINK)
        baseDmg *= 5
      else
        baseDmg = (baseDmg * 31 / 10).floor   # Average damage dealt
      end
    when "HitTwoToThreeTimes"
      if user.hasActiveAbility?(:SKILLLINK)
        baseDmg *= 3
      else
        baseDmg = (baseDmg * 2.5).floor   # Average damage dealt
      end
  ...
  def pbRoughDamage(move, user, target, skill, baseDmg)
    ...
    if skill >= PBTrainerAI.mediumSkill && user.abilityActive?
      ...
      #       round.
      abilityBlacklist = [:ANALYTIC, :SNIPER, :PREDATOR, :TINTEDLENS, :AERILATE, :PIXILATE, :REFRIGERATE]
      canCheck = true

  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "AddDrowsyMistToFoeSide"
      if user.pbOpposingSide.effects[PBEffects::DrowsyMist] >= 2
        ...
        score += 8 * @battle.pbAbleNonActiveCount(user.idxOpposingSide)
        score += [26, 13][user.pbOpposingSide.effects[PBEffects::DrowsyMist]]
      end
    #---------------------------------------------------------------------------
    ...
    when "SwapSideEffects"
      if skill >= PBTrainerAI.mediumSkill
        ...
        bad_effects = [:Spikes, :StickyWeb, :ToxicSpikes, :StealthRock, :DrowsyMist].map! { |e| PBEffects.const_get(e) }
    ...
    when "RemoveUserBindingAndEntryHazards"
      ...
      if @battle.pbAbleNonActiveCount(user.idxOwnSide) > 0
        ...
        score += 80 if user.pbOwnSide.effects[PBEffects::DrowsyMist] > 0

...
#===============================================================================
# Entry hazard. Lays drowsy mist on the opposing side (max. 2 layers).
# (Toxic Spikes)
#===============================================================================
class Battle::Move::AddDrowsyMistToFoeSide < Battle::Move
  def canMagicCoat?; return true; end

  def pbMoveFailed?(user, targets)
    if user.pbOpposingSide.effects[PBEffects::DrowsyMist] >= 2
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
    return false
  ...
  def pbEffectGeneral(user)
    user.pbOpposingSide.effects[PBEffects::DrowsyMist] += 1
    @battle.pbDisplay(_INTL("Drowsy mist spread all around {1}'s feet!",
                            user.pbOpposingTeam(true)))
  end
end
...
class Battle::Move::RemoveUserBindingAndEntryHazards < Battle::Move::StatUpMove
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    if user.pbOwnSide.effects[PBEffects::DrowsyMist] > 0
      user.pbOwnSide.effects[PBEffects::DrowsyMist] = 0

...
#===============================================================================
# Increases the user's Defense, Special Defense and accuracy by 1 stage each. (Metal Post)
#===============================================================================
class Battle::Move::RaiseUserDefSpDefAcc1 < Battle::Move::MultiStatUpMove
  def initialize(battle, move)
    super
    @statUp = [:DEFENSE, 1, :SPECIAL_DEFENSE, 1, :ACCURACY, 1]
  end
end
...
#===============================================================================
# Move depends on user's form. (Springtide Storm)
#===============================================================================
class Battle::Move::SpringtideStorm < Battle::Move
  def pbAdditionalEffect(user, target)
    return false if !user.isSpecies?(:ENAMORUS)
    if user.form == 0
...
class Battle::Move::LowerTargetEvasion1RemoveSideEffects < Battle::Move::TargetStatDownMove
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    return false if targetSide.effects[PBEffects::StealthRock] ||
                    ...
                    targetSide.effects[PBEffects::DrowsyMist] > 0 ||
    ...
    return false if Settings::MECHANICS_GENERATION >= 6 &&
                    ...
                    targetOpposingSide.effects[PBEffects::DrowsyMist] > 0 ||
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.pbOwnSide.effects[PBEffects::DrowsyMist] > 0 ||
        (Settings::MECHANICS_GENERATION >= 6 &&
        target.pbOpposingSide.effects[PBEffects::DrowsyMist] > 0)
      target.pbOwnSide.effects[PBEffects::DrowsyMist]      = 0
      target.pbOpposingSide.effects[PBEffects::DrowsyMist] = 0 if Settings::MECHANICS_GENERATION >= 6
      @battle.pbDisplay(_INTL("{1} blew away drowsy mist!", user.pbThis))
    end
...
#===============================================================================
# For 5 rounds, swaps all battlers' base Attack with base Special Attack.
# (Extortion Room)
#===============================================================================
class Battle::Move::StartSwapAllBattlersBaseOffensiveStats < Battle::Move
  def pbEffectGeneral(user)
    if @battle.field.effects[PBEffects::ExtortionRoom] > 0
      @battle.field.effects[PBEffects::ExtortionRoom] = 0
      @battle.pbDisplay(_INTL("Extortion Room wore off, and the Defense and Sp. Atk stats returned to normal!"))
    else
      @battle.field.effects[PBEffects::ExtortionRoom] = 5
      @battle.pbDisplay(_INTL("It created an ominous area in which the Attack and Sp. Atk stats are swapped!"))
    end
  end

  def pbShowAnimation(id, user, targets, hitNum = 0, showAnimation = true)
    return if @battle.field.effects[PBEffects::ExtortionRoom] > 0   # No animation
    super
  end
end

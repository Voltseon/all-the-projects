...
class Battle::Move::HealUserByTargetAttackLowerTargetAttack1 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.hasActiveAbility?(:LIQUIDOOZE)
      ...
      healAmt = (healAmt * 1.3).floor if user.hasActiveAbility?(:DRAINBOOST)
      @battle.pbDisplay(_INTL("{1}'s HP was restored.", user.pbThis)) if user.pbRecoverHP(healAmt) > 0
    end
  end
end
...
class Battle::Move::HealUserAndAlliesQuarterOfTotalHP < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if @battle.allSameSideBattlers(user).none? { |b| b.canHeal? }
      @battle.pbDisplay(_INTL("But it failed!")) if !user.pbTakeDamageNoxiousPulse
      return true
    end
    return false
  end
...
class Battle::Move::HealTargetHalfOfTotalHP < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}'s HP was restored.", target.pbThis)) if target.pbRecoverHP(hpGain) > 0
  end
end
...
class Battle::Move::HealTargetDependingOnGrassyTerrain < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}'s HP was restored.", target.pbThis)) if target.pbRecoverHP(hpGain)
  end
end

...
#===============================================================================
# Power increases for each consecutive use. (Brandish)
#===============================================================================
class Battle::Move::Brandish < Battle::Move
  def pbChangeUsageCounters(user, specialUsage)
    oldVal = user.effects[PBEffects::Brandish]
    super
    maxMult = 1
    while (@baseDamage << (maxMult - 1)) < 160
      maxMult += 1   # 1-4 for base damage of 20, 1-3 for base damage of 40
    end
    user.effects[PBEffects::Brandish] = (oldVal >= maxMult) ? maxMult : oldVal + 1
  end

  def pbBaseDamage(baseDmg, user, target)
    return baseDmg + user.effects[PBEffects::Brandish] * 30
  end
end
...
#===============================================================================
# User takes recoil damage. Boosts speed stat. (Wave Crash)
#===============================================================================
class Battle::Move::RecoilBoostsSpeed < Battle::Move::RecoilQuarterOfDamageDealt
  def pbRecoilDamage(user, target)
...
#===============================================================================
# User takes recoil damage equal to 1/2 of the damage this move dealt.
# Also has a chance to raise attack stat
# (Giant Punch)
#===============================================================================
class Battle::Move::RaiseUserAttackRecoilHalfOfDamageDealt < Battle::Move::RecoilMove
  def pbRecoilDamage(user, target)
    return (target.damageState.totalHPLost / 2.0).round
  end
...
#===============================================================================
# Type depend on the user's held berry. Destroys the berry.
# (Regurgitate)
#===============================================================================
class Battle::Move::TypeDependOnUserBerryCanConsume < Battle::Move
  def pbMoveFailed?(user, targets)
    # NOTE: Unnerve does not stop a Pokémon using this move.
    item = user.item
    rItem = user.recycleItem
    if (((item && item.is_berry? && user.itemActive?) ||
       item.flags.any? { |f| f[/^NaturalGift_/i] })) || 
       ((rItem && rItem.is_berry? && user.belched?) ||
       (rI.flags.any? { |f| f[/^NaturalGift_/i] } && user.belched?))
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
    return false
  ...
  #       this is good enough.
  def pbBaseType(user)
    item = (user.item.is_berry?  ? user.item : user.recycleItem)
    ret = :NORMAL
    if item
      item.flags.each do |flag|
        next if !flag[/^NaturalGift_(\w+)_(?:\d+)$/i]
  ...
  def pbEndOfMoveUsageEffect(user, targets, numHits, switchedBattlers)
    ...
    # NOTE: There is no item consumption animation.
    user.pbConsumeItem(true, true, false) if user.item
    resetBelch if user.recycleItem
  end
end

class Battle::Move
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    # Frigid Lake
    if user.hasActiveAbility?(:FRIGIDLAKE) && type == :WATER
      if rand(100) < 10
        target.pbFreeze if target.pbCanFreeze?(user, false, self)
      end
    end
    # Volcanic Presence
    if type == :FIRE || @function == "ErupticMeteors"
      if @battle.allBattlers.any? { |b| b.hasActiveAbility?(:VOLCANICPRESENCE) }
        multipliers[:base_damage_multiplier] *= 1.5
      end
    end
    # Mud Sport
    ...
    # Floatation
    if user.hasActiveAbility?(:FLOATATION) && type == :FLYING
      if [:Rain, :HeavyRain].include?(user.effectiveWeather) ||
        @battle.field.effects[PBEffects::WaterSportField] > 0 ||
        @battle.allBattlers.any? { |b| b.effects[PBEffects::WaterSport] }
        multipliers[:base_damage_multiplier] *= 2
      end
    end
    # Terrain moves
    ...
    when :Sun, :HarshSun
      multipliers[:final_damage_multiplier] *= 1.5 if @function == "ErupticMeteors"
      case type
      when :FIRE
    ...
    when :Rain, :HeavyRain
      multipliers[:final_damage_multiplier] /= 2 if @function == "ErupticMeteors"
      case type
      when :FIRE
        multipliers[:final_damage_multiplier] /= 2
      when :WATER

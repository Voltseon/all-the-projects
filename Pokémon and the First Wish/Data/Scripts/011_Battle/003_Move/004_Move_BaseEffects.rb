...
class Battle::Move::HealingMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.hp == user.totalhp
      @battle.pbDisplay(_INTL("{1}'s HP is full!", user.pbThis)) if !user.pbTakeDamageNoxiousPulse
      return true
    end
    return false
  end
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("{1}'s HP was restored.", user.pbThis)) if user.pbRecoverHP(amt) > 0
  end
end
...
class Battle::Move::RecoilMove < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    return if user.hasActiveAbility?(:FULLFORCE)

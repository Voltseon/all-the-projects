...
class Battle::Move::RandomlyDamageOrHealTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if target.pbTakeDamageNoxiousPulse
      return true
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}'s HP was restored.", target.pbThis)) if target.pbRecoverHP(target.totalhp / 4) > 0
  end
...
class Battle::Move::HealAllyOrDamageFoe < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    return if !@healing
    @battle.pbDisplay(_INTL("{1}'s HP was restored.", target.pbThis)) if target.pbRecoverHP(target.totalhp / 2) > 0
  end
...
#===============================================================================
# Deals more damage in Wonder Room and deals 
# damage based on the target's lowest defense stat
# (Wonder Slash)
#===============================================================================
class Battle::Move::PowersUpInWonderRoom < Battle::Move
  ...
  def pbOnStartUse(user, targets)
    # Calculate target's effective attacking value
    ...
    targets.each do |target|
      ...
      realDef    += (defense.to_f * stageMul[defStage] / stageDiv[defStage]).floor
      spDef      = target.spdef
      spDefStage = target.stages[:SPECIAL_DEFENSE] + 6
      realSpDef  += (spAtk.to_f * stageMul[spDefStage] / stageDiv[spDefStage]).floor
    end
    # Determine move's category
    @calcCategory = (realDef < realSpDef) ? 0 : 1
  end
...
#===============================================================================
# Deals more damage in Extortion Room and deals 
# damage based on the user's highest attack stat
# (Extortion Slash)
#===============================================================================
class Battle::Move::PowersUpInExtortionRoom < Battle::Move
  def initialize(battle, move)
    super
    @calcCategory = 1
  end
...
#===============================================================================
# Nausiates the foe. This round, if it tries to heal, it loses 1/2 of its max
# HP instead. (Noxious Pulse)
#===============================================================================
class Battle::Move::TargetNextHealingMoveDamagesTarget < Battle::Move
  def ignoresSubstitute?(user); return true; end
  def canMagicCoat?;            return true; end

  def pbFailsAgainstTarget?(user, target, show_message)
    if target.effects[PBEffects::NoxiousPulse]
      @battle.pbDisplay(_INTL("But it failed!")) if show_message
      return true
    end
    return false
  ...
  def pbEffectAgainstTarget(user, target)
    target.effects[PBEffects::NoxiousPulse] = true
    @battle.pbDisplay(_INTL("{1} is feeling nausious!", user.pbThis))
  end
end
...
# (Counter)
#===============================================================================
class Battle::Move::GuardianOfSteel < Battle::Move::FixedDamageMove
  def pbDisplayChargeMessage(user)
    @battle.pbCommonAnimation("GuardianOfSteel", user)
    @battle.pbDisplay(_INTL("{1} is standing guard!", user.pbThis))
  ...
  def pbFixedDamage(user, target)
    dmg = user.effects[PBEffects::Counter] * 1.5
    dmg = 1 if dmg == 0
    return dmg
  end
end

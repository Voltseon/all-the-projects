...
class AnimationSprite < RPG::Sprite
  ...
  def setCoords
    rawx = (((@tileX * Game_Map::REAL_RES_X) - @map.display_x) / Game_Map::X_SUBPIXELS).ceil + Game_Map::TILE_WIDTH / 2
    self.x = rawx * TilemapRenderer.zoom_x - (TilemapRenderer.zoom_x-1) * Graphics.width/2
    rawy = (((@tileY * Game_Map::REAL_RES_Y) - @map.display_y) / Game_Map::Y_SUBPIXELS).ceil + Game_Map::TILE_HEIGHT
    self.y = rawy * TilemapRenderer.zoom_y - (TilemapRenderer.zoom_y-1) * Graphics.height/2

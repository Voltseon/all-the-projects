...
class Player < Trainer
  ...
  def money
    # Get amount of RELICGOLD in bag
    relicgold = $bag.quantity(:RELICGOLD)
    relicsilver = $bag.quantity(:RELICSILVER)
    reliccopper = $bag.quantity(:RELICCOPPER)
    # Calculate total money
    total = (relicgold * 1000) + (relicsilver * 100) + (reliccopper * 1)
    return total
  end
  ...
  def money=(value)
    # Convert money to RELICGOLD, RELICSILVER, and RELICCOPPER
    relicgold = value / 1000
    relicsilver = (value % 1000) / 100
    reliccopper = (value % 100) / 1
    # Set amount of RELICGOLD, RELICSILVER, and RELICCOPPER in bag
    $bag.remove(:RELICGOLD, $bag.quantity(:RELICGOLD))
    $bag.remove(:RELICSILVER, $bag.quantity(:RELICSILVER))
    $bag.remove(:RELICCOPPER, $bag.quantity(:RELICCOPPER))
    $bag.add(:RELICGOLD, relicgold)
    $bag.add(:RELICSILVER, relicsilver)
    $bag.add(:RELICCOPPER, reliccopper)
  end

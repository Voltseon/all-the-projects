...
class Game_Player < Game_Character
  ...
  SCREEN_CENTER_Y = ((Settings::SCREEN_HEIGHT / 2) - (Game_Map::TILE_HEIGHT / 2) + 8) * Game_Map::Y_SUBPIXELS
  ...
  def set_movement_type(type)
    ...
    when :throwing
      new_charset = pbGetPlayerCharset(meta.throw_charset)
    when :book
      new_charset = pbGetPlayerCharset(meta.book_charset)
    when :surf_fishing
  ...
  #-----------------------------------------------------------------------------
  # * Set Map Display Position to Center of Screen smoothly
  #-----------------------------------------------------------------------------
  def center_smooth(x, y)
    target = [(x * Game_Map::REAL_RES_X) - SCREEN_CENTER_X,(y * Game_Map::REAL_RES_Y) - SCREEN_CENTER_Y]
    until (self.map.display_x-target[0]).abs < 1 && (self.map.display_y-target[1]).abs < 1
      self.map.display_x = Math.lerp(self.map.display_x, target[0], 0.3)
      self.map.display_y = Math.lerp(self.map.display_y, target[1], 0.3)
      Graphics.update
      $scene.update
      target = [(x * Game_Map::REAL_RES_X) - SCREEN_CENTER_X,(y * Game_Map::REAL_RES_Y) - SCREEN_CENTER_Y]
    end
  ...
  def update_command_new
    ...
    unless pbMapInterpreterRunning? || $game_temp.message_window_showing ||
           $game_temp.in_mini_update || $game_temp.in_menu || $scene.active_hud.aiming
  ...
  def update_move
    if !@moved_last_frame || @stopped_last_frame   # Started a new step
      if pbTerrainTag.ice && !$PokemonGlobal&.bicycle
  ...
  def update_screen_position(last_real_x, last_real_y)
    ...
    target = [last_real_x - SCREEN_CENTER_X,last_real_y - SCREEN_CENTER_Y]
    ...
    self.map.display_x = ease_in_out(self.map.display_x, target[0], 0.16)
    self.map.display_y = ease_in_out(self.map.display_y, target[1], 0.16)
=begin

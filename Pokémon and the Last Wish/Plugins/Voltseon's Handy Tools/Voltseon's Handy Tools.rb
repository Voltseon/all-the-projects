...
def vAI(itm, qty=1)
  ...
  #Alternates: vAddItem(), vAddItemSilent(), vItemAdd(), vItemSilent(), vAIS()
end
...
def vHP(pok)
  $Trainer.has_species?(pok.upcase)
...
def vCry(i,vol=80,pch=100,form=0)
  species = GameData::Species.get(i.upcase).species
  Pokemon.play_cry(species, form, vol, pch)
...
def vTSSR(swt,min,max)
  ...
  #Example: vTSSR("A",5,15) - Toggles Self Switch "A" for events 5 through 15
...
# Pokedex: Set Seen | Variables: pok = The Pokemon
def vPDS(pok)
  species = GameData::Species.get(pok.upcase).id
  $Trainer.pokedex.set_seen(species)
  #Example: vPDS("Pikachu") - Sets the given species as seen in the Pokedex
  #Alternatives: vPokedexSeen(), vSetSeen(), vSeenSpecies()
end

# Pokedex: Set Owned | Variables: pok = The Pokemon
def vPDO(pok)
  species = GameData::Species.get(pok.upcase).id
  vPDS(species)
  $Trainer.pokedex.set_owned(species)
  #Example: vPDO("Pikachu") - Sets the given species as owned in the Pokedex
  #Alternatives: vPokedexOwned(), vSetOwned(), vOwnedSpecies()
end

# Play Credits | Variables: none
def vCredits()
  $scene = Scene_Credits.new
  #Example: vCredits() - Plays the credits scene
  #Alternatives: vPlayCredits()
end
...
def pbBattleAnimationOverride(viewport,battletype=0,foe=nil)
  ...
  if (battletype==1 || battletype==3) && foe.length==1   # Against single trainer
    ...
    if tr_type
      ...
      tgraphic    = sprintf("vsTrainer_%s_%d", tr_type.to_s, $PokemonTemp.partner_data[3]) rescue nil if [:POKEMONTRAINER_Leaf, :POKEMONTRAINER_Red].include?(tr_type)
...
class Player < Trainer
  def trainer_type
    ...
    @trainer_type == :POKEMONTRAINER_Red ? @trainer_type = :LW1_M : @trainer_type = :LW1_F
    return @trainer_type
  end
end

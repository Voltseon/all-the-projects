...
#===============================================================================
class PokemonCoinStoreAdapter
  def getMoney
    return $Trainer.coins
  end
  ...
  def pbGetCoinString
    moneyString=""
    begin
      moneyString=_INTL("{1} chips",$Trainer.coins.to_s_formatted)
    rescue
      moneyString=_INTL("0 chips")
    end
    return moneyString
  end
...
# Pokémon CoinStore
#===============================================================================
class Window_PokemonCoinStore < Window_DrawableCommand
...
class PokemonCoinStore_Scene
  ...
  def pbRefresh
    ...
    @sprites["moneywindow"].text = _INTL("Chips:\r\n<r>{1}", @adapter.getMoneyString)
  end
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    @sprites["itemwindow"] = Window_PokemonCoinStore.new(stock, winAdapter,
...
#
#===============================================================================
class PokemonCoinStoreScreen
  def initialize(scene,stock)
    @scene=scene
    @stock=stock
    @adapter=PokemonCoinStoreAdapter.new
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getMoney<price
        pbDisplayPaused(_INTL("You don't have enough chips."))
        next
      end
      if GameData::Item.get(item).is_important?
        if !pbConfirm(_INTL("Certainly. You want {1}. That will be {2} chips. OK?",
        ...
        next if quantity==0
        price*=quantity
        if !pbConfirm(_INTL("{1}, and you want {2}. That will be {3} chips. OK?",
...
#
#===============================================================================
def pbPokemonCoinStore(stock,prices=[],speech=nil,cantsell=false)
  for i in 0...stock.length
  ...
  stock.compact!
  scene = PokemonCoinStore_Scene.new
  screen = PokemonCoinStoreScreen.new(scene,stock)
  screen.pbBuyScreen

...
#-------------------------------------------------------------------------------
# Berries Ready Message Hud component
#-------------------------------------------------------------------------------
class BerriesReadyHud < Component
  def initialize
    @counter = 0
  end
  ...
  def refresh
    ...
    for pot in $Trainer.berry_pots
      next if pot[0] == nil
      numPots += 1 if pot[1] < 1
    end
    if numPots > 0
      text = _INTL("{1} crop{2} {3} ready to harvest!",numPots, numPots == 1 ? "" : "s", numPots == 1 ? "is" : "are")
      pbSetSmallFont(@sprites["overlay"].bitmap)
      pbDrawTextPositions(@sprites["overlay"].bitmap,[[text,Graphics.width/2 - 8, 0,1,@baseColor,@shadowColor]])
    end
  end

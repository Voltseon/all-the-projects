...
class BattleIntroAnimation < PokeBattle_Animation
  ...
  def createProcesses
    appearTime = 15   # This is in 1/20 seconds
    # Player sprite, partner trainer sprite
    @battle.player.each_with_index do |_p,i|
      makeSlideSprite("player_#{i+1}",@battle.trainerBattle? ? 1 : -1,appearTime,PictureOrigin::Bottom)
    end
    ...
    for i in 0...@battle.battlers.length
      makeSlideSprite("shadow_#{i}",((i%2)==0) ? 1 : -1,appearTime,PictureOrigin::Center) if (@battle.battlers[i] && !@battle.battlers[i].opposes?(0))
    end

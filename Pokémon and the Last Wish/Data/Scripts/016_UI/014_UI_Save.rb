...
class PokemonSave_Scene
  def pbStartScreen
    ...
    @viewport.z=99999
    @sprites={}
    @sprites["iamjustacheck"]=Window_AdvancedTextPokemon.new("")
    @sprites["iamjustacheck"].visible = false
    bluetext = ["0070F8","78B8E8"]
    redtext = ["E82010","F8A8B8"]
    greentext = ["209808","90F090"]
    if isDarkWindowskin(@sprites["iamjustacheck"].windowskin)
      bluetext = bluetext.reverse
      redtext = redtext.reverse
      greentext = greentext.reverse
    end
    ...
    themeamount = 0
    themeamount = $game_variables[51].length if $game_variables[51].is_a?(Array)
    textColor = ["#{bluetext[0]},#{bluetext[1]}","#{redtext[0]},#{redtext[1]}","#{bluetext[0]},#{bluetext[1]}"][$Trainer.gender]
    locationColor = "#{greentext[0]},#{greentext[1]}"   # green
    ...
    loctext+=_INTL("Themes<r><c3={1}>{2}</c3><br>",textColor,themeamount)

...
class PokemonMartAdapter
  ...
  def addItem(item,qt=1)
    return $PokemonBag.pbStoreItem(item,qt)
  end
      ...
      if added!=quantity
        ...
        if $PokemonBag
          if quantity>=10 && GameData::Item.get(item).is_poke_ball? && GameData::Item.exists?(:PREMIERBALL)
            premier_amt = (quantity/10).floor
            if @adapter.addItem(GameData::Item.get(:PREMIERBALL), premier_amt)
              pbDisplayPaused(_INTL("I'll throw in {1} Premier Ball{2}, too.", (premier_amt>1) ? "some (#{premier_amt})" : "a", (premier_amt>1) ? "s" : ""))
            end
          end
        end
      end

  ...
  def refresh
    if @refreshBox
      ...
      pbDrawShadowText(@contents,xval,8,widthval,32,
         boxname,Color.new(248,248,248),Color.new(72,72,72))
      @refreshBox = false
    end
    yval = self.y+30
...
class PokemonBoxPartySprite < SpriteWrapper
  ...
  def refresh
    ...
    pbDrawTextPositions(self.bitmap,[
       [_INTL("Back"), 86, 240, 2, Color.new(248, 248, 248), Color.new(72, 72, 72), 1]
    ])
...
class PokemonStorageScene
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    buttonshadow = Color.new(72,72,72)
    ...
    if !pokemon.egg?
      imagepos = []
      if pokemon.male?
        textstrings.push([_INTL("♂"),148,2,false,Color.new(24,112,216),Color.new(72,72,72)])
      elsif pokemon.female?
        textstrings.push([_INTL("♀"),148,2,false,Color.new(248,56,32),Color.new(72,72,72)])
      end
  ...
  def pbRelease(selected,heldpoke)
    ...
    if command==1
      ...
      if !pokemon.isSpecies?(:JIRACHI)
        ...
        pbDisplay(_INTL("{1} refused to go!",pkmnname))
      end
    end
    return
  end

...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    if @pokemon.type1==@pokemon.type2
      overlay.blt(400,78,@typebitmap.bitmap,type1rect)
    else
      overlay.blt(366,78,@typebitmap.bitmap,type1rect)
      overlay.blt(436,78,@typebitmap.bitmap,type2rect)
    end
    textpos=[
       [_INTL("Teach which move?"),16,2,0,Color.new(248,248,248),Color.new(72,72,72)]
    ]
    imagepos=[]
    yPos=76
    for i in 0...VISIBLEMOVES
      ...
      if moveobject
        ...
        textpos.push([moveData.name,80,yPos,0,Color.new(248,248,248),Color.new(72,72,72)])
        if moveData.total_pp>0
          textpos.push([_INTL("PP"),112,yPos+32,0,Color.new(248,248,248),Color.new(72,72,72)])
          textpos.push([_INTL("{1}/{1}",moveData.total_pp),230,yPos+32,1,
             Color.new(248,248,248),Color.new(72,72,72)])
        else
          textpos.push(["-",80,yPos,0,Color.new(248,248,248),Color.new(72,72,72)])
          textpos.push(["--",228,yPos+32,1,Color.new(248,248,248),Color.new(72,72,72)])
        end
      end
      yPos+=64
    end
    ...
    textpos.push([_INTL("Category"),272,108,0,Color.new(248,248,248),Color.new(72,72,72)])
    textpos.push([_INTL("Power"),272,140,0,Color.new(248,248,248),Color.new(72,72,72)])
    textpos.push([basedamage<=1 ? basedamage==1 ? "???" : "---" : sprintf("%d",basedamage),
          468,140,2,Color.new(248,248,248),Color.new(72,72,72)])
    textpos.push([_INTL("Accuracy"),272,172,0,Color.new(248,248,248),Color.new(72,72,72)])
    textpos.push([accuracy==0 ? "---" : "#{accuracy}%",
          468,172,2,Color.new(248,248,248),Color.new(72,72,72)])
    ...
    if @sprites["commands"].index>0
      imagepos.push(["Graphics/Pictures/reminderButtons",124,350,76,0,76,32])
    end

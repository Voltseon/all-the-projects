...
ItemHandlers::UseInField.add(:EXPCAPSULE,proc { |item|
  if $Trainer.expcapsule == 0
    pbMessage(_INTL("Exp. Capsule is empty."))
  else
    pbMessage(_INTL("Currently holding: {1} Exp.",$Trainer.expcapsule.to_s_formatted))
  end
  if pbConfirmMessage("Disable Exp Capsule?")
    $PokemonBag.pbChangeItem(:EXPCAPSULE,:EXPCAPSULEOFF)
    pbMessage(_INTL("The Exp Capsule was disabled."))
    next 1
  else
    next 0
  end
...
ItemHandlers::UseInField.add(:EXPCAPSULEOFF,proc { |item|
  if $Trainer.expcapsule == 0
    pbMessage(_INTL("Exp. Capsule is empty."))
  else
    pbMessage(_INTL("Currently holding: {1} Exp.",$Trainer.expcapsule.to_s_formatted))
  end
  if pbConfirmMessage("Enable Exp Capsule?")
    $PokemonBag.pbChangeItem(:EXPCAPSULEOFF,:EXPCAPSULE)
    pbMessage(_INTL("The Exp Capsule was enabled."))
    next 1
  else
    next 0
  end
...
#-------------------------------------------------------------------------------
# Attribute in Player to save the exp capsule in the save file
#-------------------------------------------------------------------------------
class Player < Trainer
	attr_writer :expcapsule

	def expcapsule
		@expcapsule = 0 if !@expcapsule
		return @expcapsule
	end
end
...
class PokeBattle_Battle
  ...
  def pbGainExp
    ...
    expCapsule = (GameData::Item.exists?(:EXPCAPSULE) && $PokemonBag.pbHasItem?(:EXPCAPSULE))
    p1 = pbParty(0)
    @battlers.each do |b|
      ...
      if numPartic>0 || expShare.length>0 || expAll
        ...
        eachInTeam(0,0) do |pkmn,i|
          ...
          pbGainEVsOne(i,b)
          pbGainExpOne(i,b,numPartic,expShare,expCapsule,expAll)
        end
        ...
        if expAll
          ...
          eachInTeam(0,0) do |pkmn,i|
            ...
            pbDisplayPaused(_INTL("Your party Pokémon in waiting also got Exp. Points!")) if showMessage && !expCapsule
            pbDisplayPaused(_INTL("Your party Pokémon in waiting's Exp. Points were also added to the Exp Capsule!")) if showMessage && expCapsule
            showMessage = false
            pbGainEVsOne(i,b)
            pbGainExpOne(i,b,numPartic,expShare,expCapsule,expAll,false)
          end
        end
      end
  ...
  def pbGainExpOne(idxParty,defeatedBattler,numPartic,expShare,expCapsule,expAll,showMessages=true)
    pkmn = pbParty(0)[idxParty]   # The Pokémon gaining EVs from defeatedBattler
    ...
    # Exp Capsule
    if expCapsule
      $Trainer.expcapsule += expGained
      expGained = 0
      pbDisplayPaused(_INTL("{1}'s Exp. Points were added to the Exp Capsule!",pkmn.name)) if showMessages
      return
    end
    # "Exp gained" message
    if showMessages
...
echoln("Loaded plugin: EXP Capsule") if Essentials::VERSION != "19.1"
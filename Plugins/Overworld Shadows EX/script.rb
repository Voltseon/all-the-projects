  ...
  def initialize(sprite, event, viewport = nil)
    ...
    if !defined?(Game_FollowingPkmn) || !@event.is_a?(Game_FollowingPkmn)
      if @event != $game_player
        ...
        name = ($player.planet != nil) ? OWShadowSettings::PLAYER_SHADOW_FILENAME + "_#{$player.planet.id.to_s}" : OWShadowSettings::PLAYER_SHADOW_FILENAME
      end
    end
    name = OWShadowSettings::DEFAULT_SHADOW_FILENAME if nil_or_empty?(name) || !pbResolveBitmap("Graphics/Characters/Shadows/" + name)
...
def get_player_shadow
  return $scene.spritesetGlobal.playersprite.ow_shadow
end

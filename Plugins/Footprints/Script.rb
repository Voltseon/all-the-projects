...
class Game_Character
  ...
  def get_last_pos
	...
	def foot_prints? = $game_map.terrain_tag(get_last_pos[0], get_last_pos[1]) == FootprintVariables::TERRAIN_FOOT && $scene.is_a?(Scene_Map) && $scene.spriteset? && $game_map.get_tile(get_last_pos[0], get_last_pos[1], 1) == 0 && $game_map.get_tile(get_last_pos[0], get_last_pos[1], 2) == 0
...
class Footsprite
  def initialize(sprite,event,viewport,map,direction,nid,chardata,player)
    ...
    @sprite  = Sprite.new(viewport)
		...
		@sprite.bitmap = RPG::Cache.load_bitmap("Graphics/Pictures/Footprints/", file)
		# Set position

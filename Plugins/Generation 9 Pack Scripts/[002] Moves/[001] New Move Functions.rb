...
class Battle::Move::LowerTargetDefense1FlinchTarget < Battle::Move
  ...
  def pbAdditionalEffect(user, target)
    return if target.damageState.substitute
    chance = ($player.murphyslaw ? user.idxOwnSide == 0 ? 0 : 100 : pbAdditionalEffectChance(user, target, 50))
...
#===============================================================================
class Battle::Move::IncreasePowerWhileElectricTerrain < Battle::Move
  def pbBaseDamage(baseDmg, user, target)

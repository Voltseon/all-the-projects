...
class PokemonPartyHud < Component
  def startComponent(viewport)
    ...
    @shadowbar = AnimatedBitmap.new(MENU_FILE_PATH + "overlayShadow")
    ...
    @shadowbmp = Bitmap.new(MENU_FILE_PATH + "overlayShadowIcon")
  end
  ...
  def refresh
    ...
    for i in 0...$player.party.length
      ...
      # EXP / Heart Gauge
      if pokemon.shadowPokemon?
        if pokemon.heart_gauge > 0
          w = (pokemon.heart_gauge.to_f * 24 * 1.0)/pokemon.max_gauge_size
          w = 1 if w < 1.0
          w = 0 if w.is_a?(Float) && w.nan?
          w = ((w/2).round) * 2 if w > 0
          shadowrect = Rect.new(0, 0, w, 2)
          @sprites["overlay"].bitmap.blt(spacing + (Graphics.width/8) + 22, Graphics.height/2 - 94, @shadowbar.bitmap, shadowrect)
        end
      else
        if pokemon.exp>0
...
class DateAndTimeHud < Component
  ...
  def refresh
    text = pbGetSeasonName(pbGetSeason)

...
FRAMEWIDTH_M = 88              # width of each Male frame           
FRAMEHEIGHT_M = 148            # height of each Male frame

FRAMEWIDTH_F = 88              # width of each Female frame           
FRAMEHEIGHT_F = 148            # height of each Female frame
...
#===============================================================================
  def selection
    ...
    if !Kernel.pbConfirmMessage(_INTL("\\bSo, that's how you want to look?"))
      Kernel.pbMessageDisplay(@objectsList["msgwindow"],
        _INTL("\\bI suppose you'll have to give it a second shot then!"))
      17.times do
        Graphics.update
      ...
      Kernel.pbMessageDisplay(@objectsList["msgwindow"],
        _INTL("Select an outfit.\\wtnp[0]"))
      @objectsList["msgwindow"].visible=true
    else
      pbChangePlayer(@select)
      if @select == 0
        $game_variables[26]="boy"
      else
        $game_variables[26]="girl"
      end
      Kernel.pbMessageDisplay(@objectsList["msgwindow"],
        _INTL("\\bExcellent!"))
      Kernel.pbMessageDisplay(@objectsList["msgwindow"],
        _INTL("\\bA perfectly acceptable outfit for a perfectly outstanding {1}!",$game_variables[26]))
      @objectsList["msgwindow"].visible=false
      if $game_variables[26]="boy"
        $game_variables[26]=0
      else
        $game_variables[26]=1
      end

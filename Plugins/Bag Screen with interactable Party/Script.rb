      ...
      if !@pokemon.egg?
        ...
        if @pokemon.shadowPokemon?
          pbDrawImagePositions(@overlaysprite.bitmap,
                               [["Graphics/Pictures/shadow", 76, 44, 0, 0, 16, 16]])
        elsif @pokemon.shiny?
          pbDrawImagePositions(@overlaysprite.bitmap,
    ...
    if @bag.last_viewed_pocket == 1 && item #Items Pocket
      ...
      if itm.is_machine?
        ...
        $player.party.each_with_index do |pkmn, i|
          if pkmn.egg? || pkmn.shadowPokemon? || $player.metronome
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if cmdRead >= 0 && command == cmdRead   # Read mail
        ...
        elsif itm.is_important? && !itm.is_mega_stone? && !itm.is_memory?
...
def pbBagUseItem(bag, item, scene, screen, chosen, bagscene=nil)
  ...
  if itm.is_machine?    # TM, HM or TR
    ...
    if $player.metronome
      pbMessage(_INTL("Only Metronome is allowed!")) { screen.pbUpdate }
    elsif pkmn.egg?

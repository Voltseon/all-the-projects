...
class Game_Follower
  ...
  if !method_defined?(:show_dust_animation)
    def show_dust_animation
      ...
      spriteset&.addUserAnimation(($PokemonGlobal&.diving ? Settings::DUST_ANIMATION_DIVE_ID : Settings::DUST_ANIMATION_ID), self.x, self.y, true, 1)
    end
  end
end
...
class Game_FollowerFactory
  ...
  def move_followers
    leader = $game_player
    ((leader.move_speed/1.5).round).times { pbWait(1) } unless leader.moved_last_frame || (@events.length > 0 && @events[0].moving?)

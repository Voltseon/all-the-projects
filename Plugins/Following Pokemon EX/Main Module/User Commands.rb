module FollowingPkmn
  ...
  def self.talk
    ...
    random_val = rand(6)
    if pbIsMountable($player.first_able_index)
      pbMountPkmn(0, false) if pbConfirmMessage("Would you like to mount #{first_pkmn.name}?")
    else
      if $PokemonGlobal&.follower_hold_item
        EventHandlers.trigger_2(:following_pkmn_item, first_pkmn, random_val)
      elsif $player.hatemode
        pbMessage(_INTL("{1} doesn't want to talk to {2}!", first_pkmn.name, $player.name))
      elsif $player.lovemode
        pbMessage(_INTL("{1} loves walking with {2}!", first_pkmn.name, $player.name))
      else

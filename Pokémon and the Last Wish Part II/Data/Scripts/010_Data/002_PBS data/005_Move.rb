module GameData
  class Move
    ...
    def type
      return GameData::Type.get($Trainer.custom_move[1]) if @id == :CUSTOMMOVE
      return @type
    end

    def category
      return $Trainer.custom_move[2] if @id == :CUSTOMMOVE
      return @category
    end

    # @return [String] the translated name of this move
    def name
      return $Trainer.custom_move[0] if @id == :CUSTOMMOVE
      return pbGetMessage(MessageTypes::Moves, @id_number)
    end

    # @return [String] the translated description of this move
    def description
      return $Trainer.custom_move[3] if @id == :CUSTOMMOVE
      return pbGetMessage(MessageTypes::MoveDescriptions, @id_number)
    end

    def physical?
      return $Trainer.custom_move[2]==0 if @id == :CUSTOMMOVE
    ...
    def special?
      return $Trainer.custom_move[2]==1 if @id == :CUSTOMMOVE
      return false if @base_damage == 0

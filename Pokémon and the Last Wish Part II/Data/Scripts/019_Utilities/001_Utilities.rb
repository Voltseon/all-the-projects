...
def pbGetRegionalNumber(region, species)
  ...
  dex_list.each_with_index do |s, index|
    return index + 1 if GameData::Species.get(s).real_name == GameData::Species.get(species).real_name
  end
  return 0
end
...
def pbCameraCapture
  ...
  filestart = t.strftime("[%Y-%m-%d] %H_%M_%S.%L")
  Dir.mkdir("Screenshots/") if !FileTest.directory?("Screenshots/")
  Dir.mkdir("Screenshots/Taken By Cameron/") if !FileTest.directory?("Screenshots/Taken By Cameron/")
  filestart = "./Screenshots/Taken By Cameron/" + $game_map.name + " - " + filestart + ".png"
  Graphics.screenshot(filestart)
end

class Game_Character
  ...
  def move_speed=(val)
    ...
    self.move_speed_real = (val==9) ? 80 :(val==8) ? 20 :(val == 7) ? 28.8 :(val == 6) ? 64 : (val == 5) ? 32 : (2 ** (val + 1)) * 0.8
  end
  ...
  def screen_z(height = 0)
    ...
    if @tile_id > 0
      ...
      if self != $game_player
        if self.name[/weeds/i] || self.name[/groundchips/i]
          return z
        end
  ...
  def playermoveto(x, y)
    ...
    $PokemonGlobal.dependentEvents.each_with_index do |event,i|
      ...
      $PokemonTemp.dependentEvents.realEvents[i].moveto(newx, newy)
    end
    triggerLeaveTile
  end

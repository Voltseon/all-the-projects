class Game_Player < Game_Character
  ...
  def pbCanRun?
    ...
    return input && $Trainer.has_running_shoes && !jumping? &&
       ...
       !$PokemonGlobal.scuba && !$PokemonGlobal.extremespeed
  end
  ...
  def character_name
    ...
    if !@move_route_forcing && $Trainer.character_ID>=0
      ...
      if meta && !$PokemonGlobal.bicycle && !$PokemonGlobal.diving && !$PokemonGlobal.surfing && !$PokemonGlobal.scuba && !$PokemonGlobal.extremespeed
  ...
  def update_command
    if $PokemonGlobal.extremespeed
      self.move_speed = 9   # Extreme Speed
    elsif $game_player.pbTerrainTag.ice || $PokemonGlobal.surfing
      self.move_speed = 5     # Sliding on ice, surfing
    elsif !moving? && !@move_route_forcing && $PokemonGlobal
      if $PokemonGlobal.bicycle
        self.move_speed = 5   # Cycling
      elsif $PokemonGlobal.scuba
        self.move_speed = 8   # Scuba
      elsif pbCanRun?
        self.move_speed = 7   # Running
      else
  ...
  def update_pattern
    if $PokemonGlobal.surfing || $PokemonGlobal.diving
      p = (Graphics.frame_count%60)*@@bobFrameSpeed
      @pattern = p.floor if !@lock_pattern
      @pattern_surf = ((p*4).floor)%4
      @bob_height = (p.floor>=2) ? 2 : 0
    else
      @bob_height = 0
      super
    end

...
class Game_Map
  ...
  def passable?(x, y, d, self_event = nil)
    ...
    for i in [2, 1, 0]
      ...
      # Extreme Speed check
      elsif self_event != nil && $PokemonGlobal.extremespeed
        for j in [2, 1, 0]
          facing_tile_id = data[newx, newy, j]
          return false if facing_tile_id == nil
          facing_terrain = GameData::TerrainTag.try_get(@terrain_tags[facing_tile_id])
          return facing_terrain.extreme_speed
        end
      end
  ...
  def playerPassable?(x, y, d, self_event = nil)
    ...
    for i in [2, 1, 0]
      ...
      passage = @passages[tile_id]
      if terrain
        # Ignore extreme speed if using it
        return true if terrain.extreme_speed && $PokemonGlobal.extremespeed
        ...
        # Can't jump over bounce if not active
        return true if terrain.bounce && $PokemonMap.bounceUsed
  ...
  def bush?(x,y)
    for i in [2, 1, 0]
      tile_id = data[x, y, i]
      return false if tile_id == nil || GameData::TerrainTag.try_get(@terrain_tags[tile_id]).bridge &&

class PokeBattle_Battler
  ...
  def pbCanInflictStatus?(newStatus,user,showMessages,move=nil,ignoreStatus=false)
    ...
    # Spirit Candle immunity
    if self.hasActiveItem?(:SPIRITCANDLE)
      @battle.pbDisplay(_INTL("{1}'s Spirit Candle protected it from being statused!",pbThis)) if showMessages
      return false
    end
    return true
  end

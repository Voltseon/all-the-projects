...
BattleHandlers::SpeedCalcItem.add(:POLISHER,
...
BattleHandlers::AccuracyCalcUserItem.add(:ACTIVATEDCHARCOAL,
  proc { |item,mods,user,target,move,type|
    mods[:accuracy_multiplier] *= 1.3 if (user.isSpecies?(:SABLEYE) && user.form != 0) || (user.isSpecies?(:MAWILE) && user.form != 0)
  }
)
...
BattleHandlers::AccuracyCalcTargetItem.add(:ACTIVATEDCHARCOAL,
  proc { |item,mods,user,target,move,type|
    mods[:accuracy_multiplier] *= 0.7 if target.isSpecies?(:SABLEYE_1) || target.isSpecies?(:MAWILE_1)
  }
)
...
BattleHandlers::DamageCalcUserItem.add(:WAXCANDLE,
  proc { |item,user,target,move,mults,baseDmg,type|
    if !move.is_a?(PokeBattle_Confusion)
...
BattleHandlers::DamageCalcUserItem.add(:ANCIENTNECKLACE,
  proc { |item,user,target,move,mults,baseDmg,type|
    if user.isSpecies?(:SPIRITOMB_1) && move.specialMove?
...
BattleHandlers::DamageCalcTargetItem.add(:ANCIENTNECKLACE,
  proc { |item,user,target,move,mults,baseDmg,type|
    mults[:defense_multiplier] *= 1.5 if move.specialMove? && user.isSpecies?(:SPIRITOMB_1)
  }
)
...
BattleHandlers::DamageCalcTargetItem.add(:MUDPLATING,
  proc { |item,user,target,move,mults,baseDmg,type|
    if target.isSpecies?(:QUAGSIRE) && !move.specialMove?
      mults[:defense_multiplier] *= 1.5
      mults[:attack_multiplier] *= 1.5
    end
  }
...
BattleHandlers::DamageCalcTargetItem.add(:MANGANESE,
  proc { |item,user,target,move,mults,baseDmg,type|
    if target.isSpecies?(:FLOATZEL)
      mults[:defense_multiplier] *= 1.5
    end
  }
)
...
BattleHandlers::CriticalCalcUserItem.add(:METALBARB,
  proc { |item,user,target,c|
    next c+2 if user.isSpecies?(:CACTURNE_1)
  }
)
...
BattleHandlers::UserItemAfterMoveUse.add(:WAXCANDLE,
  proc { |item,user,targets,move,numHits,battle|
    next if numHits==0
    hitBattler = false
    targets.each do |b|
      isAlive = true if !b.fainted?
      break if isAlive
    end
    next if !isAlive
    targets.each do |b|
    ...
    next if !hitBattler
    next false if !user.pbCanLowerStatStage?(:SPEED,user)
    itemName = GameData::Item.get(item).name
    next user.pbLowerStatStageByCause(:SPEED,1,user,itemName)
  }
)
...
BattleHandlers::EOREffectItem.add(:SPIRITCANDLE,
  proc { |item,battler,battle|
    next if !battler.takesIndirectDamage?
    PBDebug.log("[Item triggered] #{battler.pbThis}'s #{battler.itemName} (recoil)")
    battler.pbReduceHP(battler.totalhp/16)
    battle.pbDisplay(_INTL("{1} lost some of its HP!",battler.pbThis))
    battler.pbItemHPHealCheck
    battler.pbFaint if battler.fainted?
  }
)
...
BattleHandlers::ItemOnSwitchIn.add(:MAGNETICRING,
  proc { |item,battler,battle|
    battle.pbDisplay(_INTL("{1} started levitating!",
       battler.pbThis)) if (battler.isSpecies?(:MANKEY) || battler.isSpecies?(:PRIMEAPE)) && battler.form!=0
  }
)

...
BattleHandlers::PriorityChangeAbility.add(:QUICKFIST,
  proc { |ability,battler,move,pri|
...
BattleHandlers::TargetAbilityOnHit.copy(:IRONBARBS,:ROUGHSKIN,:ROUGHSHELL)

BattleHandlers::TargetAbilityOnHit.add(:TAINTEDBODY,
  proc { |ability,user,target,move,battle|
    next if !move.pbContactMove?(user)
    battle.pbShowAbilitySplash(target)
    if user.takesIndirectDamage?(PokeBattle_SceneConstants::USE_ABILITY_SPLASH) &&
...
BattleHandlers::TargetAbilityOnHit.add(:WEATHERING,
  proc { |ability,user,target,move,battle|
    next if ![:GROUND,:WATER].include?(move.calcType)
...
BattleHandlers::UserAbilityOnHit.add(:STATICVEINS,
  proc { |ability,user,target,move,battle|
    next if !move.contactMove?
    next if battle.pbRandom(100)>=30
    battle.pbShowAbilitySplash(user)
...
BattleHandlers::EORWeatherAbility.add(:WEATHERING,
  proc { |ability,weather,battler,battle|
    next unless [:Rain, :HeavyRain, :Hail, :Sandstorm, :StrongWinds].include?(weather)
    battle.pbShowAbilitySplash(battler)
    battler.pbRaiseStatStageByAbility(:SPEED,1,battler,false)
...
BattleHandlers::EORGainItemAbility.copy(:HARVEST,:ENIGMAFARM)

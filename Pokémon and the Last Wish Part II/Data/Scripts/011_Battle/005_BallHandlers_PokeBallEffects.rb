...
BallHandlers::ModifyCatchRate.add(:WISHBALL,proc { |ball,catchRate,battle,battler,ultraBeast|
  if battler.isSpecies?(:JIRACHI)
    catchRate *= 3.5
  end
  next catchRate
})
...
BallHandlers::ModifyCatchRate.add(:REVIVALBALL,proc { |ball,catchRate,battle,battler,ultraBeast|
  catchRate *= 2
  if battler.isSpecies?(:CRANIDOS) || battler.isSpecies?(:RAMPARDOS) ||
    battler.isSpecies?(:SHIELDON) || battler.isSpecies?(:BASTIODON) || battler.isSpecies?(:RELICANTH)
    catchRate *= 2.5
  end
  next catchRate
})

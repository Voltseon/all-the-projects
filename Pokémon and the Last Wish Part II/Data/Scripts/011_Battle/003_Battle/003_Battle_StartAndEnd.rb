class PokeBattle_Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 1
        $DiscordRPC.state = "Battling wild " + foeParty[0].name
        $DiscordRPC.update
        pbDisplayPaused(_INTL("Oh! A wild {1} appeared!",foeParty[0].name))
      when 2
        $DiscordRPC.state = "Battling wild " + foeParty[0].name + " and " + foeParty[1].name
        $DiscordRPC.update
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      when 0 then tMoney*=1.2
      when 2 then tMoney*=0.8
      when 3 then tMoney*=0.7
      end
      tMoney.round
      oldMoney = pbPlayer.money
      pbPlayer.money += tMoney.round
    ...
    if @field.effects[PBEffects::PayDay]>0
      ...
      oldMoney = pbPlayer.money
      pbPlayer.money += @field.effects[PBEffects::PayDay].round
      moneyGained = pbPlayer.money-oldMoney
      if moneyGained>0
  ...
  def pbLoseMoney
    ...
    # Difficulty Modifier
    case $PokemonSystem.difficulty
    when 0 then tMoney*=0.9
    when 2 then tMoney*=2
    when 3 then tMoney*=2.5
    end

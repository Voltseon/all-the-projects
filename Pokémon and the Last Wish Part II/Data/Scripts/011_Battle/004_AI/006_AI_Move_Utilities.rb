class PokeBattle_AI
  ...
  def pbRoughDamage(move,user,target,skill,baseDmg)
    ...
    if move.specialMove?(type) && move.function!="122" && move.function!="176"  # Psyshock & Magnet Wave
    ...
    if skill>=PBTrainerAI.mediumSkill && user.itemActive?
      ...
      #       round.
      itemBlacklist = [:EXPERTBELT,:LIFEORB,:WAXCANDLE]
    ...
    if skill>=PBTrainerAI.mediumSkill
      ...
      when :Sandstorm
        if target.pbHasType?(:ROCK) && move.specialMove?(type) && move.function != "122" && move.function!="176"  # Psyshock & Magnet Wave

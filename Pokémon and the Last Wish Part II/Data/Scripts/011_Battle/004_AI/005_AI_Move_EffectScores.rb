  ...
  def pbGetMoveScoreFunctionCode(score,move,user,target,skill=100)
    ...
    when "118"
      if @battle.field.effects[PBEffects::Gravity]>0
        ...
        score -= 20 if user.hasActiveItem?(:MAGNETICRING) && (user.isSpecies?(:MANKEY) || user.isSpecies?(:PRIMEAPE)) && user.form!=0
        ...
        score += 20 if target.hasActiveItem?(:MAGNETICRING) && (target.isSpecies?(:MANKEY) || target.isSpecies?(:PRIMEAPE)) && target.form!=0
      end
    #---------------------------------------------------------------------------
    when "119"
    ...
    when "125"
      ...
      if hasUnusedMoves
        (skill>=PBTrainerAI.mediumSkill) ? score-=90 : score-=40
      else
        (skill>=PBTrainerAI.mediumSkill) ? score+=20 : score+=10
      end
    #---------------------------------------------------------------------------
    when "126"

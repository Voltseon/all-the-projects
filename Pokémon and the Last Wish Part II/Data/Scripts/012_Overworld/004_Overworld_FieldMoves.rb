...
def pbCheckHiddenMoveBadge(badge=-1,showmsg=true)
  ...
  pbMessage(_INTL("Sorry, a new Stamp is required.")) if showmsg
  return false
end

def pbCheckHiddenMoveSwitch(badge=-1,showmsg=true)
  return true if badge<0   # No badge requirement
  return true if $DEBUG
  if $game_switches[badge]
    return true
  end
  pbMessage(_INTL("Sorry, a new Stamp is required.")) if showmsg
  return false
...
# Fly
#===============================================================================
HiddenMoveHandlers::CanUseMove.add(:PSYCHOMATTER,proc { |move,pkmn,showmsg|
  if $game_player.pbHasDependentEvents?
    pbMessage(_INTL("It can't be used when you have someone with you.")) if showmsg
    next false
  end
...
HiddenMoveHandlers::UseMove.add(:PSYCHOMATTER,proc { |move,pokemon|
  ...
  pbFadeOutIn {
    ...
    FollowingPkmn.move_route([PBMoveRoute::Left,PBMoveRoute::TurnDown],true)
  }
  pbEraseEscapePoint
  next true
})

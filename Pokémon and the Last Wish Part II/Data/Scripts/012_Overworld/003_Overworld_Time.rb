...
module PBDayNight
  ...
  # Used for gatekeepers in route gates
  def self.greet(name1,name2,vertical=false)
    time = pbGetTimeNow if !time
    greeting = "Good night!"
    ...
    elsif (time.hour>=17 && time.hour<20)
      greeting = "Good evening!"
    end
    if !vertical
      pbMessage(_INTL("\\xn[Gatekeeper]\\b{1}\\nTo the west is {2}.\\nTo the east is {3}.",greeting,name1,name2))
    else
      pbMessage(_INTL("\\xn[Gatekeeper]\\b{1}\\nTo the north is {2}.\\nTo the south is {3}.",greeting,name1,name2))
    end
  end

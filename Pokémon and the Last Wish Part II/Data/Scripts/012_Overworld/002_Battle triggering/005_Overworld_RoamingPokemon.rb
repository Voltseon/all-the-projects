...
Events.onMapChange += proc { |_sender,e|
  ...
  Settings::ROAMING_SPECIES.each_with_index do |pkmn, i|
    ...
    if $PokemonGlobal.roamPosition[i] != $game_map.map_id
...
EncounterModifier.register(proc { |encounter|
  ...
  next encounter if rand(100) < 50   # 50% chance of encountering a roaming Pokémon
...
def pbRoamingPokemonBattle(species, level)
  ...
  case decision
  when 1
    $PokemonGlobal.roamPokemon[idxRoamer] = nil
  when 4
    $PokemonGlobal.roamPokemon[idxRoamer]       = true
    $PokemonGlobal.roamPokemonCaught[idxRoamer] = true
  end
  ...
  return (decision!=2 && decision!=5 && decision!=1)
end

...
Events.onStepTaken += proc {
  next if $PokemonGlobal.extremespeed
...
def pbOnStepTaken(eventTriggered)
  ...
  repel_active = ($PokemonGlobal.repel > 0) || $PokemonBag.pbHasItem?(:REPELCHARM)
...
def pbEventCanReachPlayer?(event, player, distance)
  return false if $PokemonGlobal.extremespeed
...
def pbSetPokemonCenter(override=[],map_id=$game_map.map_id)
  ...
  $PokemonGlobal.pokecenterX         = override[0] if override != []
  $PokemonGlobal.pokecenterY         = override[1] if override != []
  $PokemonGlobal.pokecenterDirection = override[2] if override != []
end
...
def pbReceiveItem(item,quantity=1)
  ...
  move = item.move
  meName = (item.is_key_item?) ? "Key item get" : "Item get"
  meName = "DPPT 103 Get Accessory" if item.is_outfit?
  if item == :LEFTOVERS
    pbMessage(_INTL("\\me[{1}]You obtained some \\c[1]{2}\\c[0]!\\wtnp[30]",meName,itemname))
  elsif item.is_machine?   # TM or HM
    pbMessage(_INTL("\\me[{1}]You obtained \\c[1]{2} {3}\\c[0]!\\wtnp[30]","DPPT 031 Get TMHM",itemname,GameData::Move.get(move).name))
  elsif quantity>1

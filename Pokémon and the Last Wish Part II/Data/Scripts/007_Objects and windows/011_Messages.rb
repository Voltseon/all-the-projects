...
def pbDisplayCoinsWindow(msgwindow,goldwindow)
  ...
  coinwindow=Window_AdvancedTextPokemon.new(_INTL("Chips:\n<ar>{1}</ar>",coinString))
...
def pbDisplayBattlePointsWindow(msgwindow)
  ...
  pointswindow=Window_AdvancedTextPokemon.new(_INTL("Cable Points:\n<ar>{1}</ar>", pointsString))
...
def pbFreeTextBig(msgwindow,currenttext,passwordbox,maxlength,width=240,height=128)
  window=Window_MultilineTextEntry.new(currenttext,0,384-height,width,height)
  ret=""
  window.maxlength=maxlength
  window.visible=true
  window.z=99999

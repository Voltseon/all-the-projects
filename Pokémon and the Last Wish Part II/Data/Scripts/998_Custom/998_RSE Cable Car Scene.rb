...
if defined?(PluginManager)
  PluginManager.register({
    ...
    :link => "https://reliccastle.com/resources/536/"
  })
end
...
class CableCarScene
  def initialize(going_up, map_id, map_x, map_y, direction=2, weather=true)
    ...
    if going_up
      ...
      if !$Trainer.isFemale?
        @sprites["Player"].bitmap = Bitmap.new("Graphics/Pictures/CableCar/player_male_#{$Trainer.outfit}")
      else
        @sprites["Player"].bitmap = Bitmap.new("Graphics/Pictures/CableCar/player_female_#{$Trainer.outfit}")
      end
      ...
      for i in 0..CABLE_CAR_DURATION
        if i < 4
          @sprites["Black"].opacity -= 50
        elsif i == 4
          @sprites["Black"].opacity -= 55
        elsif i >= CABLE_CAR_DURATION-4 && i < CABLE_CAR_DURATION
          @sprites["Black"].opacity += 50
        elsif i == CABLE_CAR_DURATION
          @sprites["Black"].opacity += 55
        end
        if i >= 26 && i <=30
          @sprites["Ash"].opacity += 50 if weather
        end
        @sprites["Ash"].y += 3 * CABLE_CAR_SPEED if weather
        @sprites["Ash"].x += 8 * CABLE_CAR_SPEED if weather
        @sprites["Ground"].y += 1 * CABLE_CAR_SPEED
        @sprites["Ground"].x += 4 * CABLE_CAR_SPEED
        @sprites["Hiker"].x += 2 * CABLE_CAR_SPEED if hiker
        @sprites["CC"].x -= 2 * CABLE_CAR_SPEED
        @sprites["Door"].x -= 2 * CABLE_CAR_SPEED
        @sprites["Player"].x -= 2 * CABLE_CAR_SPEED
        @sprites["Poles"].x += 6 * CABLE_CAR_SPEED
        @sprites["Poles"].y += 3 * CABLE_CAR_SPEED
        pbWait((2 / CABLE_CAR_SPEED).round)
        @sprites["Ash"].y += 3 * CABLE_CAR_SPEED if weather
        @sprites["Ash"].x += 8 * CABLE_CAR_SPEED if weather
        @sprites["Ground"].y += 1 * CABLE_CAR_SPEED
        @sprites["Ground"].x += 4 * CABLE_CAR_SPEED
        @sprites["Hiker"].y += 1 * CABLE_CAR_SPEED if hiker
        @sprites["Hiker"].x += 4 * CABLE_CAR_SPEED if hiker
        @sprites["Trees"].y += 1 * CABLE_CAR_SPEED
        @sprites["Trees"].x += 1 * CABLE_CAR_SPEED
        @sprites["Poles"].x += 6 * CABLE_CAR_SPEED
        @sprites["Poles"].y += 3 * CABLE_CAR_SPEED
        pbWait((2 / CABLE_CAR_SPEED).round)
        @sprites["Ash"].y += 3 * CABLE_CAR_SPEED if weather
        ...
        @sprites["Trees"].y += 1 * CABLE_CAR_SPEED
        @sprites["Trees"].x += 1 * CABLE_CAR_SPEED
        @sprites["CC"].y -= 2 * CABLE_CAR_SPEED
        @sprites["CC"].x -= 2 * CABLE_CAR_SPEED
        @sprites["Door"].y -= 2 * CABLE_CAR_SPEED
        @sprites["Door"].x -= 2 * CABLE_CAR_SPEED
        @sprites["Player"].y -= 2 * CABLE_CAR_SPEED
        @sprites["Player"].x -= 2 * CABLE_CAR_SPEED
        if i % 2 == 0
          @sprites["Player"].y -= 2
      ...
      if !$Trainer.isFemale?
        ...
        @sprites["Player"].bitmap = Bitmap.new("Graphics/Pictures/CableCar/player_female#{$Trainer.outfit}")
      end

...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    pbDrawShadowText(@sprites["text"].bitmap,8,16,118,26,
       _INTL("Your chips"),Color.new(60,60,60),Color.new(216,195,147),1)
    pbDrawShadowText(@sprites["text"].bitmap,8,82,118,26,
       _INTL("Earned chips"),Color.new(60,60,60),Color.new(216,195,147),1)
    # Draw current level
    pbDrawShadowText(@sprites["level"].bitmap,8,150,118,28,
       _INTL("Level {1}",@level.to_s),Color.new(60,60,60),Color.new(216,195,147),1)
    ...
    if $Trainer.coins >= Settings::MAX_COINS
      pbMessage(_INTL("You've gathered {1} Chips. You cannot gather any more.", Settings::MAX_COINS.to_s_formatted))
        ...
        for i in 0...@squares.length
          if @index[0]*64+128==@squares[i][0] && @index[1]*64==@squares[i][1] && @squares[i][3]==false
            ...
            if @squares[i][2]==0
              ...
              pbMessage(_INTL("\\me[Voltorb Flip game over]Oh no! You get 0 Chips!\\wtnp[50]"))
              pbShowAndDispose
              @sprites["mark"].bitmap.clear
              if @level>1
              ...
              pbDrawShadowText(@sprites["level"].bitmap,8,150,118,28,"Level "+@level.to_s,Color.new(60,60,60),Color.new(216,195,147),1)
              @points=0
              pbUpdateCoins
...
#        pbMessage(_INTL("This means you've found all the Coins in this game, so the game is now over."))
        pbMessage(_INTL("\\se[Voltorb Flip gain coins]{1} received {2} Chips!",$Trainer.name,@points.to_s_formatted))
        # Update level text
        @sprites["level"].bitmap.clear
        pbDrawShadowText(@sprites["level"].bitmap,8,150,118,28,_INTL("Level {1}",@level.to_s),Color.new(60,60,60),Color.new(216,195,147),1)
...
#          end
      ...
      @sprites["curtain"].opacity=100
      if @points==0
        if pbConfirmMessage("You haven't found any Chips! Are you sure you want to quit?")
          @sprites["curtain"].opacity=0
          pbShowAndDispose
          @quit=true
        end
      elsif pbConfirmMessage(_INTL("If you quit now, you will recieve {1} Chip(s). Will you quit?",@points.to_s_formatted))
        pbMessage(_INTL("{1} received {2} Chip(s)!",$Trainer.name,@points.to_s_formatted))
        $Trainer.coins+=@points
        @points=0
        pbUpdateCoins
        @sprites["curtain"].opacity=0
...
def pbVoltorbFlip
  if GameData::Item.exists?(:COINCASE) && !$PokemonBag.pbHasItem?(:COINCASE)
    pbMessage(_INTL("You can't play unless you have a Chip Case."))
  elsif $Trainer.coins == Settings::MAX_COINS
    pbMessage(_INTL("Your Chip Case is full!"))
  else

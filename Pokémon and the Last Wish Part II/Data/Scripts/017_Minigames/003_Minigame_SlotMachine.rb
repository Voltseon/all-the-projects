  ...
  def pbMain
    ...
    loop do
      ...
      if @sprites["credit"].score == Settings::MAX_COINS
        pbMessage(_INTL("You've got {1} Chips.", Settings::MAX_COINS.to_s_formatted))
        break
      elsif $Trainer.coins==0
        pbMessage(_INTL("You've run out of Chips.\nGame over!"))
        break
...
def pbSlotMachine(difficulty=1)
  if GameData::Item.exists?(:COINCASE) && !$PokemonBag.pbHasItem?(:COINCASE)
    ...
    pbMessage(_INTL("You don't have any Chips to play!"))

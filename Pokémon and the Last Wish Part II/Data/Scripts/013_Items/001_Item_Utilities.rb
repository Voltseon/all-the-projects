...
#===============================================================================
def pbBikeCheck
  if $PokemonGlobal.surfing || $PokemonGlobal.diving || $PokemonGlobal.scuba ||
     (!$PokemonGlobal.bicycle && $game_player.pbTerrainTag.must_walk) || $PokemonGlobal.extremespeed
...
def pbUseItem(bag,item,bagscene=nil)
  ...
  if itm.is_machine?    # TM or TR or HM
    ...
    if itm.id == :OUTFITKIT
      outfitable = [:BUIZEL,:FLOATZEL,:WOOPER,:QUAGSIRE,:MAWILE,:SABLEYE,:SPIRITOMB,:CACNEA_1,:CACTURNE_1,:CACNEA_2,:CACTURNE_2,
        :BULBASAUR,:IVYSAUR,:VENUSAUR,:TOTODILE,:CROCONAW,:FERALIGATR,:TORCHIC,:COMBUSKEN,:BLAZIKEN,:CELEBI,:MANKEY,:PRIMEAPE,:PORYGON2,:PORYGONZ]
      annot = []
      for pkmn in $Trainer.party
        elig = outfitable.include?(pkmn.species)
        annot.push((elig) ? _INTL("ABLE") : _INTL("NOT ABLE"))
      end
    end
    if itm.id == :FORMDEVICE
      devicable = [:ROTOM,:SHUCKLE,:JIRACHI,:DEOXYS,:MACHAMP]
      annot = []
      for pkmn in $Trainer.party
        elig = devicable.include?(pkmn.species)
        annot.push((elig) ? _INTL("ABLE") : _INTL("NOT ABLE"))
      end
    end
    pbFadeOutIn {

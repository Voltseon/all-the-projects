...
class SpritePositioner
  def pbOpen
    ...
    @viewport.z = 99999
    battlebg   = "Graphics/Battlebacks/field_bg"
    playerbase = "Graphics/Battlebacks/default_base0"
    enemybase  = "Graphics/Battlebacks/default_base1"
  ...
  def pbChangeSpecies(species)
    ...
    @sprites["pokemon_0"].setSpeciesBitmap(spe, 0, frm, rand(0..1) == 1, false, true)
    @sprites["pokemon_1"].setSpeciesBitmap(spe, 0, frm, rand(0..1) == 1)
  ...
  def pbChooseSpecies
    ...
    cw.y = Graphics.height - cw.height
    allspecies = []
    pbLoadRegionalDexes.each_with_index { |d, index|
      d.each do |s|
    ...
    allspecies.push([:MACHAMP_1, GameData::Species.get(:MACHAMP_1).species, "Machamp (form 1)"])
    allspecies = allspecies.uniq

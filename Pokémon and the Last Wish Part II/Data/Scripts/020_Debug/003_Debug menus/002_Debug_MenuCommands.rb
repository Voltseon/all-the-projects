...
DebugMenuCommands.register("demoparty", {
  ...
  "effect"      => proc {
    party = []
    species = [:FERALIGATR, :STARAPTOR, :FLOATZEL, :SPIRITOMB, :TYRANITAR, :CELEBI]
    for id in species
...
DebugMenuCommands.register("setbp", {
  "parent"      => "playermenu",
  "name"        => _INTL("Set Cable Points"),
  "description" => _INTL("Edit how many Cable Points you have."),
  "effect"      => proc {
    params = ChooseNumberParams.new
    params.setRange(0, Settings::MAX_BATTLE_POINTS)
    params.setDefaultValue($Trainer.battle_points)
    $Trainer.battle_points = pbMessageChooseNumber(_INTL("Set the player's CP amount."), params)
    pbMessage(_INTL("You now have {1} CP.", $Trainer.battle_points.to_s_formatted))
  }
})

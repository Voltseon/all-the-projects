...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def initialize(bag,filterlist,pocket,x,y,width,height)
    ...
    @basewindow = SpriteWindow_Base.new(0,0,50,50)
  ...
  def refresh
    if @basewindow
      if isDarkWindowskin(@basewindow.windowskin)
        @selarrow  = AnimatedBitmap.new("Graphics/Pictures/Bag/cursor")
        @swaparrow  = AnimatedBitmap.new("Graphics/Pictures/Bag/cursor_swap")
      else
        @selarrow  = AnimatedBitmap.new("Graphics/Pictures/Bag/cursor_dark")
        @swaparrow  = AnimatedBitmap.new("Graphics/Pictures/Bag/cursor_swap_dark")
      end
    end
    @item_max = itemCount()
...
class PokemonBag_Scene
  ...
  DARKTEXT              = Color.new(51,51,51)
  DARKTEXTSHADOW        = Color.new(206,206,206)
  ITEMSVISIBLE          = 7
  ...
  def pbStartScene(bag,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sprites["itemlist"] = Window_PokemonBag.new(@bag,@filterlist,lastpocket,168,-6,314,40+28+ITEMSVISIBLE*32)
    ...
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize("",
       72, 272, Graphics.width - 72 - 24, 124, @viewport)
    ...
    @sprites["cmdwindow"] = SpriteWindow_Base.new(186,2,278,276)
    cmdwindow = @sprites["cmdwindow"]
    cmdwindow.z        = @sprites["itemtext"].z-1
  ...
  def pbRefresh
    if isDarkWindowskin(@sprites["msgwindow"].windowskin)
      @sprites["itemlist"].baseColor   = ITEMLISTBASECOLOR
      @sprites["itemlist"].shadowColor = ITEMLISTSHADOWCOLOR
    else
      @sprites["itemlist"].baseColor   = DARKTEXT
      @sprites["itemlist"].shadowColor = DARKTEXTSHADOW
    end
  ...
  def pbRefreshIndexChanged
    ...
    if itemlist.top_row>0
      overlay.blt(470,20,@sliderbitmap.bitmap,Rect.new(0,0,36,38))
      showslider = true
    end
    ...
    @sprites["itemtext"].text =
      if itemlist.item
        item = GameData::Item.get(itemlist.item)
        (item.move) ? GameData::Move.get(item.move).description : item.description
      else
        _INTL("Close bag.")
      end
  end

...
class MapBottomSprite < SpriteWrapper
  ...
  def refresh
    self.bitmap.clear
    textpos = [
       [@mapname,18,-8,0,Color.new(248,248,248),Color.new(72,72,72)],
       [@maplocation,18,348,0,Color.new(248,248,248),Color.new(72,72,72)],
       [@mapdetails,Graphics.width-16,348,1,Color.new(248,248,248),Color.new(72,72,72)]
    ]
...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(aseditor=false,mode=0)
    ...
    addBackgroundOrColoredPlane(@sprites,"background","mapbg",Color.new(72,72,72),@viewport)
    ...
    Settings::ROAMING_SPECIES.each_with_index do |pkmn, index|
      next if !$game_switches[pkmn[2]]
      next if $PokemonGlobal.roamPokemon[index]==true
      roamingPosition = GameData::MapMetadata.get($PokemonGlobal.roamPosition[index]).town_map_position
      next if roamingPosition.nil?
      @sprites["roaming#{index}"] = IconSprite.new(0,0,@viewport)
      @sprites["roaming#{index}"].setBitmap("Graphics/Pictures/roaming#{pkmn[0]}.png")
      @sprites["roaming#{index}"].x = -SQUAREWIDTH/2+(roamingPosition[1]*SQUAREWIDTH)+(Graphics.width-@sprites["map"].bitmap.width)/2
      @sprites["roaming#{index}"].y = -SQUAREHEIGHT/2+(roamingPosition[2]*SQUAREHEIGHT)+(Graphics.height-@sprites["map"].bitmap.height)/2
    end
    if mode>0
      k = 0
      for i in LEFT..RIGHT

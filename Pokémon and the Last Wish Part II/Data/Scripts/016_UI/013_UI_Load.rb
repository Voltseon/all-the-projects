  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if @isContinue
        ...
        textpos.push([_INTL("Stamps:"),16*2,53*2,0,TEXTCOLOR,TEXTSHADOWCOLOR])
...
class PokemonLoad_Scene
  def pbStartScene(commands, show_continue, trainer, frame_count, map_id)
    ...
    $DiscordRPC.details = "Idling on the Load Screen"
    $DiscordRPC.large_image = "icon"
    $DiscordRPC.large_image_text = "Pokémon and the Last Wish Part II"
    $DiscordRPC.timestamp_start = Time.now.to_i
    $DiscordRPC.update
  end

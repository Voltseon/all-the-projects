...
class PokemonPartyPanel < SpriteWrapper
  ...
  def initialize(pokemon,index,viewport=nil)
    ...
    @outfitsprite = ChangelingSprite.new(0,0,viewport)
    @outfitsprite.addBitmap("outfit","Graphics/Pictures/Party/icon_outfit")
    @outfitsprite.z = self.z+3
    @outfitsprite.changeBitmap("outfit")
  ...
  def outfitcheck
    ret = false
    if @pokemon.form == 1 && [:BULBASAUR,:IVYSAUR,:VENUSAUR,:TOTODILE,:CROCONAW,
      :FERALIGATR,:TORCHIC,:COMBUSKEN,:BLAZIKEN,:CELEBI,:PORYGON2,:PORYGONZ].include?(@pokemon.species)
        ret = true
    elsif @pokemon.form == 2 && [:BUIZEL,:FLOATZEL,:WOOPER,:QUAGSIRE,:MAWILE,
        :SABLEYE,:SPIRITOMB,:CACNEA,:CACTURNE,:MANKEY,:PRIMEAPE].include?(@pokemon.species)
        ret = true
    end
    return ret
  end
  ...
  def refresh
    ...
    @outfitsprite.visible = outfitcheck if @outfitsprite && !@outfitsprite.disposed?
    ...
    if @outfitsprite && !@outfitsprite.disposed?
      if @outfitsprite.visible
        @outfitsprite.x     = self.x+62
        @outfitsprite.y     = self.y+16
        @outfitsprite.color = self.color
      end
    end
    if @overlaysprite && !@overlaysprite.disposed?
      @overlaysprite.x     = self.x
  ...
  def update
    ...
    @outfitsprite.update if @outfitsprite && !@outfitsprite.disposed?
  end
end
...
class PokemonParty_Scene
  ...
  def pbShowCommandsF(text,commands,index=0)
    ret = -1
    helpwindow = @sprites["helpwindow"]
    helpwindow.visible = true
    using(cmdwindow = Window_CommandPokemonColor.new(commands)) {
  ...
  def pbUseTM(bag,pokemon)
    ...
    pbFadeOutIn {
      ...
      ret = screen.pbChooseItemScreen(Proc.new { |item|
        ...
        next false if pokemon.hasMove?(move) || !pokemon.compatible_with_move?(move)
        next false if !GameData::Item.get(item).is_machine?
        next true
      })
      yield if block_given?
    }
  ...
  def pbCheckTM(bag,pokemon)
    ret = false
    bag.pockets[4].each do |item|
      itm = GameData::Item.get(item[0])
      next false if !itm.is_machine?
      move = itm.move
      next false if pokemon.hasMove?(move) || !pokemon.compatible_with_move?(move)
      ret = true
    end
...
class PokemonPartyScreen
  ...
  def pbShowCommandsF(helptext,commands,index=0)
    return @scene.pbShowCommandsF(helptext,commands,index)
  end
  ...
  def pbChooseMoveF(pokemon,helptext,index=0)
    movenames = []
    for i in pokemon.moves
      next if !i || !i.id
      if i.total_pp<=0
    ...
    return @scene.pbShowCommandsF(helptext,movenames,index)
  end
  ...
  def pbPokemonScreen
    ...
    loop do
      ...
      if !pkmn.egg?
        commands[cmdChangeMoves = commands.length]     = _INTL("Moves")
        if pkmn.mail
      ...
      cmdMoves.each_with_index do |cmd, i|
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(pkmn.moves[i].id)
          ...
          if pbConfirmUseHiddenMove(pkmn,pkmn.moves[i].id)
            @scene.pbEndScene
            if pkmn.moves[i].id == :PSYCHOMATTER
      ...
      if cmdSummary>=0 && command==cmdSummary
        ...
        pbSet(4, species_name)
        pbSet(5, pbEnterPokemonName(_INTL("{1}'s nickname?", pkmn.speciesName), 0, Pokemon::MAX_NAME_SIZE, "", pkmn))
        if pbGet(5) == pbGet(4) || pbGet(5) == ""
          # Resets the Pokémon's Nickname
          pkmn.name = nil
        else
          pkmn.name = pbGet(5)
        end
      elsif cmdChangeMoves>=0 && command==cmdChangeMoves
        # Change Moves
        changemovescommands = []
        cmdLearnMoves  = -1
        cmdTeachMoves  = -1
        cmdRemoveMoves = -1
        # Build the commands
        changemovescommands[cmdTeachMoves=changemovescommands.length]   = _INTL("Teach") if @scene.pbCheckTM($PokemonBag,pkmn)
        changemovescommands[cmdLearnMoves=changemovescommands.length]   = _INTL("Relearn") if pkmn.can_relearn_move?
        changemovescommands[cmdRemoveMoves=changemovescommands.length]  = _INTL("Forget") if pkmn.moves.length>1
        changemovescommands[changemovescommands.length]                 = _INTL("Cancel")
        command = @scene.pbShowCommands(_INTL("Do what with {1}'s moveset?",pkmn.name),changemovescommands)
        if cmdLearnMoves>=0 && command==cmdLearnMoves   # Learn
          pbRelearnMoveScreen(pkmn)
        elsif cmdRemoveMoves>=0 && command==cmdRemoveMoves   # Forget
          moveindex = pbChooseMoveF(pkmn, _INTL("Forget which move?"),0)
          if moveindex >= 0
            movename = pkmn.moves[moveindex].name
            if @scene.pbConfirmMessageSerious(_INTL("Are you sure you want {1} to forget how to use {2}?", pkmn.name, movename))
              pkmn.forget_move_at_index(moveindex)
              @scene.pbMessage(_INTL("1, 2, and...\\wt[16] ...\\wt[16] ... Poof!\\se[Battle ball drop]\1"))
              @scene.pbMessage(_INTL("{1} forgot how to use {2}.", pkmn.name, movename))
              pbRefreshSingle(pkmnid)
            end
          end
        elsif cmdTeachMoves>=0 && command==cmdTeachMoves   # Teach
          item = @scene.pbUseTM($PokemonBag,pkmn) {
            @scene.pbSetHelpText((@party.length>1) ? _INTL("Choose a Pokémon.") : _INTL("Choose Pokémon or cancel."))
          }
          if item

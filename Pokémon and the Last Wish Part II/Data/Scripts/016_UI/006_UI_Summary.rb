...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
       ...
       [@pokemon.level.to_s,46,86,0,base,shadow],
       [_INTL("Item"),66,312,0,base,shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name,16,346,0,base,shadow])
    else
      textpos.push([_INTL("None"),16,346,0,base,shadow])
    end
    # Write the Pokémon's happiness
    textpos.push([_INTL("{1}",@pokemon.happiness),170,312,0,base,Color.new(51,51,51)]) 
    # Write the gender symbol
    if @pokemon.male?
      textpos.push([_INTL("♂"),178,56,0,Color.new(24,112,216),shadow])
    elsif @pokemon.female?
      textpos.push([_INTL("♀"),178,56,0,Color.new(248,56,32),shadow])
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw the Pokémon's markings
    drawMarkings(overlay,84,294)
  ...
  def drawPageOne
    ...
    base   = Color.new(248,248,248)
    shadow = Color.new(72,72,72)
    dexNumBase   = (@pokemon.shiny?) ? Color.new(248,56,32) : base
    dexNumShadow = (@pokemon.shiny?) ? Color.new(224,152,144) : shadow
    ...
    textpos = [
       ...
       [@pokemon.speciesName,435,106,2,base,shadow],
    ...
    dexnumshift = false
    if $Trainer.pokedex.unlocked?(2)   # National Dex is unlocked
      dexnumshift = true if Settings::DEXES_WITH_OFFSETS.include?(2)
    else
      dexnum = 0
    ...
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("RENTAL"),435,170,2,base,shadow])
      textpos.push(["?????",435,202,2,base,shadow])
    else
      ownerbase   = base
      ownershadow = shadow
      ...
      textpos.push([sprintf("%05d",@pokemon.owner.public_id),435,202,2,base,shadow])
    end
    ...
    if @pokemon.shadowPokemon?
      ...
      memo = sprintf("<c3=F8F8F8,484848>%s\n",heartmessage)
      drawFormattedTextEx(overlay,234,304,264,memo)
    else
      endexp = @pokemon.growth_rate.minimum_exp_for_level(@pokemon.level + 1)
      textpos.push([_INTL("Exp. Points"),238,234,0,base,shadow])
      textpos.push([@pokemon.exp.to_s_formatted,488,266,1,base,shadow])
      textpos.push([_INTL("To Next Lv."),238,298,0,base,shadow])
      textpos.push([(endexp-@pokemon.exp).to_s_formatted,488,330,1,base,shadow])
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw Pokémon type(s)
  ...
  def drawPageOneEgg
    ...
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("None"),16,346,0,Color.new(192,200,208),Color.new(72,72,72)])
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    memo = ""
    ...
    if @pokemon.timeReceived
      ...
      memo += _INTL("<c3=F8F8F8,484848>{1} {2}, {3}\n",date,month,year)
    end
    # Write map name egg was received on
    mapname = pbGetMapNameFromId(@pokemon.obtain_map)
    mapname = @pokemon.obtain_text if @pokemon.obtain_text && !@pokemon.obtain_text.empty?
    if mapname && mapname != ""
      memo += _INTL("<c3=F8F8F8,484848>A mysterious Pokémon Egg received from <c3=F83820,484848>{1}<c3=F8F8F8,484848>.\n",mapname)
    else
      memo += _INTL("<c3=F8F8F8,484848>A mysterious Pokémon Egg.\n",mapname)
    end
    memo += "\n" # Empty line
    # Write Egg Watch blurb
    memo += _INTL("<c3=F8F8F8,484848>\"The Egg Watch\"\n")
  ...
  def drawPageTwo
    ...
    if showNature
      ...
      memo += _INTL("<c3=F83820,484848>{1}<c3=F8F8F8,484848> nature.\n",natureName)
    end
    # Write date received
    if @pokemon.timeReceived
      date  = @pokemon.timeReceived.day
    ...
    memo += sprintf("<c3=F83820,484848>%s\n",mapname)
    ...
    if @pokemon.obtain_method == 1
      ...
      mapname = _INTL("Faraway place") if nil_or_empty?(mapname)
      memo += sprintf("<c3=F83820,484848>%s\n",mapname)
      memo += _INTL("<c3=F8F8F8,484848>Egg hatched.\n")
    else
      memo += "\n"   # Empty line
    end
  ...
  def drawPageThree
    ...
    if !@pokemon.shadowPokemon? || @pokemon.heartStage > 3
      @pokemon.nature_for_stats.stat_changes.each do |change|
        statshadows[change[0]] = Color.new(165,114,93) if change[1] > 0
    ...
    textpos = [
       ...
       [sprintf("%d/%d",@pokemon.hp,@pokemon.totalhp),462,70,1,base,shadow],
       [_INTL("Attack"),248,114,0,base,statshadows[:ATTACK]],
       [sprintf("%d",@pokemon.attack),456,114,1,base,shadow],
       [_INTL("Defense"),248,146,0,base,statshadows[:DEFENSE]],
       [sprintf("%d",@pokemon.defense),456,146,1,base,shadow],
       [_INTL("Sp. Atk"),248,178,0,base,statshadows[:SPECIAL_ATTACK]],
       [sprintf("%d",@pokemon.spatk),456,178,1,base,shadow],
       [_INTL("Sp. Def"),248,210,0,base,statshadows[:SPECIAL_DEFENSE]],
       [sprintf("%d",@pokemon.spdef),456,210,1,base,shadow],
       [_INTL("Speed"),248,242,0,base,statshadows[:SPEED]],
       [sprintf("%d",@pokemon.speed),456,242,1,base,shadow],
       [_INTL("Ability"),224,278,0,base,shadow]
    ]
    # Draw ability name and description
    ability = @pokemon.ability
    if ability
      textpos.push([ability.name,362,278,0,Color.new(177,255,130),shadow])
      drawTextEx(overlay,224,320,282,2,ability.description,Color.new(255,188,94),shadow)
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw HP bar
    if @pokemon.hp>0
      w = @pokemon.hp*100*1.0/@pokemon.totalhp
      w = 1 if w<1
      w = ((w/2).round)*2
      hpzone = 0
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    if @pokemon.type1==@pokemon.type2
      ...
      overlay.blt(88,78,@typebitmap.bitmap,type1rect)
      overlay.blt(158,78,@typebitmap.bitmap,type2rect)
    end
  end
...
def drawPageFive
  overlay = @sprites["overlay"].bitmap
   ...
   # Draw ability name and description
    ability = @pokemon.ability
    if ability
      textpos.push([ability.name,374,278,0,Color.new(177,255,130),shadow])
      drawTextEx(overlay,224,320,282,2,ability.description,Color.new(255,188,94),shadow)
    end
   pbDrawTextPositions(overlay,textpos)
  end
...
class PokemonSummaryScreen
  ...
  def pbStartForgetScreen(party,partyindex,move_to_learn)
    ...
    loop do
      ...
      if pbConfirmMessage(_INTL("Are you sure you want to remove a HM Move?."))
        break
      else
        @scene.pbUpdate
      end

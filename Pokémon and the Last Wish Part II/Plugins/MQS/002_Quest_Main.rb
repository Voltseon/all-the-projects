...
class Player_Quests
  ...
  def activateQuest(quest,color,story)
    ...
    #pbMessage(_INTL("\\se[{1}]<ac>\\rNew mission received!\\c[0]\nCheck your mission log for more details!</ac>",QUEST_JINGLE))
  end
  ...
  def failQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        #pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Mission failed!</c2>\nYour mission log has been updated!</ac>",QUEST_FAIL))
        break
      end
    end
    if !found
  ...
  def completeQuest(quest,color,story)
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        @active_quests.delete_at(i)
        found = true
        #pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Mission completed!</c2>\nYour mission log has been updated!</ac>",QUEST_JINGLE))
        break
      end
    end
    if !found

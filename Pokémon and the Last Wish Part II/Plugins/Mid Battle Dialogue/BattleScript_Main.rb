...
# For Start Battle Messages
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      $DiscordRPC.details = "In a Wild Battle"
      foeParty = pbParty(1)
      case foeParty.length
      when 1
        $DiscordRPC.state = "Battling Lvl. " + foeParty[0].level.to_s + " " + foeParty[0].name
        $DiscordRPC.update
        # Edited
      ...
      when 2
        $DiscordRPC.details = "In a Wild Double Battle"
        $DiscordRPC.state = "Battling two Lvl. " + foeParty[0].level.to_s + " " + foeParty[0].name
        $DiscordRPC.update
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      when 3
        $DiscordRPC.state = "Battling wild " + foeParty[0].name + ", " + foeParty[1].name + " and " + foeParty[2].name
        $DiscordRPC.update
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      $DiscordRPC.details = "In a Trainer Battle"
      case @opponent.length
      when 1
        $DiscordRPC.state = "Battling " + @opponent[0].full_name
        $DiscordRPC.update
        # Edited
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      when 2
        $DiscordRPC.state = "Battling " + @opponent[0].full_name + " and " + @opponent[1].full_name
        $DiscordRPC.update
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      when 3
        $DiscordRPC.state = "Battling " + @opponent[0].full_name + ", " + @opponent[1].full_name + " and " + @opponent[2].full_name
        $DiscordRPC.update
        case TrainerDialogue.eval("battleStart")
        when -1
...
# Loss and Win Dialogue
  def pbEndOfBattle
    ...
    $DiscordRPC.details = "Exploring the Peskan Region"
    ...
    if damage>=target.hp
      ...
      if nonLethal?(user,target)
        ...
        if target.hasActiveAbility?(:STURDY) && !@battle.moldBreaker
          ...
          damage -= 1
        elsif (target.hasActiveItem?(:FOCUSBAND) || target.pokemon.poke_ball == :REVIVALBALL) && @battle.pbRandom(100)<10
...
class PokeBattle_Scene
  ...
  def pbInitSprites
    mbd_initSprites
    if @battle.wildBattle?
      trainerfile = "Graphics/Trainers/POKEMONTRAINER_Red"
      trainerfile = "Graphics/Trainers/POKEMONTRAINER_Leaf" if $Trainer.character_ID == 1

module FollowingPkmn
  ...
  #-----------------------------------------------------------------------------
  # Control the following Pokemon using VASP
  #-----------------------------------------------------------------------------
  def self.move_to_location(new_x, new_y, wait_complete = false)
    return if !FollowingPkmn.can_check?
    $PokemonGlobal.dependentEvents.each_with_index do |event,i|
      next if !event[8][/FollowerPkmn/]
      evnt = $PokemonTemp.dependentEvents.realEvents[i]
      pbAStarMoveRoute(evnt,calc_path_through(evnt, [new_x, new_y]), wait_complete)
    end
    pbMapInterpreter&.command_210 if wait_complete
  end
  #-----------------------------------------------------------------------------

...
class PokeSearch_Scene
  ...
  def setpreviouses
    if !getEncData.nil?
      @current_mon = $PokemonSystem.previous_mon if getEncData[0].include?($PokemonSystem.previous_mon);
    end
    if !$PokemonSystem.previous_repel.nil?
      @current_repel = $PokemonSystem.previous_repel if vHI($PokemonSystem.previous_repel)
    end
    if !$PokemonSystem.previous_berry.nil?
      @current_berry = $PokemonSystem.previous_berry if vHI($PokemonSystem.previous_berry)
    end
  end
  ...
  def selectBerry
    pbFadeOutIn {
      ...
      @current_berry = berry
      $PokemonSystem.previous_berry = @current_berry
    }
  end
  ...
  def description(item)
    ...
    when :MAXREPEL
      return _INTL("Because of the Max Repel encounters are guaranteed.")
    when :LUMBERRY
    ...
    when :JABOCABERRY
      return _INTL("Jaboca Berries increase the odds of male encounters")
    when :ROWAPBERRY
      return _INTL("Rowab Berries increase the odds of female encounters")
    else
  ...
  def selectMon
    ...
    if commands[command] != "Cancel"
      ...
      $PokemonSystem.previous_mon = @current_mon
    end
  end
  ...
  def selectRepel
    pbFadeOutIn {
      ...
      @current_repel = repel
      $PokemonSystem.previous_repel = @current_repel
    }
  end
  ...
  def startSearch
    ...
    if @current_berry == :LEPPABERRY
      level = [level-3,1].max
    elsif @current_berry != nil && ![:CHESTOBERRY, :CHERIBERRY, :PECHABERRY, :RAWSTBERRY, :PERSIMBERRY, :ASPEARBERRY, :LUMBERRY, :ORANBERRY, :SITRUSBERRY, :GOLDBERRY].include?(@current_berry)
  ...
  def drawPresent
    ...
    if @current_mon
      pbDrawTextPositions(overlay,[["#{GameData::Species.get(@current_mon).name}",Graphics.width/2,100,2,ITEMTEXTBASECOLOR,ITEMTEXTSHADOWCOLOR]])
    end
    case @index_ver
    when 0
      case @index_hor
...
class PokemonSystem
  ...
  attr_writer :previous_berry
  attr_writer :previous_repel
  attr_writer :previous_mon

  def previous_berry
    @previous_berry = nil if !@previous_berry
    return @previous_berry
  end

  def previous_repel
    @previous_repel = nil if !@previous_repel
    return @previous_repel
  end

  def previous_mon
    @previous_mon = nil if !@previous_mon
    return @previous_mon
  end

def pbAppearPokemonDayCare(args)
  # Iterate through both daycare mons
  $PokemonGlobal.daycare.each_with_index do |pkmn, index|
    # Check if there is a pokemon deposited in this slot
    if pkmn[0].nil?
      # Turn off selfswitch A for this event and break if no pokemon
      vSSF(args[index])
    else
      # Turn on selfswitch A if there is
      vSST(args[index])
      # Change the graphic of the mon
      pbChangeEventSpriteToMon(args[index],pkmn[0])
    end
  end
end
...
def pbChangeEventSpriteToMon(eventID, mon)
  ...
  thisevent = getEventFromID($game_map.map_id,eventID)
  # Setting up shininess, form and stuffs
  fname = _INTL("Followers/")
  fname = _INTL("Followers shiny/") if mon.shiny?
  fname += _INTL("{1}",GameData::Species.get(mon.species).id)
  fname += _INTL("_{1}",mon.form) if mon.form>0
  fname += _INTL(".png")
...
# Makes it so that you can talk to the Pokémon and it has dialogue.
def pbTalkToDaycareMon(id)
  pbRandMessageDaycare($PokemonGlobal.daycare[id][0])
end

def pbRandMessageDaycare(pkmn)
  value = 0
  case rand(6)
  when 0

...
def moveOutfits
  outfits = [:OUTFITBLUE, :OUTFITRED, :OUTFITGREEN, :OUTFITBLACK, :OUTFITGOLD, :OUTFITROYAL]
  outfits.each do |outfit|
    if ItemStorageHelper.pbQuantity($PokemonBag.pockets[8],outfit) > 0
      ItemStorageHelper.pbDeleteItem($PokemonBag.pockets[8], outfit, 1)
      vAI(outfit)
    end
  end
end
...
def updatePokemonLevel(pokemon, modifier)
  ...
  # Caps
  new_level = [1,new_level].max
  new_level = [100,new_level].min
...
Events.onWildBattleEnd += proc { |_sender, e|
  if !$Trainer.encounter_maps.include?($game_map.name) && !$Trainer.encounter_mons.include?(GameData::Species.get(e[0]).species) && $Trainer.nuzlocke_started
